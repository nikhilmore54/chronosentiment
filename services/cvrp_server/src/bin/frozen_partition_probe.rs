...
fn main() {
    ...
    for route_idx in 0..root_routes.len() {
        for cust_idx in 0..root_routes[route_idx].len() {
            ...
            for target_route_idx in 0..root_routes.len() {
                ...
                let target_load: i32 = root_routes[target_route_idx]
                    .iter()
                    .map(|&c| instance.customers[c].demand)
                    .sum();
    ...
    println!(
        "Frozen Probe Finished. Evaluated {} valid transfers. Skipped {} invalid capacity.",
        transitions_tested, invalid_capacity
    );
}
