...
impl Ord for State {
    fn cmp(&self, other: &Self) -> Ordering {
        ...
        match other.peak_dist.partial_cmp(&self.peak_dist).unwrap_or(Ordering::Equal) {
...
fn generate_neighbors(
    ...
    let mut add_neighbor = |new_routes: Vec<Vec<usize>>, move_name: String| {
        ...
        for r in &new_routes {
            ...
            if load > instance.capacity { return; }
        }
        ...
        if !involves_diff { return; }

        let overlap_tgt = new_set.intersection(target_edges).count();
        let dist: f64 = new_routes.iter().map(|r| calc_route_distance(instance, r)).sum();
    ...
    for r1 in 0..num_routes {
        for i in 0..routes[r1].len() {
            ...
            for r2 in 0..num_routes {
                let limit = if r1 == r2 { routes[r2].len() } else { routes[r2].len() + 1 };
                for j in 0..limit {
                    if r1 == r2 && (i == j || i + 1 == j) { continue; }
        ...
        for i in 0..routes[r1].len() {
            for r2 in r1..num_routes {
                ...
                for j in start_j..routes[r2].len() {
                    ...
                    add_neighbor(new_routes, format!("Exchange({}, {})", routes[r1][i], routes[r2][j]));
                }
            }
        }
    }
...
fn search_path(
    ...
    println!("\n=== Starting Bidirectional Search: {} ===", direction_name);
    ...
    let diff_edges: HashSet<_> = start_edges.symmetric_difference(&target_edges).cloned().collect();
    
    let start_dist: f64 = start_routes.iter().map(|r| calc_route_distance(instance, r)).sum();
    let target_dist: f64 = target_routes.iter().map(|r| calc_route_distance(instance, r)).sum();
    
    println!("Start Dist: {:.4}, Target Dist: {:.4}", start_dist, target_dist);
    println!("Start Overlap: {}", start_edges.intersection(&target_edges).count());
    ...
    let initial_state = State {
        ...
        path: vec![("Start".to_string(), start_dist, start_edges.intersection(&target_edges).count(), start_edges.len(), start_dist)],
    };
    ...
    while let Some(state) = pq.pop() {
        if state.overlap_target == 36 {
            ...
            for (step, (m, d, tgt, src, p)) in state.path.iter().enumerate() {
                println!("{:3} | {:20} | Dist: {:.4} | Peak: {:.4} | TgtOv: {} | SrcOv: {}", step, m, d, p, tgt, src);
            }
            return;
        }

        let canon = get_canonical_edges(&state.routes);
        if visited.contains_key(&canon) { continue; }
        visited.insert(canon, state.peak_dist);

        expanded += 1;
        if expanded % 1000 == 0 {
            println!("Expanded {}, PQ size: {}, Current state overlap: {}, Current peak: {:.4}", 
                expanded, pq.len(), state.overlap_target, state.peak_dist);
        }
        
        if expanded >= 5000 {
            println!("Hit expansion limit. Target not reached. Best Overlap: {}, Best Peak: {:.4}", state.overlap_target, state.peak_dist);
            break;
        }

        let neighbors = generate_neighbors(&state, instance, &diff_edges, &target_edges, &start_edges);
        for n in neighbors {
            pq.push(n);
        }
    }
