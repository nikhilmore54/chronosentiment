...
use cvrp::moga_impl::{CvrpEvaluator, CvrpLocalSearch, CvrpMutator, CvrpRouteAwareMutator};
...
fn run_bfs_probe(
...

    {
        let model = cvrp::moga_impl::CvrpConstraintModel {
            instance: evaluator.instance.clone(),
        };
        let budget = coralys_core::operators::OperatorBudget {
            max_iterations: 1,
            max_time_ms: 1000,
        };
        coralys_core::operators::ImprovementOperator::improve(ls, &mut root_opt, &model, &budget)
            .unwrap();
    }
    ...
    while let Some(current_cand) = queue.pop_front() {
        ...
        let current_eval = evaluator.evaluate(
            &current_cand,
            &coralys_moga::runtime::optimization::metric::MetricReport::default(),
        );
        ...
        for _ in 0..branching_factor {
            ...
            let child_eval = evaluator.evaluate(
                &child_opt,
                &coralys_moga::runtime::optimization::metric::MetricReport::default(),
            );
            ...
            writeln!(
                file,
                "{},{},{},{}",
                current_hash, child_hash, dist, is_elite
            )
            .unwrap();
    ...
    println!(
        "{} Probe Finished. Unique Elite Basins Found: {}",
        name,
        visited_basins.len()
    );
}
...
fn main() {
    ...
    let route_aware_mutator = CvrpRouteAwareMutator {
        instance: instance.clone(),
    };
