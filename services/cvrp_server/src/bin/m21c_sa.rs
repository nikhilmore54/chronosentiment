...
fn run_search(
    ...
    for _ in 0..iterations {
        if let Some(new_routes) = random_neighbor(instance, &current_routes, rng) {
            ...
            } else if delta == 0.0 { // Hill Climber accepts neutral moves
                accept = true;
            }
...
fn main() {
    ...
    println!("Chosen T_start: {:.4} (20% acceptance for 90th %ile)", t_start);
    ...
    let mut run_experiment = |name: &str, is_sa: bool| {
        ...
        for i in 0..10 {
            ...
            if stats.fp_797 { fp_797 += 1; }
            if stats.fp_795 { fp_795 += 1; }
            if stats.fp_792 { fp_792 += 1; }
            if stats.fp_790 { fp_790 += 1; }
        ...
        println!("Average Best BKS Overlap: {:.1}", sum_best_overlap as f64 / 10.0);
