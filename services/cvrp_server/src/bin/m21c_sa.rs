...
fn run_search(
    ...
    let mut best_overlap = get_canonical_edges(&current_routes)
        .intersection(bks_edges)
        .count();
    ...
    // geometric cooling: t_end = 0.01
    // t_end = t_start * alpha^iters => alpha = (t_end / t_start)^(1/iters)
    ...
    for _ in 0..iterations {
        if let Some(new_routes) = random_neighbor(instance, &current_routes, rng) {
            ...
            if delta < 0.0 {
                ...
                // Hill Climber accepts neutral moves
                accept = true;
            }
            ...
            if accept {
                ...
                let overlap = get_canonical_edges(&current_routes)
                    .intersection(bks_edges)
                    .count();
...
fn main() {
    ...
    println!("=== M21C-A: Neighborhood Calibration ===");
    ...
    let d_50 = deltas[(deltas.len() as f64 * 0.50) as usize];
    let d_90 = deltas[(deltas.len() as f64 * 0.90) as usize];
    let d_95 = deltas[(deltas.len() as f64 * 0.95) as usize];
    println!("50th %ile Δ: {:.2}", d_50);
    println!("90th %ile Δ: {:.2}", d_90);
    println!("95th %ile Δ: {:.2}", d_95);

    // T_start s.t. exp(-d_90 / T_start) = 0.2 => T_start = -d_90 / ln(0.2)
    let t_start = -d_90 / 0.2f64.ln();
    println!(
        "Chosen T_start: {:.4} (20% acceptance for 90th %ile)",
        t_start
    );

    let mut run_experiment = |name: &str, is_sa: bool| {
        println!("\n=== M21C: {} (10 runs, 1M iterations) ===", name);
        let mut fp_797 = 0;
        let mut fp_795 = 0;
        let mut fp_792 = 0;
        let mut fp_790 = 0;
        let mut fp_788 = 0;
        let mut sum_best_dist = 0.0;
        let mut sum_max_dist = 0.0;
        let mut sum_best_overlap = 0;
        let mut absolute_best_dist = 9999.0;
        let mut absolute_best_overlap = 0;

        for i in 0..10 {
            println!("Starting run {}/10...", i + 1);
            let stats = run_search(&instance, &r_797, &bks_edges, t_start, is_sa, &mut rng);
            if stats.fp_797 {
                fp_797 += 1;
            }
            if stats.fp_795 {
            ...
            sum_best_dist += stats.best_dist;
            sum_max_dist += stats.max_dist_seen;
            sum_best_overlap += stats.best_overlap;

            if stats.best_dist < absolute_best_dist {
                absolute_best_dist = stats.best_dist;
            }
            if stats.best_overlap > absolute_best_overlap {
                absolute_best_overlap = stats.best_overlap;
            }
        }

        println!("Average Best Dist: {:.4}", sum_best_dist / 10.0);
        println!("Absolute Best Dist: {:.4}", absolute_best_dist);
        println!("Average Max Peak Crossed: {:.4}", sum_max_dist / 10.0);
        println!(
            "Average Best BKS Overlap: {:.1}",
            sum_best_overlap as f64 / 10.0
        );
        println!("Absolute Best BKS Overlap: {}", absolute_best_overlap);
        println!("First-Passage Success Rates:");
        println!("  < 797: {}%", fp_797 * 10);
        println!("  < 795: {}%", fp_795 * 10);
        println!("  < 792: {}%", fp_792 * 10);
        println!("  < 790: {}%", fp_790 * 10);
        println!("  < 788: {}%", fp_788 * 10);
    };

    run_experiment("SA Search", true);
    run_experiment("Greedy Hill Climber", false);
}
