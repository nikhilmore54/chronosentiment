...
fn evaluate_config(name: &str, routes: &Vec<Vec<usize>>, instance: &CvrpInstance, bks_edges: &HashSet<(usize, usize)>) {
    ...
    println!("| {:8} | {:.4} | {:7} | {:9} | {:8} | {:11} |", 
        name, total_distance, missing_nodes.len(), duplicate_nodes.len(), capacity_violations, overlap);
}
...
fn main() {
    ...
    let dist_797: f64 = r_797.iter().map(|r| calc_route_distance(&instance, r)).sum();
    let dist_bks: f64 = r_bks.iter().map(|r| calc_route_distance(&instance, r)).sum();
    ...
    println!("Cluster 1 - 797 Segment Intrinsic Cost: {:.4}", calc_route_distance(&instance, &c1_797));
    println!("Cluster 1 - BKS Segment Intrinsic Cost: {:.4}", calc_route_distance(&instance, &c1_bks));
    
    let c2_797 = vec![7, 18, 20, 32, 22, 14, 27];
    let c2_bks = vec![27, 8, 14, 18, 20, 32, 22];
    println!("Cluster 2 - 797 Segment Intrinsic Cost: {:.4}", calc_route_distance(&instance, &c2_797));
    println!("Cluster 2 - BKS Segment Intrinsic Cost: {:.4}", calc_route_distance(&instance, &c2_bks));

    let c3_797 = vec![21, 6, 26, 11, 16, 10, 23, 30];
    let c3_bks = vec![30, 19, 9, 10, 23, 16, 11, 26, 6, 21];
    println!("Cluster 3 - 797 Segment Intrinsic Cost: {:.4}", calc_route_distance(&instance, &c3_797));
    println!("Cluster 3 - BKS Segment Intrinsic Cost: {:.4}", calc_route_distance(&instance, &c3_bks));
    ...
    for (name, swaps) in configs {
        ...
        routes.push(if swaps[0] == 1 { r_bks[0].clone() } else { r_797[0].clone() });
        routes.push(if swaps[1] == 1 { r_bks[1].clone() } else { r_797[1].clone() });
        routes.push(if swaps[2] == 1 { r_bks[2].clone() } else { r_797[2].clone() });
        routes.push(if swaps[1] == 1 { r_bks[3].clone() } else { r_797[3].clone() }); // C2 also affects R4
