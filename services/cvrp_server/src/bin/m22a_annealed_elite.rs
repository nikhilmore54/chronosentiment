...
fn main() {
    ...
    println!("=== M22A: Annealed Elite Refinement ===");
    println!(
    ...
    for seed_idx in 0..num_seeds {
        ...
        let mut sa_total_peak = 0.0;
        let mut sa_total_best = 0.0;
        let mut sa_total_final = 0.0;
        let mut sa_accepted_moves = 0;
        let mut sa_rejected_moves = 0;
        let mut sa_improving_moves = 0;
        let mut sa_worsening_moves = 0;
        let mut sa_bursts = 0;
        ...
        for generation in 1..=generations {
            ...
            if generation % 10 == 0 {
                ...
                for i in 0..next_gen.len() {
                    ...
                    let mut sa_peak_dist = 0.0; // Distance, so peak means worst (highest) distance encountered

                    let sa_steps = 500;
                    let mut t = 115.0; // T_start
                    let cooling = 0.99;

                    let mut curr_dist = start_dist;
                    for _ in 0..sa_steps {
                        let mut neighbor = sa_candidate.clone();
                        // fast random neighbor
                        let size = neighbor.permutation.len();
                        use rand::Rng;
                        let r: f64 = rng.gen_range(0.0..1.0);
                        if r < 0.3 {
                            let idx1 = rng.gen_range(0..size);
                            let idx2 = rng.gen_range(0..size);
                            neighbor.permutation.swap(idx1, idx2);
                        } else if r < 0.6 {
                            let val = neighbor.permutation.remove(rng.gen_range(0..size));
                            neighbor.permutation.insert(rng.gen_range(0..size), val);
                        } else {
                        ...
                        let neighbor_dist = evaluator
                            ...
                            .eval
                            .total_distance;
                        if neighbor_dist > sa_peak_dist {
                            sa_peak_dist = neighbor_dist;
                        } // Highest distance is the "peak" of worsening

                        let delta = neighbor_dist - curr_dist; // If < 0, it's an improvement (lower distance)
                        if delta < 0.0 {
                            sa_improving_moves += 1;
                        } else {
                            sa_worsening_moves += 1;
                        }
                        ...
                        if delta < 0.0 || rng.gen_range(0.0..1.0) < (-delta / t).exp() {
                            ...
                            if curr_dist < sa_best_dist {
                                sa_best_dist = curr_dist;
                                sa_best_candidate = sa_candidate.clone();
                            }
                        } else {
                            sa_rejected_moves += 1;
                        }
                    ...
                    next_gen[i] = sa_best_candidate;

                    sa_total_start += start_dist;
                    sa_total_peak += sa_peak_dist;
                    sa_total_best += sa_best_dist;
                    sa_total_final += curr_dist;
                    sa_bursts += 1;
        ...
        let avg_sa_peak = if sa_bursts > 0 {
            sa_total_peak / sa_bursts as f64
        } else {
            0.0
        };
        let avg_sa_best = if sa_bursts > 0 {
            sa_total_best / sa_bursts as f64
        } else {
            0.0
        };
        let avg_sa_final = if sa_bursts > 0 {
            sa_total_final / sa_bursts as f64
        } else {
            0.0
        };

        let avg_acc = if sa_bursts > 0 {
            sa_accepted_moves as f64 / sa_bursts as f64
        } else {
            0.0
        };
        let avg_rej = if sa_bursts > 0 {
            sa_rejected_moves as f64 / sa_bursts as f64
        } else {
            0.0
        };
        let avg_imp = if sa_bursts > 0 {
            sa_improving_moves as f64 / sa_bursts as f64
        } else {
            0.0
        };
        let avg_worsening = if sa_bursts > 0 {
            sa_worsening_moves as f64 / sa_bursts as f64
        } else {
            0.0
        };
        ...
        println!(
            "SA Stats -> Start: {:.2} | Peak Worsening: {:.2} | Best: {:.2} | Final: {:.2}",
            avg_sa_start, avg_sa_peak, avg_sa_best, avg_sa_final
        );
        println!(
            "SA Moves -> Acc: {:.1} | Rej: {:.1} | Imp: {:.1} | Wors: {:.1}",
            avg_acc, avg_rej, avg_imp, avg_worsening
        );
