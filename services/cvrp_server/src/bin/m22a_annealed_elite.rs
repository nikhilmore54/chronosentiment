...
fn main() {
    ...
    for seed_idx in 0..num_seeds {
                ...
                for i in 0..next_gen.len() {
                    ...
                    for _ in 0..sa_steps {
                        ...
                        if r < 0.3 {
                            ...
                            if idx1 > idx2 { std::mem::swap(&mut idx1, &mut idx2); }
        ...
        let avg_sa_peak = if sa_bursts > 0 { sa_total_peak / sa_bursts as f64 } else { 0.0 };
        let avg_sa_best = if sa_bursts > 0 { sa_total_best / sa_bursts as f64 } else { 0.0 };
        let avg_sa_final = if sa_bursts > 0 { sa_total_final / sa_bursts as f64 } else { 0.0 };
        
        let avg_acc = if sa_bursts > 0 { sa_accepted_moves as f64 / sa_bursts as f64 } else { 0.0 };
        let avg_rej = if sa_bursts > 0 { sa_rejected_moves as f64 / sa_bursts as f64 } else { 0.0 };
        let avg_imp = if sa_bursts > 0 { sa_improving_moves as f64 / sa_bursts as f64 } else { 0.0 };
        let avg_worsening = if sa_bursts > 0 { sa_worsening_moves as f64 / sa_bursts as f64 } else { 0.0 };
        ...
        println!("SA Stats -> Start: {:.2} | Peak Worsening: {:.2} | Best: {:.2} | Final: {:.2}", avg_sa_start, avg_sa_peak, avg_sa_best, avg_sa_final);
        println!("SA Moves -> Acc: {:.1} | Rej: {:.1} | Imp: {:.1} | Wors: {:.1}", avg_acc, avg_rej, avg_imp, avg_worsening);
