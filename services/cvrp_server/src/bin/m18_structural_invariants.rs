...
fn extract_edges(routes: &Vec<Vec<usize>>, instance: &CvrpInstance) -> Vec<(usize, usize)> {
    ...
    for r in routes {
        ...
        for &c_idx in r {
            ...
            let n1 = prev.min(c_id);
            let n2 = prev.max(c_id);
            edges.push((n1, n2));
            prev = c_id;
        }
...
fn random_ls(
    ...
    let mut cand = CvrpCandidate {
        permutation: (0..instance.customers.len()).collect(),
...

    {
        ...
        coralys_core::operators::ImprovementOperator::improve(ls, &mut cand, &model, &budget)
            .unwrap();
    }
...
fn sweep_ls(
    ...
    let mut angles: Vec<(usize, f64)> = (0..instance.customers.len())
        .map(|i| {
    ...
    angles.sort_by(|a, b| {
        let mut a_ang = a.1 + offset;
        if a_ang > PI {
            a_ang -= 2.0 * PI;
        }
        let mut b_ang = b.1 + offset;
        if b_ang > PI {
            b_ang -= 2.0 * PI;
        }
    ...
    let mut cand = CvrpCandidate {
        permutation: angles.into_iter().map(|(i, _)| i).collect(),
        last_mutation_op: None,
        last_mutation_radius: None,
        route_boundary_changes: None,
    };
...
fn main() {
    ...
    let mut process = |cand: CvrpCandidate, solver: &str| {
        ...
        writeln!(
            file,
            "{},{},{:.4},{},{}",
            solver, basin, dist, edge_str, trip_str
        )
        .unwrap();
    };
