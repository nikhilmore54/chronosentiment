...
fn main() {
    ...
    println!("=== M22B: Fitness + Novelty Elitism ===");
    ...
    for seed_idx in 0..num_seeds {
        ...
        for generation in 1..=generations {
            ...
            // 1 Global Best
            let best_fitness = evals[0].fitness();
            let threshold = 900.0f64.max(best_fitness + 100.0);
            
            next_gen.push(evals[0].genome().clone());
            let mut retained_edges = vec![get_canonical_edges(&evals[0].eval.routes)];
            
            // Up to 4 Novelty Elites
            let mut elite_count = 1;
            for eval in &evals[1..] {
                if elite_count >= 5 { break; }
                if eval.fitness() <= threshold {
                    let edges = get_canonical_edges(&eval.eval.routes);
                    let mut novel = true;
                    for arc_edges in &retained_edges {
                        if edges.symmetric_difference(arc_edges).count() < 10 {
                            novel = false;
                            break;
                        }
                    }
                    if novel {
                        next_gen.push(eval.genome().clone());
                        retained_edges.push(edges);
                        elite_count += 1;
                    }
                }
            }
