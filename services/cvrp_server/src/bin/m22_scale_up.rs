...
fn route_assignment_difference(
    sa_routes: &Vec<Vec<usize>>,
    ga_routes: &Vec<Vec<usize>>,
) -> (usize, usize) {
    let max_len = sa_routes.len().max(ga_routes.len());
    let mut sa_padded = sa_routes.clone();
    while sa_padded.len() < max_len {
        sa_padded.push(vec![]);
    }
    let mut ga_padded = ga_routes.clone();
    while ga_padded.len() < max_len {
        ga_padded.push(vec![]);
    }
    ...
    for (r_idx, r) in ga_padded.iter().enumerate() {
        for &c in r {
            ga_cust_to_route.insert(c, r_idx);
        }
    }
    ...
    fn permute(
        k: usize,
        arr: &mut Vec<usize>,
        sa_padded: &Vec<Vec<usize>>,
        ga_padded: &Vec<Vec<usize>>,
        ga_cust_to_route: &std::collections::HashMap<usize, usize>,
        best_moved: &mut usize,
        best_len_delta: &mut usize,
    ) {
        if k <= 1 {
            let mut moved = 0;
...
fn main() {
    ...
    for seed_idx in 0..num_seeds {
        ...
        for generation in 1..=generations {
            ...
            if generation % 10 == 0 && passive_archive.len() > 0 {
                ...
                for &idx in &sampled_indices {
                    ...
                    if best_dist < *start_dist {
                        ...
                        let overlap_pct = if sa_edges.len() > 0 {
                            intersection / sa_edges.len() as f64
                        } else {
                            1.0
                        };
                        ...
                        let route_count_change =
                            reinject_eval.eval.routes.len() as isize - best_routes.len() as isize;
                        let (customers_moved, max_length_delta) =
                            route_assignment_difference(&best_routes, &reinject_eval.eval.routes);
            ...
            if generation % 100 == 0 {
                let valid_pct = if sa_bursts > 0 {
                    sa_reinjections as f64 / sa_bursts as f64 * 100.0
                } else {
                    0.0
                };
                let mut current_best_arch = 9999.0;
                let mut current_max_bks = 0;
                for (d, _, edges, _) in &passive_archive {
                    if *d < current_best_arch {
                        current_best_arch = *d;
                    }
                    let ovl = edges.intersection(&bks_edges).count();
                    if ovl > current_max_bks {
                        current_max_bks = ovl;
                    }
                }
                println!(
                    "Gen {}: Best {:.2} | Arch Best {:.2} | Arch Fams {} | BKS Ovl {} | SA Bursts {} | SA Imp {} | Reinjects {} ({:.1}%)",
                    generation,
        ...
        println!(
            "Seed {} -> Dist: {:.2} | Archive Best: {:.2} | Archive Families: {}",
            seed,
            global_best_distance,
            best_archive_dist,
            passive_archive.len()
        ...
        for r in &audit_records {
            if r.edge_overlap >= 0.999 {
                b100_drift.push(r.distance_drift);
            } else if r.edge_overlap >= 0.90 {
                b90_drift.push(r.distance_drift);
            } else if r.edge_overlap >= 0.80 {
                b80_drift.push(r.distance_drift);
            } else {
                b70_drift.push(r.distance_drift);
            }
        }
        ...
        println!(
            "  100%    : {:4} samples | Median Drift: {:+.2}",
            b100_drift.len(),
            get_median(b100_drift)
        );
        println!(
            "  90-99%  : {:4} samples | Median Drift: {:+.2}",
            b90_drift.len(),
            get_median(b90_drift)
        );
        println!(
            "  80-89%  : {:4} samples | Median Drift: {:+.2}",
            b80_drift.len(),
            get_median(b80_drift)
        );
        println!(
            "  <80%    : {:4} samples | Median Drift: {:+.2}",
            b70_drift.len(),
            get_median(b70_drift)
        );
        ...
        for (i, r) in sorted_records.iter().take(5).enumerate() {
            println!(
                "  {}. Drift: {:+.2} | Edge Ovl: {:.1}% | BKS Drift: {:+2} | Cust Moved: {} | Route Cnt Chg: {:+2} | Max Len Delta: {}",
                i + 1,
                r.distance_drift,
                r.edge_overlap * 100.0,
                r.bks_overlap_drift,
                r.customers_moved,
                r.route_count_change,
                r.max_route_length_delta
            );
        }

        let mut non100_records: Vec<_> = audit_records
            .iter()
            .filter(|r| r.edge_overlap < 0.999)
            .collect();
