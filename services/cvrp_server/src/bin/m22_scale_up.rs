...
fn main() {
    ...
    for seed_idx in 0..num_seeds {
        ...
        for generation in 1..=generations {
            ...
            if generation % 100 == 0 {
                let valid_pct = if sa_bursts > 0 { sa_reinjections as f64 / sa_bursts as f64 * 100.0 } else { 0.0 };
                let mut current_best_arch = 9999.0;
                let mut current_max_bks = 0;
                for (d, _, edges, _) in &passive_archive {
                    if *d < current_best_arch { current_best_arch = *d; }
                    let ovl = edges.intersection(&bks_edges).count();
                    if ovl > current_max_bks { current_max_bks = ovl; }
                }
                println!("Gen {}: Best {:.2} | Arch Best {:.2} | Arch Fams {} | BKS Ovl {} | SA Bursts {} | SA Imp {} | Reinjects {} ({:.1}%)",
                    generation, global_best_distance, current_best_arch, passive_archive.len(), current_max_bks,
                    sa_bursts, sa_successes, sa_reinjections, valid_pct);
            }
