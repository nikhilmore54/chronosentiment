...
fn measure_preserved_subroutes(s1_routes: &Vec<Vec<usize>>, root_routes: &Vec<Vec<usize>>) -> usize {
    let mut count = 0;
    
    for root_r in root_routes {
        if root_r.len() < 3 { continue; }
        
        for window in root_r.windows(3) {
            let mut found = false;
            for s1_r in s1_routes {
                if s1_r.windows(3).any(|w| w == window || {
...
fn main() {
    ...
    for i in 0..instance.customers.len() {
        ...
        if best_nn != usize::MAX {
            let e = if i < best_nn { (i, best_nn) } else { (best_nn, i) };
            nn_edges.insert(e);
        }
    }
    ...
    let mut evaluate_child = |s1: &mut CvrpCandidate, source_type: &str| {
        ...
        let backbone_preserved = backbone_edges.intersection(&s1_edges).count() as f64 / backbone_edges.len() as f64;
