...
fn run_search(
    ...
    let start_dist = calc_total_dist(instance, &current_routes);
    ...
    println!(
        "    SA Stats -> Start: {:.2} | Peak Worsening: {:.2} | Best: {:.2}",
        start_dist, max_dist_seen, best_dist
    );
    println!(
        "    SA Moves -> Acc: {} | Rej: {} | Imp: {} | Wors: {}",
        accepted_moves, rejected_moves, improving_moves, worsening_moves
    );
...
fn main() {
    ...
    let ceilings = vec![900.0];

    for ceiling in ceilings {
        println!("\n=== M22C-0: Route-Aware SA Validation ===");
        let mut fp_788 = 0;
