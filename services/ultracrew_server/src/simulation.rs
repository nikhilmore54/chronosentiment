...
pub fn generate_baseline_schedule(
    ...
    use crate::optimizer::{ScheduleGenome, AssignmentSlot};
    ...
    let mapped_shift_types = vec![("Early", "Early"), ("Day", "Day"), ("Late", "Late"), ("Night", "Night")];
    ...
    for d in 0..num_days {
        ...
        for req in requirements {
            ...
            let mapped_shift = mapped_shift_types.iter().find(|(k, _)| *k == req.shift_type).map(|(_, v)| *v).unwrap_or("");
            for _ in 0..required {
        ...
        let mut available_nurses: Vec<String> = scenario.nurses.iter().map(|n| n.id.clone()).collect();
...
pub fn can_recover(
    ...
    if !shifts[day_idx].is_empty() { return Err("Already assigned a shift on this day".to_string()); }
    ...
    // 1. Max assignments
    let total_assignments = hyp.iter().filter(|s| !s.is_empty() && !s.contains("SICK-")).count();
    let num_days = (scenario.number_of_weeks * 7) as usize;
    let max_assignments = (contract.max_assignments as f64 * (num_days as f64 / (scenario.number_of_weeks as f64 * 7.0))).ceil() as usize;
    if total_assignments > max_assignments { return Err("Would exceed maximum total assignments".to_string()); }
    ...
    for d in 0..num_days {
        if !hyp[d].is_empty() && !hyp[d].contains("SICK-") {
            ...
            if current_work_streak > 0 && current_work_streak < contract.min_consecutive_working_days {
    ...
    for d in 1..num_days {
        let prev = &hyp[d-1];
        let curr = &hyp[d];
        if !prev.is_empty() && !curr.is_empty() && !prev.contains("SICK-") && !curr.contains("SICK-") {
            let prev_clean = prev.replace("NEW-", "").replace("RECOVERED-", "");
            let curr_clean = curr.replace("NEW-", "").replace("RECOVERED-", "");
            
            if let Some(rule) = scenario.forbidden_shift_type_successions.iter().find(|r| r.preceding == prev_clean) {
                if rule.succeeding.contains(&curr_clean) {
                    return Err(format!("Forbidden shift succession: {} -> {}", prev_clean, curr_clean));
                }
            }
        }
    }
