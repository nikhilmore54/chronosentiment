...
impl Evaluator<ScheduleGenome> for UltraCrewEvaluator {
    fn evaluate(&self, genome: &ScheduleGenome) -> FitnessVector {
        ...
        let validation_report = crate::validator::validate_schedule(&schedule, &self.scenario);
        ...
        for det in &validation_report.details {
            match det.constraint.as_str() {
                "min_assignments" | "max_assignments" => {
                    let diff = if det.actual > det.required { det.actual - det.required } else { det.required - det.actual };
                ...
                "min_consecutive_days_off" | "max_consecutive_working_days" | "min_consecutive_working_days" | "complete_weekends" => {
                    let diff = if det.actual > det.required { det.actual - det.required } else { det.required - det.actual };
        ...
        let workload_balance = assigned_counts.iter().map(|v| (v - mean_assign).powi(2)).sum::<f64>() / assigned_counts.len() as f64;
        
        let mean_wknd = weekend_counts.iter().sum::<f64>() / weekend_counts.len() as f64;
        let temporal_load_balance = weekend_counts.iter().map(|v| (v - mean_wknd).powi(2)).sum::<f64>() / weekend_counts.len() as f64;
        
        vec![s6_assignment_penalty, s7_weekend_penalty, recovery_penalty, workload_balance, temporal_load_balance]
    }
}
...
impl UltraCrewMutator {
    pub fn new(scenario: InrcScenario) -> Self {
        ...
        for n in &scenario.nurses {
            ...
            equivalence_classes.entry(key).or_insert_with(Vec::new).push(n.id.clone());
        }
        Self { scenario, equivalence_classes }
    }
    ...
    pub fn mutate_with_tier_logged(&self, genome: &ScheduleGenome, use_tier1: bool) -> (ScheduleGenome, String) {
        let mut child = genome.clone();
        if child.slots.is_empty() { return (child, "NoOp".to_string()); }
        ...
        let current_nurse_obj = self.scenario.nurses.iter().find(|n| n.id == current_nurse).unwrap();
        ...
        if eligible_targets.is_empty() {
            for target in &self.scenario.nurses {
                if target.id != current_nurse && !working_nurses.contains(&target.id) && target.skills.contains(&req_skill) {
