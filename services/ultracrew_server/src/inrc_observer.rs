...
use ultracrew::inrc::optimization::{InrcOptimizer, InrcGenome};
use coralys_moga::traits::FitnessEvaluator;
use crate::optimizer::ScheduleGenome;
...
pub fn to_inrc_genome(genome: &ScheduleGenome, scenario: &InrcScenario) -> InrcGenome {
    ...
    let nurse_map: HashMap<String, usize> = scenario.nurses.iter().enumerate()
        .map(|(i, n)| (n.id.clone(), i)).collect();
    let shift_map: HashMap<String, usize> = scenario.shift_types.iter().enumerate()
        ...
        if let Some(&nurse_idx) = nurse_map.get(&slot.assigned_nurse) {
            if let Some(&shift_idx) = shift_map.get(&slot.shift_type) {
                let index = nurse_idx * (num_days * num_shift_types)
                    + slot.day * num_shift_types + shift_idx;
                bits[index] = true;
            }
        }
    }
...
pub fn score_inrc_official(
    ...
    let total_hc = ev.hc_coverage + ev.hc_skills
        + ev.hc_one_shift_per_day + ev.hc_forbidden_successions;
