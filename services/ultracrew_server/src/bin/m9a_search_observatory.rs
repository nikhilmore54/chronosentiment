use coralys_ecology::state::{SearchSnapshot, SearchStateObservatory};
...
fn run_instance(instance: &str, seeds: &[u64], mode: CoolingMode) -> Vec<SeedStateReport> {
    ...
    if !scenario_path.exists() {
        println!(
            "Warning: Instance {} not found at {:?}. Skipping.",
            instance, scenario_path
        );
        return all_reports;
    }
    ...
    for &seed in seeds {
        ...
        for g in 1..=MAX_GENERATIONS {
            ...
            let mut best_cand: (ScheduleGenome, Vec<f64>, String) = {
                let candidates: Vec<(ScheduleGenome, Vec<f64>, String)> = (0..5)
                    .map(|_| {
                        ...
                        let (gc, op) = engine
                            .mutator
                            .mutate_with_tier_logged(&parent.genome, is_tier1);
            ...
            for _ in 0..2 {
                let is_tier1 = rng.gen_bool(0.5);
                let (neighbour, op) = engine
                    .mutator
                    .mutate_with_tier_logged(&best_cand.0, is_tier1);
                if op.contains("Tier1") {
                    tier1_attempts += 1;
                } else {
                    tier2_attempts += 1;
                ...
                if accepted {
                    ...
                    *operator_counts.entry(op.clone()).or_insert(0) += 1;
                    if op.contains("Tier1") {
                        tier1_acceptances += 1;
                        if is_improvement {
                            tier1_improvements += 1;
                        }
                    } else {
                        tier2_acceptances += 1;
                        if is_improvement {
                            tier2_improvements += 1;
                        }
                    }
                }
            ...
            if history_buffer.is_empty() {
                ...
                for hist_genome in &history_buffer {
                    ...
                    if d < min_dist_to_history {
                        min_dist_to_history = d;
                    }
                    sum_dist += d;
                }
            }
            ...
            if recent_revisits.len() > history_k {
                recent_revisits.pop_front();
            }
            let revisit_rate = recent_revisits.iter().filter(|&&r| r).count() as f64
                / recent_revisits.len() as f64;

            history_buffer.push_back(child_genome.clone());
            if history_buffer.len() > history_k {
                history_buffer.pop_front();
            }

            let acceptance_rate =
                (accepted_worse_moves + accepted_better_moves) as f64 / total_moves.max(1) as f64;
            ...
            let diversity: f64 = engine
                .archive
                .solutions
                .iter()
                .map(|sol| {
                    (0..num_objs)
                        .map(|d| (sol.fitness[d] - centroid[d]).powi(2))
                        .sum::<f64>()
                        .sqrt()
                })
                .sum::<f64>()
                / (engine.archive.solutions.len() as f64 + 1e-9);

            let child_sc = ultracrew_server::inrc_observer::score_inrc_official(
                &child_genome,
                &scenario,
                &inrc_optimizer,
            );
