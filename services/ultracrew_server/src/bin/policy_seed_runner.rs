...
use coralys_ecology::diagnostics::{
    ...
    DiagnosticResult, EcologyLockInDetector, EcologyState, ObjectiveVector,
    ProxySuppressionDetector, SearchObservation, TradeoffBasinDetector,
};
use coralys_moga::engine_proof::{Evaluator, EvolutionEngine, ParetoSolution};
use coralys_policy::{PolicyDecision, PolicyEngine, RecommendationMirrorPolicy};
...
fn main() {
    ...
    for seed in &seeds {
        ...
        for g in 1..=MAX_GENERATIONS {
            ...
            let gen_signatures: Vec<u64> = engine
                .archive
                .solutions
                .iter()
        ...
        final_diagnostics.insert(
            "Attractor".to_string(),
            attractor_detector.evaluate(final_obs),
        );
        final_diagnostics.insert(
            "TradeoffBasin".to_string(),
            tradeoff_detector.evaluate(final_obs),
        );
        final_diagnostics.insert(
            "EcologyLockIn".to_string(),
            lockin_detector.evaluate(final_obs),
        );
        final_diagnostics.insert(
            "AccumulationFailure".to_string(),
            accum_detector.evaluate(final_obs),
        );
        final_diagnostics.insert(
            "ProxySuppression".to_string(),
            suppression_detector.evaluate(final_obs),
        );
        ...
        if !rec_report.recommendations.is_empty() {
            println!(
                "  Recommendations       : {}",
                rec_report.recommendations.len()
            );
        } else {
    ...
    for r in &all_reports {
        ...
        *action_confidence
            .entry(r.decision.action.clone())
            .or_insert(0.0) += r.decision.confidence;
        confidence_sum += r.decision.confidence;
    }

    let avg_conf = if total_runs > 0 {
        confidence_sum / total_runs as f64
    } else {
        0.0
    };
    ...
    let all_same = all_reports
        .windows(2)
        .all(|w| w[0].decision.action == w[1].decision.action);
    if all_same && total_runs > 0 {
        println!(
            "  ✅ YES – all {} seeds produced the same policy action.",
            total_runs
        );
    } else {
