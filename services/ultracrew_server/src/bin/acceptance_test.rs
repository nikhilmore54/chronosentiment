...
use std::fs::File;
use std::io::Write;
use ultracrew::inrc::models::{
    InrcContract, InrcForbiddenSuccession, InrcNurse, InrcScenario, InrcShiftType,
};
...
fn main() {
    ...
    for _ in 0..total_runs {
        ...
        // Integrity Check
        if schedule.len() != scenario.nurses.len() {
            integrity_failures += 1;
        }
        for n in &scenario.nurses {
            if schedule[&n.id].len() != 56 {
                integrity_failures += 1;
            }
        }
        ...
        for shifts in schedule.values() {
            ...
            for d in 0..56 {
                if !shifts[d].is_empty() {
                    ...
                    if curr_off > nurse_max_off {
                        nurse_max_off = curr_off;
                    }
                    curr_off = 0;
                    curr_work += 1;
                } else {
            ...
            if dev.abs() > global_max_dev {
                global_max_dev = dev.abs();
            }
            dev_count += 1;
        ...
        // UI Consistency
        if (actual as f64 / (16.0 * 56.0)) > 1.0 {
            ui_consistency_failures += 1;
        }
    }
    ...
    let benchmark = BenchmarkResult {
        ...
        avg_longest_off_streak: sum_longest_off as f64
            / (total_runs * scenario.nurses.len()) as f64,
    ...
    let md = format!(
        ...
        benchmark.legal_schedules,
        benchmark.total_runs,
