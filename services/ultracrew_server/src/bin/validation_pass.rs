...
use coralys_moga::ecology::{ExternalObserver, EcologyObserver};
...
fn to_inrc_genome(genome: &ScheduleGenome, scenario: &InrcScenario) -> InrcGenome {
    ...
    let nurse_map: HashMap<String, usize> = scenario.nurses.iter().enumerate().map(|(i, n)| (n.id.clone(), i)).collect();
    let shift_map: HashMap<String, usize> = scenario.shift_types.iter().enumerate().map(|(i, s)| (s.id.clone(), i)).collect();
    
    for slot in &genome.slots {
        if slot.assigned_nurse.is_empty() { continue; }
        if let Some(&nurse_idx) = nurse_map.get(&slot.assigned_nurse) {
            if let Some(&shift_idx) = shift_map.get(&slot.shift_type) {
                let index = nurse_idx * (num_days * num_shift_types) + slot.day * num_shift_types + shift_idx;
                bits[index] = true;
            }
        }
    }
...
impl InrcExternalScorer {
    fn score_with_components(&self, genome: &ScheduleGenome) -> (f64, Vec<f64>) {
        ...
        let off_eval = self.inrc_optimizer.evaluate(&i_genome, &coralys_moga::runtime::optimization::metric::MetricReport::default());
...
fn main() {
    ...
    let inrc_context = InrcContext::new(scenario.clone(), week_data.clone(), history, ultracrew::ecology::WorkforceEcology::new());
    ...
    println!("Starting External Fitness Validation Pass for {}...", instance);
    ...
    | -> (Vec<f64>, Vec<f64>, f64, f64, Vec<f64>) {
        ...
        *eval_counter += 1;
        if policy.external_enabled && (*eval_counter % policy.external_frequency == 0 || *eval_counter == 1) {
        ...
        let novelty = if archive_size == 0 {
            ...
            for sol in &engine.archive.solutions {
                let dist = (0..num_objs).map(|d| (selection_fitness[d] - sol.fitness[d]).powi(2)).sum::<f64>().sqrt();
    ...
    let off_eval = external_scorer.inrc_optimizer.evaluate(&i_genome, &coralys_moga::runtime::optimization::metric::MetricReport::default());
    ...
    tracker.record_evaluation(base_uid, 0, base_pscore, base_escore, true, true, false, 0, 0.0, base_fitness.clone(), base_external_objs);
    writeln!(csv_file, "{},{},{:.2},{:.2},{},{},{},{},{:.4}", 0, base_uid, base_pscore, base_escore, true, true, false, 0, 0.0).unwrap();
    ...
    for g in 1..=max_generations {
        ...
        if archive_size == 1 {
            ...
            for i in 0..archive_size {
                ...
                for j in 0..archive_size {
                    if i == j { continue; }
                    let dist = (0..num_objs).map(|d| (normalized_coords[i][d] - normalized_coords[j][d]).powi(2)).sum::<f64>().sqrt();
                    if dist < min_dist { min_dist = dist; }
                }
        ...
        let parent_discovery_gen = tracker.lifecycles.get(&parent.uid).map(|l| l.discovery_generation).unwrap_or(0);
        ...
        let calc_raw_energy = |fitness: &[f64]| -> f64 { fitness.iter().map(|v| v.powi(2)).sum::<f64>().sqrt() };

        for _ in 0..num_offspring {
            let candidate_genome = engine.mutator.mutate_with_tier(&parent.genome, rng.gen_bool(0.8));
            let (sel_fitness, log_fitness, e_score, novelty, e_objs) = evaluate_candidate(
                &candidate_genome, g, parent_discovery_gen, &mut engine, &external_scorer, &policy, &mut eval_counter
            );
            let energy = calc_raw_energy(&sel_fitness);
            candidates.push((candidate_genome, sel_fitness, log_fitness, e_score, novelty, energy, e_objs));
        }
        
        let mut best_candidate = candidates.into_iter().min_by(|a, b| a.5.partial_cmp(&b.5).unwrap()).unwrap();
        ...
        for _ in 0..20 {
            let neighbor_genome = engine.mutator.mutate_with_tier(&best_candidate.0, rng.gen_bool(0.8));
            let (sel_fitness, log_fitness, e_score, novelty, e_objs) = evaluate_candidate(
                &neighbor_genome, g, parent_discovery_gen, &mut engine, &external_scorer, &policy, &mut eval_counter
            );
            let neighbor_energy = calc_raw_energy(&sel_fitness);
            
            sa_candidates.push((neighbor_genome.clone(), sel_fitness.clone(), log_fitness.clone(), e_score, novelty, neighbor_energy, e_objs.clone()));

            let delta = neighbor_energy - best_candidate.5;
            
            if delta < 0.0 || rng.gen_range(0.0..1.0) < (-delta / t).exp() {
                best_candidate = (neighbor_genome, sel_fitness, log_fitness, e_score, novelty, neighbor_energy, e_objs);
            }
            t *= alpha;
        }
        ...
        tracker.record_evaluation(final_uid, g, final_pscore, best_candidate.3, was_inserted, was_inserted, !was_inserted, age, best_candidate.4, best_candidate.2.clone(), best_candidate.6.clone());
        writeln!(csv_file, "{},{},{:.2},{:.2},{},{},{},{},{:.4}", g, final_uid, final_pscore, best_candidate.3, was_inserted, was_inserted, !was_inserted, age, best_candidate.4).unwrap();
        ...
        if g % 1000 == 0 {
            let archive_external_fitnesses: Vec<f64> = engine.archive.solutions.iter().map(|s| {
                external_scorer.observe(&s.genome)
            }).collect();
            tracker.snapshot_archive(g, &archive_external_fitnesses);
            let snap = tracker.snapshots.last().unwrap();
            println!("Gen {}: Archive {}, Best External: {}, Median: {}, Worst: {}", g, engine.archive.solutions.len(), snap.best_fitness, snap.p50, snap.worst_fitness);
            
            // Ecology Observer
            let proxy_objs: Vec<Vec<f64>> = engine.archive.solutions.iter().map(|s| s.fitness.clone()).collect();
            ecology_observer.compute_and_record_correlations(g, &proxy_objs, &archive_external_fitnesses);
            ecology_observer.record_best_external(g, snap.best_fitness);
        }
    }
    
    println!("\nValidation Complete. {} Final Candidates Evaluated.", tracker.evaluations);
    if let Some(best) = &tracker.best_ever {
        println!("Best Ever External Fitness: {} (Found at Gen {})", best.fitness, best.generation);
    }
    ...
    for r in &tracker.alignment_records {
        if r.external_fitness < current_best {
            ...
            writeln!(curve_file, "{},{},{},{:.2}", r.generation, r.uid, r.external_fitness, r.internal_fitness_sum).unwrap();
        }
    }
    ...
    let retention_error = if final_archive_uids.contains(&best_ever_uid) { 0 } else { 1 };
    let best_final_archive = tracker.snapshots.last().map(|s| s.best_fitness).unwrap_or(0.0);
    let alignment_loss = best_final_archive - best_ever;

    // Compute Correlations
    let p_scores: Vec<f64> = tracker.alignment_records.iter().map(|r| r.internal_fitness_sum).collect();
    let e_scores: Vec<f64> = tracker.alignment_records.iter().map(|r| r.external_fitness).collect();
    ...
    // Top Internal
    ranked.sort_by(|a, b| a.internal_fitness_sum.partial_cmp(&b.internal_fitness_sum).unwrap());
    ...
    let top1_overlap = top1_internal.iter().filter(|id| top1_external.contains(id)).count() as f64 / top1_count as f64;
    let top5_overlap = top5_internal.iter().filter(|id| top5_external.contains(id)).count() as f64 / top5_count as f64;
    
    let mut obj_spearmans = Vec::new();
    let num_objs = tracker.alignment_records[0].fitness_objs.len();
    for o in 0..num_objs {
        let o_scores: Vec<f64> = tracker.alignment_records.iter().map(|r| r.fitness_objs[o]).collect();
        let spearman_o = compute_spearman(&o_scores, &e_scores);
        obj_spearmans.push(spearman_o);
    }
    
    let soft_scores: Vec<f64> = tracker.alignment_records.iter().map(|r| r.external_objs[6]).collect();
    ...
    println!("Spearman Correlation (vs Soft Official Only): {:.4}", spearman_soft);
    println!("R²: {:.4}", r_squared);
    println!("Monotonicity Violations: {} / {} ({:.2}%)", monotonicity_violations, total_pairs, (monotonicity_violations as f64 / total_pairs as f64) * 100.0);
    ...
    let ext_names = ["HC_Coverage", "HC_Skills", "HC_Successions", "S1_OptCoverage", "S6_Assignment", "S7_Weekend", "SoftTotal"];
    heatmap.push_str("\n--- Official Component Attribution Heatmap (Spearman vs Proxy) ---\n");
    heatmap.push_str(format!("{:>4} | {:>10} | {:>10} | {:>10} | {:>10} | {:>10} | {:>10} | {:>10}\n", "", ext_names[0], ext_names[1], ext_names[2], ext_names[3], ext_names[4], ext_names[5], ext_names[6]).as_str());
    
    for (i, _) in obj_spearmans.iter().enumerate() {
        let o_scores: Vec<f64> = tracker.alignment_records.iter().map(|r| r.fitness_objs[i]).collect();
        let mut row = format!("O{:<3} |", i + 1);
        for ext_idx in 0..7 {
            let ext_vals: Vec<f64> = tracker.alignment_records.iter().map(|r| r.external_objs[ext_idx]).collect();
            let mut all_zero = true;
            for &v in &ext_vals { if v != 0.0 { all_zero = false; break; } }
            if all_zero {
    ...
    for snap in &tracker.snapshots {
        println!("Gen {}: Best={}, P25={}, P50={}, P75={}, Worst={}", snap.generation, snap.best_fitness, snap.p25, snap.p50, snap.p75, snap.worst_fitness);
        writeln!(drift_file, "{},{},{},{},{},{},{},{}", snap.generation, snap.best_fitness, snap.p10, snap.p25, snap.p50, snap.p75, snap.p90, snap.worst_fitness).unwrap();
    }
    
    println!("\nAlignment Loss: {} (Best Final: {} - Best Ever: {})", alignment_loss, best_final_archive, best_ever);
    ...
    for (start, end, diff) in velocities {
        println!("{:>4}-{:>4} : {:>8} pts / 1000 gen", start, end, diff as i64);
    }
    ...
    writeln!(summary, "Monotonicity Violations: {} / {} ({:.2}%)", monotonicity_violations, total_pairs, (monotonicity_violations as f64 / total_pairs as f64) * 100.0).unwrap();
    writeln!(summary, "Top 1% Champion Agreement: {:.2}%", top1_overlap * 100.0).unwrap();
    writeln!(summary, "Top 5% Champion Agreement: {:.2}%", top5_overlap * 100.0).unwrap();
    ...
    if spearman >= 0.70 && top5_overlap >= 0.60 && (alignment_loss / best_ever) <= 0.10 {
        ...
        writeln!(summary, "Result: YELLOW (Run objective-space investigation before 60-run matrix)").unwrap();
    } else {
        writeln!(summary, "Result: RED (Stop immediately. Re-evaluate objective space.)").unwrap();
    }
}
