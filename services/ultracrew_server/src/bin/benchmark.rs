use coralys_moga::engine_proof::{Evaluator, EvolutionEngine};
use rand::Rng;
...
fn main() {
    ...
    println!(
        "Starting Sprint 3.5C Persistence 25k Sweep for Scenario {} with Mode: {}",
        instance, sa_mode
    );
    ...
    engine
        .archive
        .add(coralys_moga::engine_proof::ParetoSolution {
    ...
    for g in 1..=generations {
        ...
        let inrc_improvement = if parent_inrc > child_inrc {
            parent_inrc - child_inrc
        } else {
            0.0
        };

        let was_inserted = engine
            .archive
            .add(coralys_moga::engine_proof::ParetoSolution {
        ...
        if g % 1000 == 0 {
            let best_inrc = engine
                .archive
                .solutions
                .iter()
                .map(|s| s.fitness[0] + s.fitness[1] + s.fitness[2])
                .fold(f64::INFINITY, |a, b| a.min(b));
            println!(
                "Epoch {} complete. Archive: {}, Best INRC: {:.1}",
                current_epoch, archive_size, best_inrc
            );
        }
    }
    ...
    for lag in 1..=24 {
        ...
        writeln!(
            file,
            "Lag {:02} Decay: Mean={:.4}, Median={:.4}, P25={:.4}, P75={:.4}",
            lag, mean, med, p25, p75
        )
        .unwrap();
    }
