...
use coralys_moga::ecology::champion::{ChampionStatus, ChampionTracker, ExitReason};
...
use ultracrew_server::inrc_observer::{OBSERVER_ID, score_inrc_official};
...
impl DeltaHcProbe {
    ...
    fn record(&mut self, parent_hc: usize, child_hc: usize, was_inserted: bool) {
        ...
        if delta < 0 {
            ...
            self.hc_worsening += 1;
            if was_inserted {
                self.hc_worsening_inserted += 1;
            }
        }
    }

    fn p_improving(&self) -> f64 {
        if self.total_offspring == 0 {
            return 0.0;
        }
...
impl RejectedImprovingProbe {
    ...
    fn record(
        ...
        if child_hc >= parent_hc {
            return;
        } // only fires for HC-improving offspring
        ...
        if delta_hc <= 10 {
            self.delta_hc_le10 += 1;
        } else if delta_hc <= 100 {
            self.delta_hc_le100 += 1;
        } else if delta_hc <= 1000 {
            self.delta_hc_le1000 += 1;
        } else {
            self.delta_hc_gt1000 += 1;
        }
    ...
    fn mean_delta_hc_rejected(&self) -> f64 {
        if self.total_rejected_improving == 0 {
            return 0.0;
        }
    ...
    fn mean_delta_hc_admitted(&self) -> f64 {
        if self.admitted_improving_count == 0 {
            return 0.0;
        }
...
fn crowding_distance_for(solutions: &[ParetoSolution<ScheduleGenome>], target_uid: u64) -> f64 {
    ...
    for obj in 0..num_objs {
        let mut sorted: Vec<(usize, f64)> = solutions
            .iter()
            .enumerate()
...
fn main() {
    ...
    let mut feasible_archive_members: std::collections::HashSet<u64> =
    ...
    println!(
        "  HC_OneShiftPerDay     : {}",
        base_score.hc_one_shift_per_day
    );
    println!(
        "  HC_ForbiddenSucc      : {}",
        base_score.hc_forbidden_successions
    );
    ...
    for g in 1..=max_generations {
        ...
        let parent_hc = parent_score.hc_coverage
            ...
            + parent_score.hc_forbidden_successions;
        let child_hc_for_probe = child_score.hc_coverage
        ...
        let parent_o3_for_probe = if parent.fitness.len() > 2 {
            parent.fitness[2]
        } else {
            0.0
        };
        let child_o3_for_probe = if child_fitness.len() > 2 {
            child_fitness[2]
        } else {
            0.0
        };
                    ...
                    if Some(t_uid) == best_ever_tracker_uid && domination_event.is_none() {
                        ...
                        if let Some(ref v_proxy) = best_ever_proxy_before {
                            if let Some(ref v_genome) = best_ever_genome_cache {
                                let v_ext =
                                    score_inrc_official(v_genome, &scenario, &inrc_optimizer);
                                domination_event = Some(DominationEvent {
                                    ...
                                    dominator_external: ExtMetrics {
                                        ...
                                        hc_forbidden_successions: child_score
        ...
        if g % 100 == 0 || g == max_generations {
            ...
            hc_dist_timeline.push((
                g, safe_min, hc_max, hc_sum, hc_count, hc0, hc_le5, hc_le10, hc_le20, hc_le50,
                hc_gt50,
            ));
    ...
    println!(
        "\nWrote champion_lifecycle.jsonl ({} bytes)",
        champion_jsonl.len()
    );
    ...
    println!(
        "Wrote feasibility_snapshot.jsonl ({} bytes)",
        feasibility_jsonl.len()
    );
    ...
    if let Some(ref ev) = domination_event {
        ...
        report.push_str(&format!(
            "# Gen-{} Domination Report — SD-006 Proxy Geometry Attribution\n\n",
            ev.eviction_gen
        ));
        ...
        report.push_str(&format!(
            "**Event:** Archive eviction of best-ever champion at generation {}  \n\n",
            ev.eviction_gen
        ));
        report.push_str("---\n\n");
        ...
        report.push_str(&format!(
            "## 6. Archive Rank Before Eviction (Gen {})\n\n",
            prev_gen
        ));
        report.push_str(
        ...
        let driving_objs: Vec<&str> = obj_names
            .iter()
            .enumerate()
            ...
            .collect();
        let delta_official =
            ev.dominator_external.official_total - ev.victim_external.official_total;
        ...
        println!(
            "\n=== Sprint 3.7: Gen-{} Domination Event ===",
            ev.eviction_gen
        );
        println!(
            "Victim  UID-4 official_total : {:.0}",
            ev.victim_external.official_total
        );
        println!(
            "Dominator official_total     : {:.0}",
            ev.dominator_external.official_total
        );
        println!(
            "ΔOfficialTotal               : {:+.0}",
        ...
        for (i, name) in obj_names.iter().enumerate() {
            println!(
                "  Δ{} = {:+.0}",
                name,
                ev.dominator_proxy[i] - ev.victim_proxy[i]
            );
        }
    } else {
        println!("\n[Sprint 3.7] WARNING: DominationEvent was NOT fired.");
        println!(
            "  best_ever_genome_hash set : {}",
            best_ever_genome_hash.is_some()
        );
        println!(
            "  best_ever_genome_cache set: {}",
            best_ever_genome_cache.is_some()
        );
        ...
        println!(
            "    - best_ever_proxy_before was None at eviction time (champion not in archive before add())"
        );
        println!(
            "    - best_ever_genome_cache was None (champion not seen in archive at prev gen end)"
        );
    }
    ...
    // ── Sprint 3.8: Write feasible_lineage_report.md ──────────────────────────
    {
        ...
        //   first feasible = earliest discovered_at
        //   best feasible  = lowest official_total
        let first_feasible: Option<&FeasibleLifecycle> = feasible_lifecycles
            .values()
            .min_by_key(|lc| lc.discovered_at);
        let best_feasible: Option<&FeasibleLifecycle> = feasible_lifecycles
            .values()
        ...
        let fmt_exit = |lc: &FeasibleLifecycle| -> &'static str {
            match lc.exit_reason {
                ...
                None => {
                    if lc.evicted_at.is_none() && lc.admitted_at.is_some() {
                        "Still in archive"
        ...
        let fmt_lifetime = |lc: &FeasibleLifecycle| -> String {
            match (lc.admitted_at, lc.evicted_at) {
                ...
                (Some(a), None) => {
                    format!("{} (still in archive)", max_generations.saturating_sub(a))
                }
        ...
        report
            .push_str("Applying frozen classification table from `sd005_sprint38_charter.md`.\n\n");

        let total_feasible = feasible_lifecycles.len();
        let total_admitted = feasible_lifecycles
            .values()
            .filter(|lc| lc.admitted_at.is_some())
            .count();
        let total_evicted = feasible_lifecycles
            .values()
            .filter(|lc| lc.evicted_at.is_some())
            .count();
        let still_in_archive = feasible_archive_members.len();
        let dominated_evictions = feasible_lifecycles
            .values()
        ...
        report.push_str(&format!(
            "| Evicted by Dominated | {} |\n",
            dominated_evictions
        ));
        report.push_str(&format!(
            "| Still in archive at gen {} | {} |\n\n",
        ...
        let classification = if total_feasible == 0 {
            ...
            "**Retention Failure** — Feasible genomes were admitted then evicted. \
             The archive admitted feasibility but could not retain it under selection pressure."
                .to_string()
        ...
        if total_evicted > 0 && dominated_evictions > 0 {
            ...
            report.push_str(
                "At least one feasible genome was evicted by Pareto domination. \
            ...
            report.push_str("```\n\n");
            report.push_str(
                "See `gen283_domination_report.md` for the canonical SD-006 domination event.\n\n",
            );
        }
    ...
    // ── Sprint 3.9: Write hc_distribution_report.md ───────────────────────────
    {
        ...
        if init_hc_values.is_empty() {
            ...
            if median > 50.0 {
                ...
                report.push_str(&format!(
                    "Median HC_Total = {:.1} (moderate initialization depth; RC-3 possible).\n\n",
                    median
                ));
            } else {
        ...
        report
            .push_str("| Gen | Min | Max | Mean | HC=0 | HC≤5 | HC≤10 | HC≤20 | HC≤50 | HC>50 |\n");
        report.push_str("|---|---|---|---|---|---|---|---|---|---|\n");
        for &(
            cgen,
            min_hc,
        ...
        {
            let mean_hc = if hc_count > 0 {
                hc_sum as f64 / hc_count as f64
            } else {
                0.0
            };
            report.push_str(&format!(
        ...
        if hc_dist_timeline.len() >= 2 {
            ...
            let first_mean = if first.4 > 0 {
                first.3 as f64 / first.4 as f64
            } else {
                0.0
            };
            let last_mean = if last.4 > 0 {
                last.3 as f64 / last.4 as f64
            } else {
                0.0
            };
            let delta = last_mean - first_mean;
            report.push_str(&format!(
                "Mean HC_Total at gen={}: {:.2}  \n",
                first.0, first_mean
            ));
            report.push_str(&format!(
                "Mean HC_Total at gen={}: {:.2}  \n",
        ...
        let first_mean = hc_dist_timeline
            .first()
            .map(|s| {
                if s.4 > 0 {
                    s.3 as f64 / s.4 as f64
                } else {
                    0.0
                }
            })
        ...
        let last_mean = hc_dist_timeline
            .last()
            .map(|s| {
                if s.4 > 0 {
                    s.3 as f64 / s.4 as f64
    ...
    //   Both probabilities > 0                                   → RC-1 + RC-2 interaction
    {
        ...
        report
            .push_str("**Scope:** All evaluated offspring (not just archive-admitted ones)  \n\n");
        report.push_str("---\n\n");
        ...
        report.push_str(&format!(
            "| HC-improving AND admitted | {} |\n",
            delta_hc_probe.hc_improving_inserted
        ));
        report.push_str(&format!(
            "| HC-worsening AND admitted | {} |\n",
        ...
        report.push_str(&format!(
            "**Observed:** P(improving) = {:.6}, P(admitted | improving) = {:.6}  \n\n",
            p_imp, p_imp_ins
        ));
        report.push_str(&format!("{}\n\n", classification));
    ...
    // Fires for every HC-improving offspring that was rejected by the archive.
    {
        ...
        let classification = match (sub_a_signal, sub_b_signal) {
            (true, true) => {
                "**Sub-A + Sub-B BOTH ACTIVE** — Rejected improvements are smaller than admitted ones (step-size asymmetry) AND the majority also worsen O3 (proxy pressure). Both mechanisms suppress HC-improving offspring."
            }
            (true, false) => {
                "**Sub-A DOMINANT (step-size asymmetry)** — Rejected HC-improving offspring have significantly smaller ΔHC than admitted ones. The operator produces improvements, but they are too small to survive Pareto selection. O3 pressure is not the primary gate."
            }
            (false, true) => {
            ...
            (false, false) => {
                "**INCONCLUSIVE** — Neither Sub-A nor Sub-B signal is dominant. Rejection may be driven by other proxy objectives (O1, O2, O4) or by archive capacity effects. Further decomposition required."
            }
        };
        ...
        report.push_str(&format!(
            "| |ΔHC| ≤ 10 (tiny improvement) | {} | {:.2}% |\n",
            p.delta_hc_le10,
            p.delta_hc_le10 as f64 / total_rej as f64 * 100.0
        ));
        report.push_str(&format!(
            "| 10 < |ΔHC| ≤ 100 (small improvement) | {} | {:.2}% |\n",
            p.delta_hc_le100,
            p.delta_hc_le100 as f64 / total_rej as f64 * 100.0
        ));
        report.push_str(&format!(
            "| 100 < |ΔHC| ≤ 1000 (medium improvement) | {} | {:.2}% |\n",
            p.delta_hc_le1000,
            p.delta_hc_le1000 as f64 / total_rej as f64 * 100.0
        ));
        report.push_str(&format!(
            "| |ΔHC| > 1000 (large improvement) | {} | {:.2}% |\n",
            p.delta_hc_gt1000,
            p.delta_hc_gt1000 as f64 / total_rej as f64 * 100.0
        ));
        report.push_str("\n**ΔO3 direction (rejected improving only):**\n\n");
        report.push_str("| O3 Direction | Count | % of rejected improving |\n|---|---|---|\n");
        report.push_str(&format!(
            "| O3 worsened (child_o3 > parent_o3) | {} | {:.2}% |\n",
            p.o3_worsened,
            p.o3_worsened as f64 / total_rej as f64 * 100.0
        ));
        report.push_str(&format!(
            "| O3 improved (child_o3 < parent_o3) | {} | {:.2}% |\n",
            p.o3_improved,
            p.o3_improved as f64 / total_rej as f64 * 100.0
        ));
        report.push_str(&format!(
            "| O3 neutral (|Δo3| ≤ 0.5) | {} | {:.2}% |\n",
            p.o3_neutral,
            p.o3_neutral as f64 / total_rej as f64 * 100.0
        ));
        report.push_str("\n");
        ...
        report
            .push_str("- P(child_hc < parent_hc) = 28.76% — RC-1 Operator Incapacity FALSIFIED\n");
    ...
    let retention_error = if (best_in_final - best_external_ever).abs() < 1.0 {
        0
    } else {
        1
    };
    ...
    println!(
        "Untracked evictions      : {}",
        total_evictions.saturating_sub(tracked_evictions)
    );
    println!(
        "Champions tracked        : {}",
        champion_tracker.records.len()
    );
    println!(
        "Archive size (final)     : {}",
        engine.archive.solutions.len()
    ...
    if feasible_pct < 0.05 {
        println!(
            "HINT: Feasibility Representation Failure (feasible={:.1}% < 5%)",
            feasible_pct * 100.0
        );
    }
