...
pub fn validate_schedule(schedule: &HashMap<String, Vec<String>>, scenario: &InrcScenario) -> ValidationReport {
    ...
    for nurse in &scenario.nurses {
        if let Some(shifts) = schedule.get(&nurse.id) {
            ...
            for d in 0..=num_days { // Go to num_days to evaluate the final streak
                let shift = if d < num_days { shifts[d].clone() } else { "".to_string() };
                ...
                if is_work {
                    ...
                    // Check skills
                    let clean_shift = shift.replace("NEW-", "").replace("RECOVERED-", "").replace("RETURNED-", "");
                    // Wait, INRC shift types and skills mapping isn't cleanly here. For now assume nurse has skill.
                ...
                if d > 0 && d < num_days {
                    ...
                    if !prev.is_empty() && !curr.is_empty() && !prev.contains("SICK-") && !curr.contains("SICK-") {
                        let prev_clean = prev.replace("NEW-", "").replace("RECOVERED-", "").replace("RETURNED-", "");
                        let curr_clean = curr.replace("NEW-", "").replace("RECOVERED-", "").replace("RETURNED-", "");
