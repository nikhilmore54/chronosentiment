...
fn main() {
    ...
    for mr in [0.01f64, 0.05, 0.10, 0.20].iter() {
        ...
        let (best, gens, dur) = run_experiment(cfg);
        println!(
            "mutation_rate={:.2}, best={}, gens={}, time_ms={}",
            mr, best, gens, dur
        );
    }
    ...
    for cr in [0.40f64, 0.60, 0.80].iter() {
        ...
        let (best, gens, dur) = run_experiment(cfg);
        println!(
            "crossover_rate={:.2}, best={}, gens={}, time_ms={}",
            cr, best, gens, dur
        );
    }
    ...
    let stddev = (results.iter().map(|v| (v - mean) * (v - mean)).sum::<f64>()
        / results.len() as f64)
        .sqrt();
    println!(
        "seed_stats: mean={:.2}, median={:.2}, min={:.2}, max={:.2}, stddev={:.2}",
