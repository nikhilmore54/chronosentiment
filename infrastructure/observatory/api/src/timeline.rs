...
pub fn handle_timeline(sim: &SimulationResult) -> Result<Vec<TimelineEvent>, ApiError> {
    ...
    for event in &sim.events {
        let desc = match event {
            ...
            SimEvent::QueueProgression { order_id, queue_ahead, .. } => format!("Order {} queue progression: q={}", order_id, queue_ahead),
    ...
    for i in 0..timeline.len() - 1 {
        ...
        if e1.timestamp == e2.timestamp {
            if e1.sequence_id >= e2.sequence_id {
                return Err(ApiError::InternalError("Timeline sequence_id ordering violation".to_string()));
            }
        } else if e1.timestamp > e2.timestamp {
            return Err(ApiError::InternalError("Timeline timestamp ordering violation".to_string()));
        }
    }
