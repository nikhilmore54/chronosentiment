...
fn same_seed_produces_same_population() {
    ...
    assert_eq!(
        pop1, pop2,
        "Population generation is not purely deterministic."
    );
}
...
fn mutation_preserves_candidate_validity() {
    ...
    assert!(
        changed,
        "Mutation operator never altered candidate over 100 trials (space inactive)"
    );
}
...
fn ga_evolution_is_order_deterministic() {
    ...
    for (i, (a, b)) in res1
        .generation_history
        .iter()
        .zip(res2.generation_history.iter())
        .enumerate()
    {
