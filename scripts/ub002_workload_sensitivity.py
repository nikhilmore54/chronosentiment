...
def build_payload(ub, seed, workloads):
...
def count_group_shifts(schedule, group_ids):
    """Count shifts assigned to workers in group_ids.

    schedule may be a dict {shift_id: worker_id} or a list.
    """
    count = 0
...
def main():
    ...
    for profile_name, workloads in profiles.items():
        ...
        profile_results = []
        for seed in SEEDS:
            payload = build_payload(ub, seed, workloads)
            t0 = time.time()
            try:
