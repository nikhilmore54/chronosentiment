...
if [ "${RELEASE_MODE}" = true ]; then
    log_pass "cargo test replay" "N/A (Release Mode)"
else
...
if [ "${SKIP_DOUBLE_BUILD}" = "--skip-double-build" ]; then
    ...
    cd "${REPO_ROOT}/infrastructure/core"
...
if [ "${RELEASE_MODE}" = true ]; then
    GIT_COMMIT="$(cat "${REPO_ROOT}/RELEASE_INFO.json" | grep '"commit"' | cut -d'"' -f4 || echo "unknown")"
    GIT_BRANCH="N/A (Release Mode)"
    log_pass "git state" "N/A (Release Mode) — commit=${GIT_COMMIT}"
else
