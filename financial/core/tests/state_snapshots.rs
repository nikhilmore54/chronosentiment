...
fn replay_state_snapshot_is_stable() {
    let events = vec![NormalizedMarketEvent {
    ...
    assert_eq!(
        snapshot_hash, "260_1_false",
        "Internal causality/state must be perfectly rigid."
    );
}

#[test]
fn snapshot_roundtrip_is_lossless() {
    let events = vec![chronosentiment_core::NormalizedMarketEvent {
        asset: "TEST".to_string(),
        exchange_ts: 500,
        price: 50000.0,
        volume: 1.0,
    ...
    assert_eq!(
        state_hash_pre, deserialized,
        "Replay state failed roundtrip serialization."
    );
}
