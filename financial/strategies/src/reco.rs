...
impl RecommendationEngine {
    ...
    pub fn process(
        ...
        if strategies.len() < config.min_pool_size {
            ...
            strategies = all.into_iter().take(k_need).collect();
        }
        ...
        if std::env::var("POOL_DEBUG").map_or(false, |v| {
            ...
            let fs: Vec<f64> = strategies.iter().map(|e| e.fitness).collect();
        ...
        let pool_size = strategies.len() as f64;
