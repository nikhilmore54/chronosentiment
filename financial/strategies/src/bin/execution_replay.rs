...
#[command(
    author,
    version,
    about = "Phase 5B Execution Replay with Perturbations"
)]
struct Args {
    #[arg(short, long)]
...
fn main() {
    ...
    let geometry = CognitionGeometry::EventReset {
    ...
    for (i, &price) in prices.iter().enumerate() {
        ...
        let signal = i >= 20
            && ((price > sma && prices[i - 1] <= sma) || (price < sma && prices[i - 1] >= sma));
        ...
        if signal {
            ...
            let dynamic_miss_prob =
                args.missed_fill_prob + (args.latency_ms as f64 / 1000.0) * local_vol * 100.0;
