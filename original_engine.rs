...
impl EvaluationMetrics {
    pub fn get_expected_mfe(&self, rank: f64, vol_bucket: usize) -> f64 {
        let r_bucket = (rank * 10.0).floor()f64.clamp(0.0, 9.0) as usize;
    ...
    pub fn zeroed() -> Self {
        Self {
            bucket_mfe_sum: [[0.0; 5]; 10],
            bucket_mae_sum: [[0.0; 5]; 10],
            bucket_time_sum: [[0.0; 5]; 10],
            bucket_count: [[0; 5]; 10],
        }
    }

    pub fn blend(&mut self, next: Self, alpha: f64) {
        self.blend_with_min_count(next, alpha, 3);
    }

    pub fn blend_with_min_count(&mut self, next: Self, alpha: f64, min_count: usize) {
        for r in 0..10 {
            for v in 0..5 {
                if next.bucket_count[r][v] < min_count {
...
static RANKSTATS_PROBE_ONCE: OnceLock<()> = OnceLock::new();

fn maybe_emit_rankstats_probe(rank_stats: &EvaluationMetrics) {
    if std::env::var("RANKSTATS_PROBE").is_err() {
        return;
    }
    if RANKSTATS_PROBE_ONCE.get().is_some() {
        return;
    }
    let mut values: Vec<f64> = Vec::with_capacity(10 * 5);
    let mut counts: Vec<usize> = Vec::with_capacity(10 * 5);
    let mut non_default = 0usize;
    for r in 0..10 {
        for v in 0..5 {
            let count = rank_stats.bucket_count[r][v].max(1);
            let mfe = rank_stats.bucket_mfe_sum[r][v] / count as f64;
            values.push(mfe);
            counts.push(rank_stats.bucket_count[r][v]);
            if (rank_stats.bucket_mfe_sum[r][v] - 0.0045).abs() > 1e-9
                || rank_stats.bucket_count[r][v] != 1
            {
                non_default += 1;
            }
        }
    ...
    println!(
        "RANKSTATS_SUMMARY buckets={} non_default={} mean_mfe={:.6} std_mfe={:.6} min_mfe={:.6} max_mfe={:.6}",
        values.len(),
        non_default,
        mean_mfe,
        std_mfe,
    ...
    println!("RANKSTATS_BUCKETS counts={:?}", counts);
    let _ = RANKSTATS_PROBE_ONCE.set(());
}
...
#[allow(dead_code)] // PLANNED: regime-aware fitness path — wiring deferred to Phase 2
enum Regime {
    Dead,
    Live,
}
...
pub fn detect_market_regime(price: f64, sma20: f64, momentum: f64, norm_vol: f64) -> ScenarioContext {
    let funcs = crate::DOMAIN_DELEGATES.get().expect("Domain delegates uninitialized");
    (funcs.detect_scenario)(price, sma20, momentum, norm_vol)
}
...
pub struct DecisionReport {
    ...
    pub recommendation: Option<ExecutionProposal>,
...
// ==========================================
#[allow(dead_code)] // PLANNED: GA→ESE bridge — wiring into GA evaluation loop deferred to Phase 2
fn simulate_execution_via_ese(
    ...
    side: Side,
    quantity: u32,
    execution_events: &[chronosentiment_core::MarketEvent],
    entry_idx: usize,
    max_hold: usize,
) -> ExecutionResult {
...
struct ScenarioMetrics {
    ...
    #[allow(dead_code)] // PLANNED: efficiency distribution read path not yet wired
    ...
    pub vip_count: usize,
    #[allow(dead_code)] // PLANNED: Phase 14 consensus bridge audit counter — read path not yet wired
    ...
    #[allow(dead_code)] // PLANNED: Phase 14 signal e-score accumulator — read path not yet wired
...
impl ScenarioMetrics {
    ...
    #[allow(dead_code)] // PLANNED: funnel analytics reporting — call sites not yet wired
    ...
    fn adaptive_threshold(&self, percentile: f64) -> f64 {
        ...
        percentilef64(&self.adaptive.score_history, percentile)
    }
    ...
    fn record_trade(
        ...
        let quality = (realized_pnl / denom)f64.clamp(-2.0, 2.0);
    ...
    #[allow(dead_code)] // PLANNED: entropy analytics accessor — call sites not yet wired
    ...
    #[allow(dead_code)] // PLANNED: signal entropy analytics accessor — call sites not yet wired
    ...
    fn calculate_institutional_entropy(&self) -> f64 {
        ...
        let entropy_cv = (std_dev / mean.max(1e-6))f64.clamp(0.0, 2.0) / 2.0;

        // Margin Strength [0.0, 1.0] - distance from boundary
        let margin_strength = meanf64.clamp(0.0, 1.0);
        ...
        (1.0 - entropy_score)f64.clamp(0.0, 1.0)
    }
}
...
/// Very chatty per-signal / per-trade logs inside `evaluate_strategy`.
/// Enable with `GA_DEBUG`, `GA_TRACE`, or `GA_HOT_LOG=1` (same intent as trace for hot paths).
#[inline(always)]
fn ga_log_hotpath() -> bool {
    ga_debug_enabled()
        || std::env::var("GA_TRACE").is_ok()
        || std::env::var("GA_HOT_LOG").map_or(false, |v| {
            !v.is_empty() && v != "0" && v.to_lowercase() != "false"
        })
}

/// Per-bucket / periodic GA progress (still noisy at scale). Enable with `GA_LOG_PROGRESS=1` or `GA_DEBUG=1`.
#[inline(always)]
fn ga_log_progress() -> bool {
    ga_debug_enabled()
        || std::env::var("GA_LOG_PROGRESS").map_or(false, |v| {
            !v.is_empty() && v != "0" && v.to_lowercase() != "false"
        })
}
...
fn percentilef64(values: &Vec<f64>, p: f64) -> f64 {
    if values.is_empty() {
        return 0.0;
    }
...
pub fn calculate_alignment_centroid(evals: Vec<&CandidateEvaluation>) -> Candidate {
    if evals.is_empty() {
...
impl GlobalEvoState {
    ...
    pub fn aggregate(&mut self, snapshots: &HashMap<String, AssetSnapshot>, generation: usize) {
        ...
        let _active_snapshots: Vec<&AssetSnapshot> = snapshots
            .values()
        ...
        for (symbol, s) in snapshots {
            ...
            let normalized_log_q = (s.max_log_queue / global_max_log_q)f64.clamp(0.0, 1.0);

            // Activity-weighted contribution: trade_density * fill_rate * normalized_log_q
            let raw_weight = (s.trade_density * s.fill_rate * normalized_log_q)f64.clamp(0.0, 1.0);
        ...
        for s in snapshots.values() {
            ...
            let w = raw.sqrt()f64.clamp(0.0, 1.0);
        ...
        self.agreement_streak =
            ((0.7 * self.agreement_streak as f64 + 0.3 * current_signal) as f64).round() as u32;
        ...
        if self.agreement_ema > 0.3 && self.stability_ema > 0.2 {
            pull_strength = ((self.agreement_ema - 0.3) / 0.4)f64.clamp(0.0, 1.0) * 0.07;
        }
        ...
        let _velocity_trend = if self.velocity_history.len() >= 3 {
...
fn scenario_execution_signature_from_simulation(
    events: &[chronosentiment_core::SimEvent],
    entry_order_id: &str,
    exit_order_id: &str,
    fill_efficiency: f64,
    ...
    for ev in events {
        match ev {
            chronosentiment_core::SimEvent::ExecutionDirective {
                order_id,
                timestamp,
                ..
            } => {
            ...
            chronosentiment_core::SimEvent::OrderEnteredQueue {
                order_id,
                queue_ahead,
                ..
            }
            | chronosentiment_core::SimEvent::QueueProgression {
                order_id,
                queue_ahead,
                ..
            } => {
            ...
            chronosentiment_core::SimEvent::PartialFill {
                order_id,
                timestamp,
                ..
            } => {
    ...
    let avg_queue_norm = (queue_raw_mean / 2500.0)f64.clamp(0.0, 4.0);
    ...
    let latency_norm = (latency_raw_mean / 200.0)f64.clamp(0.0, 4.0);

    let sig = ScenarioExecutionSignature {
        avg_queue_ahead: avg_queue_norm,
        avg_latency: latency_norm,
        fill_ratio: fill_efficiencyf64.clamp(0.0, 1.0),
        participation: participation_ratef64.clamp(0.0, 1.0),
        execution_variance: 0.0,
    };
...
/// Represents a tradable signal supported by multiple candidates (agents).
#[derive(Debug, Clone, Serialize, Deserialize)]
...
pub fn get_coarse_feature_hash(candidate: &Candidate) -> u64 {
    use std::collections::hash_map::DefaultHasher;
    use std::hash::{Hash, Hasher};
use std::hash::BuildHasher;
    let mut s = DefaultHasher::new();
    // Coarse rounding to group similar candidates while avoiding collisions
    ((candidate.queue_threshold / 500) * 500).hash(&mut s);
    ((candidate.base_edge / 50) * 50).hash(&mut s);
    ((candidate.selectivity / 10) * 10).hash(&mut s);
    s.finish()
}
...
impl CandidateEvaluation {
    pub fn new_legacy(
        ...
        slf.candidate = candidate.clone();
        slf.behavioral_signature = candidate.get_signature();
        slf.fitness = fitness;
        slf.avg_pnl = avg_pnl;
    ...
    pub fn new_legacy_with_flag(flag: &str) -> Self {
        Self {
            ...
            candidate: Candidate::from_seed(0),
    ...
    pub fn calculate_institutional_weight(&self) -> f64 {
        let fitness_w = (self.fitness * 100.0)f64.clamp(-1.0, 1.0);

        let capture_w =
            (0.6 * self.short_term_capture_eff + 0.4 * self.long_term_capture_eff)f64.clamp(0.3, 1.2);

        let fill_w = self.scenario_signature.fill_ratiof64.clamp(0.0, 1.0);
        let stability_w = self.consistency_scoref64.clamp(0.0, 1.0);
    ...
    /// predicted_pnl: expected pnl based on candidate signal
    pub fn update_capture_efficiency(&mut self, realized: f64, predicted: f64) {
        if predicted.abs() < 1e-9 {
            return;
        } // Denominator Guard 1
        let ratio = (realized / predicted)f64.clamp(-2.0, 2.0); // Safe ratio with institutional clamp
...
fn is_execution_feasible(conviction: f64, exec_score: f64) -> (bool, f64) {
    ...
    threshold = thresholdf64.clamp(0.25, 0.45);
...
pub fn calculate_capture_efficiency(realized: f64, expected: f64) -> f64 {
    let funcs = crate::DOMAIN_DELEGATES.get().expect("Domain delegates uninitialized");
    (funcs.calculate_efficiency)(realized, expected)
}

pub fn classify_efficiency(e: f64) -> &'static str {
    let funcs = crate::DOMAIN_DELEGATES.get().expect("Domain delegates uninitialized");
    (funcs.classify_efficiency)(e)
}
...
pub struct GaResult {
    ...
    pub final_generation_best: CandidateEvaluation,
    pub generation_history: Vec<CandidateEvaluation>,
    pub best_per_regime: HashMap<String, CandidateEvaluation>,
    pub clusters_per_regime: HashMap<String, Vec<CandidateEvaluation>>,
...
impl Default for CandidateEvaluation {
    fn default() -> Self {
        CandidateEvaluation {
            candidate_edges: Vec::new(),
            winner_idx: 0,
...
pub fn get_strategy_classification(eval: &CandidateEvaluation) -> String {
...
pub struct Candidate {
    ...
    /// Population lineage / slot id (older elite JSON omits this field).
    #[serde(default)]
    pub lineage: usize,
}
...
impl Candidate {
    ...
    pub fn load_from_file(path: &str) -> std::io::Result<Self> {
        ...
        let candidate: Self = serde_json::from_str(&json).map_err(|e| std::io::Error::new(std::io::ErrorKind::Other, e))?;
        Ok(candidate)
    }
...
pub struct GaConfig {
    ...
    pub rank_stats: EvaluationMetrics,
}
...
impl Default for GaConfig {
    fn default() -> Self {
        ...
        let _fitness_mode = std::env::var("GA_FITNESS_MODE")
            .ok()
        ...
        Self {
            ...
            rank_stats: EvaluationMetrics::default(),
        }
    }
}
...
pub fn run_ga_evolution<'a>(
    ...
    let mut final_generation_best: Option<CandidateEvaluation> = None;
    let mut global_generation_history: Vec<CandidateEvaluation> = Vec::new();
    let mut final_p: Vec<Candidate> = Vec::new();
    ...
    let mut best_per_bucket: BTreeMap<(String, String), CandidateEvaluation> = BTreeMap::new();
    let mut all_final_evaluations: Vec<CandidateEvaluation> = Vec::new();
    ...
    for ((asset, regime), scenarios) in asset_regime_scenarios {
        ...
        let mut bucket_best_overall: Option<CandidateEvaluation> = None;
        ...
        for generation in 0..config.generations {
            ...
            if let Some(ref evals) = evaluations_opt {
                let mut next_rank_stats = EvaluationMetrics::default();
                ...
                for eval in evals {
                    for outcome in &eval.pnl_history {
                        ...
                        if !outcome.is_probe && outcome.rank >= 0.1 {
                            let r_bucket = (outcome.rank * 10.0).floor()f64.clamp(0.0, 9.0) as usize;
                            let v_bucket = outcome.vol_bucket.min(2);
                            
                            // 🔥 Winsorize extreme outcomes (remove outliers)
                            let mfe = outcome.m_favorablef64.clamp(0.0, 0.04); 
                            let mae = outcome.m_adversef64.clamp(-0.03, 0.0);
            ...
            if let Some(mut evaluations) = evaluations_opt {
                ...
                if generation % 5 == 0 && ga_log_progress() {
                ...
                if let Some(best_ref) = evaluations.first() {
                    ...
                    if ga_log_progress() {
                        println!("Top 5 candidates (Gen {}):", generation);
                    }
    ...
    (
        GaResult {
            global_best: global_best.unwrap_or_else(CandidateEvaluation::default),
            global_best_generation,
            final_generation_best: final_generation_best
                .unwrap_or_else(CandidateEvaluation::default),
...
pub fn evaluate_ensemble_robustness(
    ...
    let mut evals: Vec<Option<CandidateEvaluation>> = Vec::new();
    for r_config in &regimes {
    ...
    let (_mean, _variance, global_cv) = if !fitness_values.is_empty() {
...
pub fn evaluate_ensemble_strategy(
    ...
    for s in ensemble {
        if let Some(e) = evaluate_strategy(s, pair, config, 0, 0.0, 0, 0.0, 1.0, 0) {
            ...
            member_evals.push(CandidateEvaluation::default());
        }
    }
    ...
    for i in 0..signal_count {
        ...
        let mut total_w = 0.0;
        for (idx, candidate) in ensemble.iter().enumerate() {
    ...
    for current_idx in (period + 2)..(signal_count - 10) {
        ...
        for (i, candidate) in ensemble.iter().enumerate() {
        ...
        // Phase 13.5: Entropy = how much disagreement exists among candidates
        ...
        let entropy_norm = {
            ...
            if total_members > 0.0 {
                let buy_p = inputs
                    ...
                    .filter(|i| {
                        let threshold =
                            (i.evaluation.candidate.take_profit as f64 / 10000.0).max(0.0004);
                ...
                let max_entropy = (3.0f64).ln(); // 3 states
                let raw_entropy: f64 = [buy_p, sell_p, neutral_p]
                    .iter()
                    .filter(|&&p| p > 1e-9)
                    .map(|&p| -p * p.ln())
                    .sum();
                (raw_entropy / max_entropy)f64.clamp(0.0, 1.0)
            } else {
                0.0
            }
        };

        if decision.combined_action != chronosentiment_core::pipeline::SignalAction::HOLD
            && decision.consensus_score.abs() > 0.001
        {
            ...
            let conviction = ConvictionOutcome {
                ...
                is_valid: true,
                expected_edge: 0.0,
                edge_weight: conviction_scoref64.clamp(0.5, 2.0),
                norm_momentum: 0.5,
                norm_volume: 0.5,
                norm_vol_score: 0.5,
                norm_vol: 0.001,
                selection_threshold: 0.5,
                is_bearish: decision.combined_action == chronosentiment_core::pipeline::SignalAction::SELL,
                roll: 0.0,
                raw_q_ratio: 0.0,
                regime: ScenarioContext::MeanReversion,
            ...
            let strength = (0.8 * directional_edge + 0.2 * score_norm)f64.clamp(0.05, 1.0);
    ...
    let payoff_ratio = (avg_win / avg_loss.max(1e-9))f64.clamp(0.5, 3.0);
    ...
    Some(CandidateEvaluation {
        ...
        candidate: ensemble[0].clone(), // Template
        capability: ScenarioCapability::Executable,
        real_dom: 0.0,
        ...
        aqg_skip_ratio: 0.0, // Calculated at population level for single-candidate but this is ensemble
        ..Default::default()
    })
}
...
pub fn evaluate_population_scoped(
    ...
    unique_count: usize,
    expansion_bias: f64,
) -> (Option<Vec<CandidateEvaluation>>, Option<CandidateEvaluation>, Vec<f64>) {
    ...
    let per_member: Vec<Option<CandidateEvaluation>> = if threads <= 1 {
        population
            .iter()
            .enumerate()
            .map(|(idx, candidate)| {
...
fn deduplicate_population(
    ...
    // Refill with random candidates if we removed duplicates
    while new_population.len() < config.population_size {
        let random_strat = Candidate {
            queue_threshold: rng.gen_range((60 * GA_GENE_SCALE)..=(120 * GA_GENE_SCALE)),
            base_edge: rng.gen_range((1 * GA_GENE_SCALE)..=(15 * GA_GENE_SCALE)),
...
pub fn calculate_effective_diversity(evaluations: &[CandidateEvaluation]) -> f64 {
    if evaluations.is_empty() {
        return 0.0;
    }
    ...
    let mut _total_fitness_sum = 0.0;
...
pub fn calculate_population_stats(evaluations: &[CandidateEvaluation]) -> (f64, f64) {
    if evaluations.is_empty() {
        return (0.0, 0.0);
    }
...
pub fn calculate_genotype_distance_normalized(a: &Candidate, b: &Candidate) -> f64 {
    let raw_dist = calculate_genotype_distance(a, b);
    (raw_dist / 8627.0).min(1.0)
}

pub fn calculate_population_diversity(population: &[Candidate]) -> f64 {
...
pub fn calculate_genotype_distance(a: &Candidate, b: &Candidate) -> f64 {
    let mut dist = 0.0;
...
#[allow(dead_code)] // PLANNED: population diversity penalty — wiring into selection loop deferred to Phase 2
fn apply_similarity_penalty(evaluations: &mut Vec<CandidateEvaluation>) {
    ...
    for eval in evaluations.iter() {
        metrics.min_threshold = metrics.min_threshold.min(eval.candidate.queue_threshold);
        metrics.max_threshold = metrics.max_threshold.max(eval.candidate.queue_threshold);
        metrics.min_edge = metrics.min_edge.min(eval.candidate.base_edge);
        metrics.max_edge = metrics.max_edge.max(eval.candidate.base_edge);
        metrics.min_tp = metrics.min_tp.min(eval.candidate.take_profit);
        metrics.max_tp = metrics.max_tp.max(eval.candidate.take_profit);
        metrics.min_sl = metrics.min_sl.min(eval.candidate.stop_loss);
        metrics.max_sl = metrics.max_sl.max(eval.candidate.stop_loss);
    }
    ...
    let top_strats: Vec<Candidate> = evaluations
        .iter()
        .take(5)
    ...
    for eval in evaluations.iter_mut() {
        ...
        for top in &top_strats {
            if &eval.candidate == top {
                continue;
            }

            // DYNAMIC NORMALIZATION: abs(a-b) / population_range
            let d1 = (eval.candidate.queue_threshold as f64 - top.queue_threshold as f64).abs()
                / range_threshold;
            let d2 = (eval.candidate.base_edge as f64 - top.base_edge as f64).abs() / range_edge;
            let d3 = (eval.candidate.take_profit as f64 - top.take_profit as f64).abs() / range_tp;
            let d4 = (eval.candidate.stop_loss as f64 - top.stop_loss as f64).abs() / range_sl;
        ...
        let penalty_factor = (1.0 - 0.2 * max_similarity)f64.clamp(0.8, 1.0);
...
pub fn crossover(a: &Candidate, b: &Candidate, rng: &mut StdRng) -> Candidate {
...
pub fn evolve_generation(
    ...
    evo: &AssetEvoState,
    cooldown: u32,
    anchor: Option<&Candidate>,
    global_mean: Option<&Candidate>,
    pull_strength: f64,
    generation: usize,
    eval_stability: &[bool],
) -> Vec<Candidate> {
...
use std::hash::BuildHasher;
    ...
    let mut next_gen: Vec<Candidate> = Vec::new();
    ...
    if is_monoculture && generation % 5 == 0 {
        ...
        evaluations
            .iter()
            .take(2)
            .for_each(|e| next_gen_recovery.push(e.candidate.clone()));
    ...
    let mut elites: Vec<CandidateEvaluation> = Vec::new();
    ...
    for candidate in &sorted {
        ...
        if is_diverse {
            let elite = candidate.candidate.clone();
    ...
    let k = (config.population_size / 4)f64.clamp(2, 3);
    ...
    if !next_gen.is_empty() {
        let mut clusters: Vec<&Candidate> = vec![&next_gen[0]];
    ...
    let super_elites: Vec<&CandidateEvaluation> = evaluations
        .iter()
        .filter(|e: &&CandidateEvaluation| {
    ...
    if !super_elites.is_empty() {
        ...
        for _ in 0..synthesis_count {
            ...
            let _elite_cutoff = (evaluations.len() as f64 * 0.2) as usize;
    ...
    while next_gen.len() < config.population_size && attempts_total < max_attempts {
        ...
        if parent1.strategy_id == parent2.strategy_id {
            let mut offspring = parent1.candidate.clone();
            mutate_strategy(
                &mut offspring,
                rng,
                parent1.trade_count,
        ...
        *parent_usage.entry(p2_key).or_insert(0) += 1;
        ...
        let mut offspring = if crossover_suppressed {
            parent1.candidate.clone() // Suppression: Bypass recombination to break cloning dominance
        } else {
            crossover(&parent1.candidate, &parent2.candidate, rng)
        };

        // FIX 4 — ENFORCE BEHAVIORAL DIVERGENCE (Phenotype check)
        if calculate_genotype_distance(&offspring, &parent1.candidate) < 0.1 {
        ...
        // 🔥 FORCE BREAKOUT
        if strategy_hash(&offspring) == strategy_hash(&parent1.candidate) {
            mutate_strategy(
                &mut offspring,
                rng,
                parent1.trade_count,
...
fn tournament_selection_diverse_with_idx<'a>(
    evaluations: &'a Vec<CandidateEvaluation>,
    k: usize,
    rng: &mut StdRng,
    elites: &Vec<CandidateEvaluation>,
    pnl_mu: f64,
    pnl_sigma: f64,
    std_mu: f64,
    std_sigma: f64,
) -> (usize, &'a CandidateEvaluation) {
    let mut best: Option<(usize, &CandidateEvaluation, f64)> = None;
    for _ in 0..k {
        let idx = rng.gen_range(0..evaluations.len());
...
fn tournament_selection_diverse<'a>(
    ...
    let mut best: Option<(&CandidateEvaluation, f64)> = None;
    for _ in 0..k {
...
fn tournament_selection_failure_mode<'a>(
    ...
    let active_evals: Vec<&CandidateEvaluation> = evaluations
        .iter()
        .filter(|e| e.total_attempts >= 10)
        .collect();

    // 2. Apply FIXED Fitness Floor (-0.02)
    let viable_evals: Vec<&CandidateEvaluation> = active_evals
        .iter()
        .filter(|e| e.fitness > -0.02)
        .cloned()
        .collect();

    // 3. Fallback: If no viable candidates, fallback to all active ones (Selection adapts, constraint stays)
...
pub fn synthesize_super_elite(parents: &Vec<&CandidateEvaluation>, _rng: &mut StdRng) -> Candidate {
    // 1. Component: Filters (Best Pattern Credibility)
    let filter_parent = parents
        .iter()
        .max_by(|a: &&&CandidateEvaluation, b: &&&CandidateEvaluation| {
            (***a)
    ...
    Candidate {
        // Group: Thresholds
        queue_threshold: thresh_parent.candidate.queue_threshold,
        base_edge: thresh_parent.candidate.base_edge,
        selectivity: thresh_parent.candidate.selectivity,

        // Group: Execution
        take_profit: exec_parent.candidate.take_profit,
        stop_loss: exec_parent.candidate.stop_loss,
        holding_period: exec_parent.candidate.holding_period,
        entry_offset: exec_parent.candidate.entry_offset,

        // Group: Filters
        w_conviction: filter_parent.candidate.w_conviction,
        w_momentum: filter_parent.candidate.w_momentum,
        w_volatility: filter_parent.candidate.w_volatility,
        exp_conviction: filter_parent.candidate.exp_conviction,
        exp_momentum: filter_parent.candidate.exp_momentum,
        exp_volatility: filter_parent.candidate.exp_volatility,

        archetype: filter_parent.candidate.archetype,

        // Group: Phase D.1.21 (Reality Archetypes)
        direction_bias: filter_parent.candidate.direction_bias,
        vol_floor: thresh_parent.candidate.vol_floor, // thresholds come from consistent parent
        mom_floor: thresh_parent.candidate.mom_floor,
        edge_ratio: exec_parent.candidate.edge_ratio, // RR comes from pnl parent
        participation_threshold: filter_parent.candidate.participation_threshold,
        exec_aggression: 50,
        latency_bias: 10,
        fill_threshold: 50, lineage: 0,
    }
...
pub fn strategy_to_id(s: &Candidate) -> String {
    format!(
...
pub fn mutate_strategy(
    ...
    if rng.gen::<f64>() < (0.05 + 0.10 * evo.mutation_scale).min(0.25) {
        *candidate = random_strategy(&GaConfig::default(), rng);
        return;
    }
    ...
    // 🔥 LINEAGE FORCE-MUTATION: Resurrection pressure if candidate is economically inactive
    ...
    if parent_trade_count == 0 {
        mutation_rate = (mutation_rate * 4.0f64)f64.clamp(0.20, 1.0);
    }

    for _ in 0..num_mutations {
        let mut mutation_type = if parent_trade_count == 0 {
            4 // 🔥 ALWAYS execution mutation for dead candidates
        ...
        let exec_signal = ((parent_trade_count as f64).ln_1p() / 4.0)f64.clamp(0.10, 0.85);
        ...
        match mutation_type {
            0 => {
                ...
                candidate.queue_threshold = (candidate.queue_threshold as i64 + delta)
                    f64.clamp((20 * GA_GENE_SCALE) as i64, (200 * GA_GENE_SCALE) as i64)
                    as u64;
            }
            1 => {
                // Flip TP/SL (within bounds - ATR Multipliers)
                if rng.gen_bool(0.3) {
                    std::mem::swap(&mut candidate.take_profit, &mut candidate.stop_loss);
                }
            }
            2 => {
                // Base_edge change
                ...
                candidate.base_edge = (candidate.base_edge as i64 + delta)
                    f64.clamp((1 * GA_GENE_SCALE) as i64, (50 * GA_GENE_SCALE) as i64)
                    as u64;
            }
            3 => {
                ...
                candidate.take_profit =
                    (candidate.take_profit as i64 + delta_tp)f64.clamp(100, 500) as u64;
                candidate.stop_loss = (candidate.stop_loss as i64 + delta_sl)f64.clamp(50, 300) as u64;
            }
            4 => {
                ...
                match rng.gen_range(0..6) {
                    0 => {
                        candidate.w_conviction =
                            (candidate.w_conviction as i64 + delta_w)f64.clamp(5, 150) as u64
                    }
                    1 => {
                        candidate.w_momentum =
                            (candidate.w_momentum as i64 + delta_w)f64.clamp(5, 150) as u64
                    }
                    2 => {
                        candidate.w_volatility =
                            (candidate.w_volatility as i64 + delta_w)f64.clamp(5, 150) as u64
                    }
                    3 => {
                        candidate.exp_conviction =
                            (candidate.exp_conviction as i64 + delta_e)f64.clamp(50, 300) as u64
                    }
                    4 => {
                        candidate.exp_momentum =
                            (candidate.exp_momentum as i64 + delta_e)f64.clamp(50, 300) as u64
                    }
                    _ => {
                        candidate.exp_volatility =
                            (candidate.exp_volatility as i64 + delta_e)f64.clamp(50, 300) as u64
                    }
                }
            }
            _ => {
                ...
                if rng.gen_bool(mutation_rate) {
                    candidate.selectivity = rng.gen_range(60..=90);
                }
                if rng.gen_bool(mutation_rate) {
                    candidate.archetype = rng.gen_range(0..=3);
                }
            }
        }
    }
    ...
    if rng.gen_bool(mutation_rate) {
        candidate.direction_bias = [0, 50, 100][rng.gen_range(0..3)];

        candidate.vol_floor = (candidate.vol_floor as i32 + rng.gen_range(-5..6))f64.clamp(5, 80) as u8;

        candidate.mom_floor = (candidate.mom_floor as i32 + rng.gen_range(-5..6))f64.clamp(5, 80) as u8;

        candidate.edge_ratio =
            (candidate.edge_ratio as i32 + rng.gen_range(-10..11))f64.clamp(100, 300) as u8;

        candidate.participation_threshold =
            (candidate.participation_threshold as i32 + rng.gen_range(-5..6))f64.clamp(10, 90) as u8;

        candidate.entry_offset = (candidate.entry_offset + rng.gen_range(-2..3))f64.clamp(-10, 15);
    }
    // 🔥 Coupled mutation (coherent behavior shifts)
    if rng.gen_bool((0.35 * evo.mutation_scale)f64.clamp(0.0, 1.0)) {
        candidate.take_profit = rng.gen_range(120..400);
        candidate.stop_loss = rng.gen_range(80..250);
        candidate.selectivity = rng.gen_range(50..95);
        candidate.entry_offset = rng.gen_range(-5..10);
    }

    if rng.gen_bool((0.1 * evo.mutation_scale)f64.clamp(0.0, 1.0)) {
        candidate.base_edge = rng.gen_range(1 * GA_GENE_SCALE..50 * GA_GENE_SCALE);
    }
    ...
    if let Some(anc) = anchor {
        ...
        candidate.queue_threshold = pull_u64(candidate.queue_threshold, anc.queue_threshold);
        candidate.base_edge = pull_u64(candidate.base_edge, anc.base_edge);
        candidate.take_profit = pull_u64(candidate.take_profit, anc.take_profit);
        candidate.stop_loss = pull_u64(candidate.stop_loss, anc.stop_loss);
        candidate.holding_period = pull_u64(candidate.holding_period, anc.holding_period);
        candidate.w_conviction = pull_u64(candidate.w_conviction, anc.w_conviction);
        candidate.w_momentum = pull_u64(candidate.w_momentum, anc.w_momentum);
        candidate.w_volatility = pull_u64(candidate.w_volatility, anc.w_volatility);
        candidate.exp_conviction = pull_u64(candidate.exp_conviction, anc.exp_conviction);
        candidate.exp_momentum = pull_u64(candidate.exp_momentum, anc.exp_momentum);
        candidate.exp_volatility = pull_u64(candidate.exp_volatility, anc.exp_volatility);
        candidate.selectivity = pull_u8(candidate.selectivity, anc.selectivity);
        candidate.archetype = pull_u8(candidate.archetype, anc.archetype);
        candidate.entry_offset = pull_i32(candidate.entry_offset, anc.entry_offset);
        candidate.direction_bias = pull_u8(candidate.direction_bias, anc.direction_bias);
        candidate.vol_floor = pull_u8(candidate.vol_floor, anc.vol_floor);
        candidate.mom_floor = pull_u8(candidate.mom_floor, anc.mom_floor);
        candidate.edge_ratio = pull_u8(candidate.edge_ratio, anc.edge_ratio);
        candidate.participation_threshold = pull_u8(
            candidate.participation_threshold,
    ...
    if !is_elite && is_stable {
        if let (Some(mean), true) = (global_mean, pull_strength > 0.0) {
            ...
            candidate.queue_threshold =
                lerp_u64(candidate.queue_threshold, mean.queue_threshold, p_pull);
            candidate.base_edge = lerp_u64(candidate.base_edge, mean.base_edge, p_pull);
            candidate.take_profit = lerp_u64(candidate.take_profit, mean.take_profit, p_pull);
            candidate.stop_loss = lerp_u64(candidate.stop_loss, mean.stop_loss, p_pull);
            candidate.holding_period =
                lerp_u64(candidate.holding_period, mean.holding_period, p_pull);
            candidate.w_conviction = lerp_u64(candidate.w_conviction, mean.w_conviction, p_pull);
            candidate.w_momentum = lerp_u64(candidate.w_momentum, mean.w_momentum, p_pull);
            candidate.w_volatility = lerp_u64(candidate.w_volatility, mean.w_volatility, p_pull);
            candidate.exp_conviction =
                lerp_u64(candidate.exp_conviction, mean.exp_conviction, p_pull);
            candidate.exp_momentum = lerp_u64(candidate.exp_momentum, mean.exp_momentum, p_pull);
            candidate.exp_volatility =
                lerp_u64(candidate.exp_volatility, mean.exp_volatility, p_pull);
            candidate.selectivity = lerp_u8(candidate.selectivity, mean.selectivity, p_pull);
            candidate.archetype = lerp_u8(candidate.archetype, mean.archetype, p_pull);
            candidate.entry_offset = lerp_i32(candidate.entry_offset, mean.entry_offset, p_pull);
            candidate.direction_bias = lerp_u8(candidate.direction_bias, mean.direction_bias, p_pull);
            candidate.vol_floor = lerp_u8(candidate.vol_floor, mean.vol_floor, p_pull);
            candidate.mom_floor = lerp_u8(candidate.mom_floor, mean.mom_floor, p_pull);
            candidate.edge_ratio = lerp_u8(candidate.edge_ratio, mean.edge_ratio, p_pull);
            candidate.participation_threshold = lerp_u8(
                candidate.participation_threshold,
...
pub struct GaRoundTripOutcome {
    ...
    pub raw_q_ratio: f64,
    pub fill_efficiency: f64,
    pub sim_events: Vec<chronosentiment_core::SimEvent>,
...
pub struct ConvictionOutcome {
    ...
    pub raw_q_ratio: f64,
    pub regime: ScenarioContext, // Phase D.1.24
...
pub fn evaluate_market_conviction(
    candidate: &Candidate,
    scenario_name: &str,
    signal_events: &[chronosentiment_core::MarketEvent],
    cursor_i: usize,
    trade_idx: usize,
    generation: usize,
) -> ConvictionOutcome {
    ...
    candidate.holding_period.hash(&mut hasher);
    // Phase D.1.8: Include scoring genes in perception-hash
    candidate.w_conviction.hash(&mut hasher);
    candidate.w_momentum.hash(&mut hasher);
    candidate.w_volatility.hash(&mut hasher);
    candidate.exp_conviction.hash(&mut hasher);
    candidate.exp_momentum.hash(&mut hasher);
    candidate.exp_volatility.hash(&mut hasher);
    ...
    for i in start_idx..cursor_i {
        if signal_events[i].subtype == chronosentiment_core::MarketEventType::Trade {
    ...
    let current_trade_vol = if ref_event.subtype == chronosentiment_core::MarketEventType::Trade {
        ref_event.quantity as f64
    } else {
        avg_vol
    };
    let norm_volume = (current_trade_vol / (avg_vol * 1.5))f64.clamp(0.0, 1.0);
    ...
    let norm_vol_score = (1.0 - (norm_vol / 0.002))f64.clamp(0.0, 1.0);
    ...
    let norm_momentum = (price_delta / momentum_scale)f64.clamp(-1.0, 1.0);
    ...
    let spread = 0.0f64f64.clamp(-3.0, 3.0); // Fallback until z-score is wired
    let vol = norm_volumef64.clamp(-3.0, 3.0);
    ...
    if std::env::var("EDGE_DEBUG").is_ok() {
        println!("[EDGE_DEBUG] sym={} n_vol={:.4} n_mom={:.4} trend_cons={:.3} score={:.4}", 
            scenario_name, norm_vol, norm_momentum, (price_delta / (norm_vol.max(1e-9) * 10.0))f64.clamp(0.0, 1.0), signal_score);
    }

    let _base_conviction = signal_score;

    // === D.1.21 CORE LOGIC (Anchor Scaling & Gating) ===
    let n_vol = (norm_vol * 1000.0)f64.clamp(0.0, 100.0); // 0.001 -> 1.0 -> 100.0
    let n_mom = (norm_momentum * 100.0)f64.clamp(0.0, 100.0);

    let vol_floor = candidate.vol_floor as f64;
    let mom_floor = candidate.mom_floor as f64;

    // Phase V3.2+ Signal Ladder: Adaptive Gating
    let phase = (generation as f64 / 50.0)f64.clamp(0.0, 1.0); // Hardcoded total_gen for now or passed?
    ...
    let vol_scale = (n_vol / vol_floor.max(1.0))f64.clamp(0.5, 1.5);
    let mom_scale = (n_mom / mom_floor.max(1.0))f64.clamp(0.5, 1.5);
    ...
    let dir_signal = dir_signalf64.clamp(-3.0, 3.0);
    ...
    let scaled_dir = (dir_signal * 0.5)f64.clamp(-2.0, 2.0);
    ...
    let final_score = match candidate.direction_bias {
    ...
    let trend_consistency = (price_delta / (norm_vol.max(1e-9) * 10.0))f64.clamp(0.0, 1.0);

    let regime = if norm_vol > high_vol_threshold {
        ScenarioContext::HighVolatilityNoise
    } else if trend_consistency > 0.55 {
        if dir_signal > 0.0 {
            ScenarioContext::BullTrend
        } else if dir_signal <= 0.0 {
            ScenarioContext::BearTrend
        } else {
    ...
    let p_threshold = (candidate.participation_threshold as f64 / 100.0) * gate_looseness;
    let s_threshold = candidate.selectivity as f64 / 100.0;
    ...
    ConvictionOutcome {
        ...
        edge_weight: (final_score / 0.8)f64.clamp(0.2, 3.0),
        norm_momentum,
        norm_volume,
        norm_vol_score,
        norm_vol,
...
pub fn ga_simulate_round_trip_at_cursor(
    ...
    let _interval_scale_factor = 5.0 / feed_interval_minutes;
    ...
    // In live evaluation, callers often pass the latest bar as cursor.
    // If there is no forward room, a round-trip collapses to the same tick.
    // Use a deterministic, causal retrospective anchor (past-only) so
    // geometry/feasibility can measure recent executable movement.
    let last_signal_idx = signal_events.len().saturating_sub(1);
    let last_exec_idx = execution_events.len().saturating_sub(1);
    let last_common_idx = last_signal_idx.min(last_exec_idx);
    let cursor_i = if cursor_i >= last_common_idx {
        let retro_bars = std::env::var("GA_LIVE_RETRO_BARS")
            .ok()
            .and_then(|v| v.parse::<usize>().ok())
            .unwrap_or(60);
        cursor_i.saturating_sub(retro_bars.min(cursor_i))
    } else {
        cursor_i
    };
    ...
    let _ref_ts = ref_event.exchange_ts;
    let total_offset = (config.latency_ticks as i32 + candidate.entry_offset).max(0);
    ...
    let liquidity_norm = (arrival_liquidity / (arrival_liquidity + 1000.0))f64.clamp(0.0, 1.0);
    let fill_proxy = (0.5 * liquidity_norm + 0.5 * (1.0 - queue_norm))f64.clamp(0.0, 1.0);
    ...
    let rt_fail_log = |reason: &str, raw_edge: Option<f64>, rank: Option<f64>| {
        if round_trip_probe {
            println!(
    ...
    let edge_bias = ((candidate.base_edge as f64 - 5.0) / 50.0)f64.clamp(-0.12, 0.12);
    // Use aggressiveness from conviction
    let aggressiveness = conviction.conviction_score;
    let _agg_threshold = ((aggressiveness / 1.1) + edge_bias)f64.clamp(0.05, 0.98);
    let _tick_01 = ((0.01 * crate::PRICE_SCALE as f64) as f64).round() as u64;
    ...
    let _rr = (candidate.edge_ratio as f64) / 100.0;
    ...
    let heuristic_directional_alpha = conviction.norm_momentum.abs()f64.clamp(0.0, 0.015);
    let heuristic_execution_alpha = (fill_proxy * (1.0 - (spread / sig_px.max(1e-9) / 0.001)f64.clamp(0.0, 0.5)))f64.clamp(0.0, 1.0);
    let heuristic_structural_alpha = (1.0 - conviction.norm_vol.min(0.6))f64.clamp(0.0, 1.0);
    
    let momentum_boost = 1.0 + 1.5 * conviction.norm_momentum.abs();
    let heuristic_score_base = 0.7 * heuristic_directional_alpha + 0.3 * (adjusted_atr / buy_price.max(1) as f64)f64.clamp(0.0001, 0.01);
    
    let raw_edge = (heuristic_score_base * momentum_boost * heuristic_execution_alpha * heuristic_structural_alpha)f64.clamp(0.0, 0.02);
    ...
    // This allows decent signals to inform the EvaluationMetrics, creating a real gradient.
    ...
    let rank = if rank_mode.eq_ignore_ascii_case("xsec") {
        ...
        ((raw_edge - stats.p10) / denom)f64.clamp(0.0, 1.0)
    } else {
        stats.rank(raw_edge)
    };
            ...
            if let Ok(map_raw) = std::env::var("GA_LEARN_PASS_MIN_MAP") {
                for item in map_raw.split(',') {
                    ...
                    if let (Some(k), Some(v)) = (k, v) {
                        if k == bucket_tag {
                            learn_pass_min = vf64.clamp(0.0, 1.0);
                            break;
                        }
                    }
                }
    ...
    let target_move = (exp_mfe.abs() * buy_price as f64)f64.clamp(buy_price as f64 * 0.0005, buy_price as f64 * 0.05);
    // Dynamic stops based on RISK (MAE)
    let stop_move = (exp_mae.abs() * buy_price as f64)f64.clamp(buy_price as f64 * 0.0003, buy_price as f64 * 0.03);
    ...
    let (mut tp_target, mut sl_target) = if is_long {
        ((
            (buy_price as f64 + target_move) as f64).round() as u64,
            ((buy_price as f64 - stop_move) as f64).round().max(1.0) as u64,
        )
    } else {
        ((
            (buy_price as f64 - target_move) as f64).round().max(1.0) as u64,
            ((buy_price as f64 + stop_move) as f64).round() as u64,
        )
    };

    let expected_hold_time = config.rank_stats.get_expected_time(rank, vol_bucket)f64.clamp(3.0, 300.0) as usize;
    ...
    if std::env::var("GA_BOOTSTRAP").is_ok() {
        // Keep more runway for deterministic trend regimes during bootstrap diagnostics.
        final_holding_period = final_holding_period.max(40);
    }
    ...
    let _ts_offset = base_sl * (0.2 + 0.6 * (1.0 - strength));

    // Apply finalized geometry distances to execution targets.
    // This ensures candidate shaping in TP/SL coupling affects real and ideal outcomes.
    let min_tick = 1_u64;
    let (exec_tp_target, exec_sl_target) = if is_long {
        ((
            (buy_price as f64 + tp_dist_final) as f64).round() as u64,
            ((buy_price as f64 - sl_dist_final) as f64).round().max(1.0) as u64,
        )
    } else {
        ((
            (buy_price as f64 - tp_dist_final) as f64).round().max(1.0) as u64,
            ((buy_price as f64 + sl_dist_final) as f64).round() as u64,
        )
    };
    let mut exec_tp_target = exec_tp_target;
    let mut exec_sl_target = exec_sl_target;
    if exec_tp_target == buy_price || exec_sl_target == buy_price {
        if is_long {
            exec_tp_target = buy_price + min_tick;
            exec_sl_target = buy_price.saturating_sub(min_tick);
        } else {
            exec_tp_target = buy_price.saturating_sub(min_tick);
            exec_sl_target = buy_price + min_tick;
        }
    }
    ...
    // Extract Outcome Data from Event Execution (return space, not absolute price space)
    let buy_price_f = buy_price.max(1) as f64;
    let realized_pnl = if is_long {
        (execution.exit_price as f64 - buy_price as f64) / buy_price_f
    } else {
        (buy_price as f64 - execution.exit_price as f64) / buy_price_f
    };
    let ideal_entry_price = signal_events[cursor_i].price.max(1) as f64;
    let ideal_pnl_raw = if is_long {
        (ideal_execution.exit_price as f64 - ideal_entry_price) / ideal_entry_price
    } else {
    ...
    */
    ...
    if std::env::var("EDGE_DEBUG").is_ok() {
        let delta_abs = exit_price as f64 - buy_price as f64;
        let delta_ret = delta_abs / buy_price_f;
        println!(
            "[GEOM_DEBUG] cursor={} entry={:.8} exit={:.8} delta_abs={:.8} delta_ret={:.12} horizon={} ideal_raw={:.12} ideal={:.12}",
            cursor_i,
            buy_price as f64,
            exit_price as f64,
            delta_abs,
    ...
    let mfe_scaled = match exit_reason {
        chronosentiment_core::GaExitReason::TakeProfit => tp_target,
        _ => buy_price,
    };
    let mae_scaled = match exit_reason {
        chronosentiment_core::GaExitReason::StopLoss => sl_target,
        chronosentiment_core::GaExitReason::TimeStop => exit_price,
        _ => buy_price,
    };
    ...
    let _expected_return = expected_move / buy_price_f;
    ...
    let efficiency = if ideal_pnl.abs() > 1e-6 {
        (realized_pnl / ideal_pnl)f64.clamp(-1.0, 1.0)
    } else {
        if ga_debug_enabled() {
            println!(
    ...
    let edge_quality = (mfe_pnl / mae_pnl.abs().max(1e-9))f64.clamp(0.0, 5.0);
    let edge_boost = (edge_quality / 5.0)f64.clamp(0.0, 1.0);
    let e_score = (0.7 * efficiency + 0.3 * edge_boost)f64.clamp(-1.0, 1.0);
    ...
    let p_direction = (rank * 0.8 + conviction.norm_momentum.abs() * 0.2)f64.clamp(0.0, 1.0);

    // 3. Execution feasibility (Efficiency x Fill Probability)
    let p_capture = (efficiency.max(0.0) * conviction.edge_weightf64.clamp(0.1, 1.0))f64.clamp(0.0, 1.0);
    ...
    let p_capture_est = conviction.edge_weightf64.clamp(0.1, 1.0);
    let eff_norm = (efficiency + 1.0) / 2.0; 
    let favorability_score = 0.5 * p_capture_est 
                           + 0.3 * eff_normf64.clamp(0.0, 1.0) 
                           + 0.2 * (1.0 - conviction.norm_vol)f64.clamp(0.0, 1.0);

    // 2. Conditional Probability Proxy
    let p_dir_proxy = conviction.conviction_scoref64.clamp(0.5, 1.0);
    ...
    let allocation_score = (1.0 - (dist_from_peak / effective_band_width))f64.clamp(0.0, 1.0);
    ...
    let raw_recommended_size = (kelly_proxy * allocation_score)f64.clamp(0.1, 1.0);
    ...
    let exit_reason_str = match exit_reason {
        chronosentiment_core::GaExitReason::TakeProfit => "target_hit",
        chronosentiment_core::GaExitReason::StopLoss => "stop_hit",
        chronosentiment_core::GaExitReason::TimeStop => "timeout",
        chronosentiment_core::GaExitReason::NoFill => "no_fill",
    };
    ...
    let capture_eff = raw_capturef64.clamp(-1.0, 1.0);
    ...
    Some(GaRoundTripOutcome {
        ...
        fill_efficiency: conviction.edge_weightf64.clamp(0.1, 1.0),
        sim_events: Vec::new(),
        entry_order_id,
        exit_order_id,
...
#[allow(dead_code)] // UTILITY: statistical percentile — used by analytics paths not yet wired
...
pub fn ga_scenario_rank_score(e: &CandidateEvaluation) -> f64 {
    let edge = e.robustness.max(0.0).max(e.avg_pnl.max(0.0));
    let conf = if e.trade_count > 0 {
        (e.profitable_trades as f64 / e.trade_count as f64)f64.clamp(0.0, 1.0)
    } else {
        0.0
    };
...
pub fn ga_top_k_pick_diverse(
    mut remaining: Vec<(usize, f64, CandidateEvaluation)>,
    k: usize,
    ...
    let mut out: Vec<CandidateEvaluation> = Vec::with_capacity(k);
...
fn apply_ga_top_k_selection(
    mut evaluations: Vec<CandidateEvaluation>,
    ...
    let context_evals: Vec<CandidateEvaluation> = evaluations
        .iter()
    ...
    let remaining: Vec<(usize, f64, CandidateEvaluation)> = evaluations
        .into_iter()
        .enumerate()
        .map(|(i, e)| {
    ...
    let mut final_evals: Vec<CandidateEvaluation> = indexed;
...
pub fn aggregate_strategy_reports(
    ...
    let mut elite: Vec<CandidateEvaluation> = evaluations
        .into_iter()
    ...
    let elite_evals: Vec<CandidateEvaluation> = elite.into_iter().take(8).collect();
...
pub fn extract_behavioral_clusters(
    mut population: Vec<CandidateEvaluation>,
    target_count: usize,
    ...
    let mut candidates: Vec<CandidateEvaluation> = population
        .into_iter()
...
pub fn calculate_behavioral_distance(
    ...
    if a.trade_count < MIN_TRADES || b.trade_count < MIN_TRADES {
        return calculate_genotype_distance(&a.candidate, &b.candidate);
    }
...
fn aggregate_strategy_reports_inner(
    ...
    config: &GaConfig,
    _generation: usize,
) -> Option<(CandidateEvaluation, f64)> {
    if evaluations.is_empty() {
        return None;
    }

    let total_scenarios_in = evaluations.len();
    let _has_executable = evaluations.iter().any(|e| e.capability.is_executable());
    ...
    // Penalize candidates that only work in identical conditions to ensure robustness.
    ...
    if total_scenarios > 1.0 {
        let tol = 1e-9f64.max(global_avg_pnl.abs() * 12.0);
    ...
    let global_payoff_ratio = if total_scenarios > 0.0 {
        (total_payoff_ratio_sum / total_scenarios)f64.clamp(0.0, 2.0)
    } else {
        0.0
    };
    ...
    let _pnl_scale = global_avg_pnl.abs().max(0.0001);
    ...
    let mut _total_avg_e_score = 0.0;
    ...
    for e in &evaluations {
        ...
        _total_avg_e_score += e.avg_e_score;
    ...
    for e in &evaluations {
        unique_strategies.insert(strategy_to_id(&e.candidate));
    }
    ...
    let pnl_score = (pnl_bps / 100.0)f64.clamp(-1.0, 1.0);

    // 2. Win Rate Score (0.0 -> 1.0)
    let win_score = win_ratef64.clamp(0.0, 1.0);

    // 3. Execution Score (Capture Efficiency * sqrt(Fill Rate))
    let fill_rate_score = avg_fill_rate.sqrt();
    let eff_score = (avg_efficiencyf64.clamp(0.0, 1.0) * fill_rate_score)f64.clamp(0.0, 1.0);
    ...
    if total_trade_count == 0 {
        ...
        fitness_out *= ((total_trade_count as f64) / 10.0)f64.clamp(0.5, 1.5);
    }
    ...
    // Phase 13.5: institutional shaping on top of the core PnL / win / execution blend.
    // - High normalized selectivity (institutional "shot rate" in ~0.10–0.30) decays fitness; values
    //   from per-window mocks in the ~0.01–0.05 band stay neutral so participation tests are not inverted.
    // - `avg_entropy` near ~0.45 is a robust band; extremes taper off (Gaussian bump).
    // - Low `participation_rate` scales fitness down so sparse-window profiles do not look healthy.
    let avg_entropy_agg =
        evaluations.iter().map(|e| e.avg_entropy).sum::<f64>() / total_scenarios.max(1.0);
    let selectivity_decay = if selectivity >= 0.08 {
        (-(config.lambda * 10.0) * selectivity).exp()
    } else {
        1.0
    };
    let entropy_d = avg_entropy_agg - 0.45;
    let entropy_band = (-12.0 * entropy_d * entropy_d).exp();
    let participation_boost = (0.10 + 0.90 * participation_rate)f64.clamp(0.08, 1.0);
    ...
    dispersion_bonus_agg = dispersion_bonus_aggf64.clamp(
    ...
    let _output_scenarios = evaluations.len();
    ...
    let mut unique_strategies_health: Vec<&Candidate> = Vec::new();
    ...
    for e in &evaluations {
        ...
        if !unique_strategies_health
            .iter()
            .any(|s| calculate_genotype_distance(s, candidate) < 0.05)
        {
            unique_strategies_health.push(candidate);
        }
    }
    ...
    // --- DIVERSITY AXES AUDIT ---
    {
        ...
        for e in &evaluations {
            tp.insert(e.candidate.take_profit);
            sl.insert(e.candidate.stop_loss);
            hold.insert(e.candidate.holding_period);
            edge.insert(e.candidate.base_edge);
            arch.insert(e.candidate.archetype);
            bias.insert(e.candidate.direction_bias);
        }
        if ga_debug_enabled() {
            println!(
    ...
    let mut report = CandidateEvaluation {
        strategy_id: evaluations[0].strategy_id.clone(),
        candidate: evaluations[0].candidate.clone(),
    ...
    let _exec_boost = report.avg_e_score * 0.5;
    ...
    if ga_log_progress() {
        ...
        for (i, (sym, e, _score, conf)) in final_ranking.iter().take(10).enumerate() {
            println!(
                ...
                e.consistency,
                e.opportunity * 100.0,
                (1.0 - e.avg_stability_valid / 0.18)f64.clamp(0.0, 1.0),
...
/// Deterministic slim [`CandidateEvaluation`] row so [`crate::reco::RecommendationEngine::process`]
/// can run on the live multi-candidate ensemble without a full GA training payload.
pub fn strategy_evaluation_for_live_reco_snapshot(
    candidate: Candidate,
    fitness: f64,
    avg_conviction: f64,
    ...
    let mut s = CandidateEvaluation::default();
    s.strategy_id = strategy_to_id(&candidate);
    s.candidate = candidate;
    s.behavioral_signature = s.candidate.get_signature();
    s.fitness = fitnessf64.clamp(0.0, 1.0);
    s.avg_conviction = avg_convictionf64.clamp(-1.0, 1.0);
    s.avg_pnl = s.fitness;
    let fe = execution_feasibilityf64.clamp(0.0, 1.0);
    let cap = capture_efficiencyf64.clamp(-1.0, 1.0);
    s.trade_count = if fe > 0.05 { 1 } else { 0 };
    s.execution_metrics = ExecutionMetrics {
        fill_efficiency: fe,
        capture_efficiency: cap,
        fill_rate: fe as f32,
        avg_slippage: ((1.0 - fe) * 0.25)f64.clamp(0.0, 1.0),
        latency_impact: 0.2,
...
pub fn evaluate_current_status(
    ...
    if probe_this_symbol {
        println!(
            ...
            symbol, price, momentum_3, momentum_5, vol_5, vol_20, 0.0f64, score_std, has_history as i32, bars_seen
        );
        if bars_seen < 20 {
    ...
    if candidate.archetype == 2 && last_signal == SignalType::WAIT {
    ...
    for candle in history {
        events.push(chronosentiment_core::MarketEvent {
    ...
    // 3. Conviction Engine
    let conviction = evaluate_market_conviction(candidate, "live", &events, last_idx, 0, 0);
    if probe_this_symbol {
        println!(
    ...
    let directional_edge = conviction.conviction_score.abs().powf(0.7);
    let strength = (0.8f64 * directional_edge + 0.2f64 * 0.5f64)f64.clamp(0.05f64, 1.0f64);
    ...
    let skip_orthogonal = if std::env::var("GA_BOOTSTRAP").is_ok() { false } else {
        match candidate.archetype {
            2 => regime != ScenarioContext::MeanReversion, // Mean Reversion ONLY
            1 => regime != ScenarioContext::BullTrend && regime != ScenarioContext::BearTrend, // Trend ONLY
    ...
    let mut trade_rec: Option<ExecutionProposal> = None;
    let mut final_raw_edge: f64;
    let mut feasibility = 0.0;
    let mut effective_feasibility = 0.0;

    let edge_threshold = 0.0010;
    let _edge_probe = std::env::var("EDGE_PROBE").is_ok();
    ...
    if let Some(ref rt) = outcome {
        ...
        // (deterministic; does not alter event replay — only the scalar edge used for gating/ranking).
        let cur = &history[last_idx];
        let price = cur.close as f64;
        if price.is_finite() && price > 1e-12 && last_idx >= 1 {
            let prev_price = history[last_idx - 1].close as f64;
            let prev_3_price = if last_idx >= 3 {
                history[last_idx - 3].close as f64
            } else {
                prev_price
            };
            let r1 = if prev_price > 1e-12 {
                (price / prev_price) - 1.0
            } else {
                0.0
            };
            let r3 = if prev_3_price > 1e-12 {
                (price / prev_3_price) - 1.0
            } else {
                0.0
            };
            let hi = cur.high as f64;
            let lo = cur.low as f64;
            let range = ((hi - lo) / price.max(1e-12))f64.clamp(0.0, 0.2);
            let lookback = 20usize.min(last_idx);
            let (breakout_up, breakout_dn) = if last_idx >= 2 && lookback >= 1 {
                let start = last_idx.saturating_sub(lookback);
                let end = last_idx - 1;
                let mut recent_high = f64::NEG_INFINITY;
                let mut recent_low = f64::INFINITY;
                for i in start..=end {
                    recent_high = recent_high.max(history[i].high as f64);
                    recent_low = recent_low.min(history[i].low as f64);
                }
                let bu = if price > recent_high { 1.0 } else { 0.0 };
                let bd = if price < recent_low { -1.0 } else { 0.0 };
                (bu, bd)
            } else {
                (0.0, 0.0)
            };
            let mom: f64 = 0.5 * r1 + 0.3 * r3;
            if mom.is_finite() && range.is_finite() {
                // Keep blended edge in the same numeric band as `rt.raw_edge` (downstream feas/RR gates).
                let mom_capped = momf64.clamp(-0.002, 0.002);
                let breakout = 0.001 * (breakout_up + breakout_dn);
                let mut blended =
                    (final_raw_edge + mom_capped) * (1.0 + 2.0 * range) + breakout;
                if !blended.is_finite() {
                    blended = rt.raw_edge;
                } else {
                    blended = blendedf64.clamp(0.0, 0.01);
                }
        ...
        if std::env::var("COMPONENT_TRACE").is_ok() {
            ...
            if trace_idx % 50 == 0 {
                if let Some((r1, r3, range, breakout_net)) = edge_components {
                    ...
                    let mom_capped = mom_rawf64.clamp(-0.002, 0.002);
        ...
        let _is_bearish = conviction.conviction_score < 0.0;
        ...
        feasibility = if ideal > 1e-7 {
            (realized / ideal)f64.clamp(0.0, 1.0)
        ...
        let edge_thresh = if bootstrap_mode {
            0.0001
        } else {
            edge_threshold
        };
        let _pass_main_gate = final_raw_edge >= edge_thresh && effective_feasibility > feas_thresh;
        ...
        let _final_sig = if is_bearish { SignalType::SELL } else { SignalType::BUY };
        
        // REJECT deceptive Low Hostility environments unless conviction is absolute.
        if conviction.regime == ScenarioContext::MeanReversion && conviction.conviction_score < 0.90 && std::env::var("GA_BOOTSTRAP").is_err() {
             if std::env::var("EDGE_DEBUG").is_ok() {
                 println!(
        ...
        if final_raw_edge >= edge_thresh && effective_feasibility > feas_thresh {
            ...
            let raw_rank = (final_raw_edge / 0.0050)f64.clamp(0.0, 1.0);
            let fill_prob = rt.fill_efficiency;
            let adjusted_rank = (raw_rank * effective_feasibility * fill_prob)f64.clamp(0.0, 1.0);
            
            confidence = (final_raw_edge / (final_raw_edge + 0.001))f64.clamp(0.0, 1.0);
            ...
            maybe_emit_rankstats_probe(&config.rank_stats);
            ...
            let (mfe_mult, mae_mult) = if conviction.regime == ScenarioContext::HighVolatilityNoise {
                (0.85, 1.15)
            } else {
                (0.50, 1.50)
            };
            ...
            if expected_rr >= 1.25 {
                ...
                trade_rec = Some(ExecutionProposal {
                    symbol: symbol.to_string(),
                    signal,
                    rank: adjusted_rank, // 🔥 Rank is now execution-aware
                    raw_edge: final_raw_edge,
                    confidence: (adjusted_rank * (expected_rr.min(3.0) / 3.0))f64.clamp(0.0, 1.0),
                    ...
                    holding_bars: {
                        ...
                        ((half_life / feed_interval_minutes) as f64).round() as usize
                    },
                    vol_bps,
                    vol_bucket: rt.vol_bucket,
                    is_execution: rt.is_execution,
                    position_size: {
                        let mut base_size = (adjusted_rank * adjusted_rank)f64.clamp(0.01, 1.0);
                        if symbol.starts_with("SOL") { base_size = (base_size * 1.5)f64.clamp(0.01, 1.0); }
                        else if symbol.starts_with("BTC") { base_size = (base_size * 0.25)f64.clamp(0.01, 0.5); }
                        base_size
                    },
        ...
        let fallback_edge = if price_now.abs() > 1e-12 {
        ...
        if fallback_edge > 0.0003 {   // 3 bps minimum movement
            ...
            // --- REVIVE: synthesize minimal recommendation to unblock candidate path ---
            ...
            trade_rec = Some(ExecutionProposal {
                symbol: symbol.to_string(),
                signal,
                rank: (final_raw_edge / 0.0050)f64.clamp(0.01, 1.0),
                raw_edge: final_raw_edge,
                confidence: 0.5,
                quality_score: 1.0,
                entry_price,
    ...
    // --- ALWAYS LOG EDGE_PIPE FOR DIAGNOSTICS (Truly Unconditional) ---
    {
        let (rr_val, pass_rr_gate) = if outcome.is_some() {
            ...
            let raw_rank = (final_raw_edge / 0.0050)f64.clamp(0.0, 1.0);
            let adjusted_rank =
                (raw_rank * effective_feasibility * rt.fill_efficiency)f64.clamp(0.0, 1.0);
            let mfe_raw = config.rank_stats.get_expected_mfe(adjusted_rank, rt.vol_bucket);
            let mae_raw = config.rank_stats.get_expected_mae(adjusted_rank, rt.vol_bucket);
...
// Paper-trading implementation moved to `core/src/paper.rs`.
...
pub fn load_elite_strategies(path: &str) -> Vec<CandidateEvaluation> {
    use std::fs;
...
pub fn calculate_dynamic_threshold(volatility: f64, avg_quality: f64) -> f64 {
    ...
    (base + vol_adj + quality_adj as f64)f64.clamp(0.60, 0.90)
}
...
pub fn evaluate_consensus_status(
    ...
    let lookback_bars = ((20.0 * interval_scale_factor) as f64).round() as usize;
    ...
    let eligible_voters: Vec<&CandidateEvaluation> = evals
        .iter()
    ...
    let norm_momentum = (price_delta / 0.001)f64.clamp(0.0, 1.0);
    ...
    for (i, voter) in eligible_voters.iter().enumerate() {
        ...
        let bias_archetype = classify_direction_bias(voter.candidate.direction_bias);
        weight *= regime_multiplier(current_regime, bias_archetype) * universal_multiplier;

        let consensus_score_v = conviction_guard.conviction_scoref64.clamp(-1.0, 1.0);
...
#[allow(dead_code)] // PLANNED: genome feature importance weights — extract_features() uses inline normalization; consolidation deferred
...
pub fn extract_features(candidate: &Candidate) -> Vec<f64> {
    let mut features = Vec::with_capacity(13);
    let norm = |val: f64, max: f64| (val / max)f64.clamp(0.0, 1.0);

    features.push(norm(candidate.queue_threshold as f64, 5000.0));
    features.push(norm(candidate.base_edge as f64, 500.0));
    features.push(norm(candidate.take_profit as f64, 500.0));
    features.push(norm(candidate.stop_loss as f64, 500.0));
    features.push(norm(candidate.holding_period as f64, 100.0));
    features.push(norm(candidate.w_conviction as f64, 100.0));
    features.push(norm(candidate.w_momentum as f64, 100.0));
    features.push(norm(candidate.w_volatility as f64, 100.0));
    features.push(norm(candidate.exp_conviction as f64, 100.0));
    features.push(norm(candidate.exp_momentum as f64, 100.0));
    features.push(norm(candidate.exp_volatility as f64, 100.0));
    features.push(norm(candidate.selectivity as f64, 100.0));
    features.push(norm(candidate.archetype as f64, 3.0));
...
pub fn compute_consensus_alpha(
    ...
    for candidate in elites {
        ...
        if let Some(res) =
            evaluate_strategy(candidate, scenario, &strict_config, 0, 0.0, 0, 0.0, 1.0, 0)
        {
            if ga_debug_enabled() {
                println!(
            ...
            if decision != Decision::HOLD {
                for sig in &res.emitted_signals {
                    ...
                    votes.push(SignalVote {
                        strategy_id: strategy_to_id(candidate),
                        archetype: Archetype::from(candidate.archetype),
                        confidence: sig.strength,
                        signal_features: extract_features(candidate),
    ...
    for (idx, votes) in signal_votes_map {
        ...
        let consistency = (buy_c.max(sell_c) as f64 / count.max(1) as f64)f64.clamp(0.0, 1.0);
        let exec_prob_proxy = (support_ratio * 2.0)f64.clamp(0.5, 1.0);
...
mod tests {
    ...
    fn test_ga_determinism() {
        ...
        assert_eq!(
            ga_result1.global_best.candidate, ga_result2.global_best.candidate,
            "Best candidate diverged with same seed"
        );
        assert!(
            (ga_result1.global_best.fitness - ga_result2.global_best.fitness).abs() < 1e-6,
            "Best candidate fitness diverged with same seed"
        );
        assert_eq!(
        ...
        assert_eq!(
            ga_result1.final_generation_best.candidate, ga_result2.final_generation_best.candidate,
            "Final generation best candidate diverged"
        );
        assert!(
    ...
    fn test_evaluate_strategy() {
        ...
        let report = evaluate_strategy(&candidate, &pair, &config, 0, 0.0, 0, 0.0, 1.0, 0);

        if let Some(r) = report {
            assert_eq!(r.candidate, candidate);
            assert!(!r.strategy_id.is_empty());
            assert!(
                r.fitness.is_finite(),
                "per-scenario fitness must be finite (starvation uses soft negative codes)"
            );
            if r.trade_count > 1 {
    ...
    fn test_evaluate_strategy_multi_trade_cap_respected() {
        ...
        for name in &keys {
            ...
            if let Some(r) = evaluate_strategy(&candidate, &pair, &config, 0, 0.0, 0, 0.0, 1.0, 0) {
                assert!(
                    r.trade_count <= cap,
        ...
        assert!(
            found,
            "expected at least one RELIANCE window where candidate trades (for cap test)"
        );
    }
    ...
    fn test_ga_evolution_with_benchmarks() {
        ...
        // Benchmark tapes can yield negative fitness while avg_pnl is flat or weak; require a real score.
        assert!(ga_result.global_best.fitness.is_finite());
    ...
    fn test_fitness_sparse_strategy_collapse() {
        ...
        // Participation is 0.2; aggregate fitness should stay low (may be slightly negative on all-loss edges).
        assert!(
            aggregated.fitness < 0.5 && aggregated.fitness > -1.0,
            "Sparse candidate fitness should be very low ({}).",
            aggregated.fitness
        );
    }
    ...
    fn test_fitness_high_participation_outperforms() {
        crate::init_test_delegates();
        // Candidate A: high participation (0.8), stronger pnl/trade profile
        ...
        // Candidate B: low participation (0.3), higher average active pnl
        ...
        assert!(
            agg_b.fitness > -1.0,
            "Low participation fitness {} should stay bounded.",
            agg_b.fitness
        );
    }
    ...
    fn test_fitness_low_trade_count_penalizes() {
        crate::init_test_delegates();
        let config = get_default_ga_config();
        // Candidate A: 5 trades total (5 active scenarios, 1 trade each)
        ...
        // Candidate B: strong profile with full participation and enough trades
        let mut evals_b = Vec::new();
        for _ in 0..10 {
        ...
        assert!(
            agg_a.fitness < 0.5 && agg_a.fitness > -1.0,
    ...
    fn test_multiplicative_fitness_aggregation() {
        ...
        assert!(
            agg.fitness < 0.0,
            "All-negative PnL windows should produce negative aggregate fitness, got {}",
            agg.fitness
        );
    }
    ...
    fn test_fitness_hard_collapse_threshold() {
        ...
        assert!(
            agg_collapse.fitness < 0.5 && agg_collapse.fitness > -1.0,
    ...
    fn test_fitness_relative_ordering() {
        ...
        assert!(weak.fitness > -1.0);
        assert!(strong.fitness > -1.0);
    }
    ...
    fn ga_top_k_pick_diverse_lambda_zero_matches_pure_rank_order() {
        ...
        let remaining: Vec<(usize, f64, CandidateEvaluation)> = evals
            .into_iter()
            .enumerate()
            .map(|(i, e)| {
    ...
    fn ga_top_k_pick_diverse_is_deterministic() {
        ...
        let build_remaining = |ev: Vec<CandidateEvaluation>| {
            ev.into_iter()
                .enumerate()
                .map(|(i, e)| {
                    let s = super::ga_scenario_rank_score(&e);
    ...
    fn ga_top_k_repel_vs_attract_second_pick() {
        ...
        let build = |ev: Vec<CandidateEvaluation>| {
            ev.into_iter()
                .enumerate()
                .map(|(i, e)| {
                    let s = super::ga_scenario_rank_score(&e);
    ...
    fn test_synthetic_ga_microstructure_harness() {
        ...
        let (eval, depth) = evaluate_and_aggregate_with_trade_depth(
            &ga_result.global_best.candidate,
            &config,
            &scenarios_vec,
            0,
            0.0,
        ...
        // Without starvation mercy / synthetic scores, harness fitness may be small or negative;
        // still require a finite aggregate (deterministic pipeline completed).
        assert!(
            eval.fitness.is_finite() && eval.fitness > -10.0,
            "synthetic harness produced invalid fitness {:.6}",
            eval.fitness
        );
        if ga_debug_enabled() {
            println!(
    ...
    fn test_evaluate_and_aggregate_enforces_path() {
        ...
        assert!(eval.fitness.is_finite());
        assert!(
            avg_trades_per_active > 0.0,
            "expected some trades in aggregate after evolution, got {}",
    ...
    fn test_selectivity_decay_alpha_5() {
        ...
        let mock_scenario_eval =
            |pnl: f64, trades: usize, profitable: usize| -> CandidateEvaluation {
                CandidateEvaluation {
                    avg_pnl: pnl,
                    trade_count: trades,
                    profitable_trades: profitable,
        ...
        // 1. Nominal candidate (10% selectivity -> 0.10)
