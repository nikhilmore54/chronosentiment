...
pub fn load_traffic_matrix<P: AsRef<Path>>(path: P) -> Result<TrafficMatrix> {
    ...
    let tm: TrafficMatrix =
        serde_json::from_reader(reader).context("Failed to parse traffic matrix JSON")?;
    Ok(tm)
}
...
pub fn load_scenario<P: AsRef<Path>>(path: P) -> Result<Scenario> {
    ...
    let scenario: Scenario =
        serde_json::from_reader(reader).context("Failed to parse scenario JSON")?;
    Ok(scenario)
}
...
pub fn load_solution<P: AsRef<Path>>(path: P) -> Result<Solution> {
    ...
    let solution: Solution =
        serde_json::from_reader(reader).context("Failed to parse solution JSON")?;
    Ok(solution)
}
