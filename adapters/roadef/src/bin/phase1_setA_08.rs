...
fn main() {
    println!("Loading setA-08...");
    let net = load_network("adapters/roadef/repo/challenge-roadef-2026-main/setA/setA-08-net.json")
        .unwrap();
    let tm =
        load_traffic_matrix("adapters/roadef/repo/challenge-roadef-2026-main/setA/setA-08-tm.json")
            .unwrap();
    let scenario =
        load_scenario("adapters/roadef/repo/challenge-roadef-2026-main/setA/setA-08-scenario.json")
            .unwrap();
    ...
    if !result.trajectory.is_empty() {
        ...
        let total_cache_hit_materialize_ms: f64 = result
            .trajectory
            .iter()
            .map(|g| g.cache_hit_materialize_ms)
            .sum();
