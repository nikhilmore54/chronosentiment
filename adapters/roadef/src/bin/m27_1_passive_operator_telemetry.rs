...
fn build_context_tag(path_nodes: &[u64], vol: f64, max_cap: f64, avg_congestion: f64) -> String {
...
impl<'a> Solver<'a> {
    fn solve_recursive(
        ...
        // Returns (subtree_survived, best_subtree_obj, max_survival_depth)
        ...
        for (op_idx, path) in paths.iter().enumerate() {
            ...
            if res.valid {
                ...
                let (sub_success, sub_obj, sub_depth) =
                    self.solve_recursive(demand_idx + 1, current_solution, next_cong, child_obj);
            ...
            let objective_delta = if child_obj < f64::MAX && parent_obj < f64::MAX {
                child_obj - parent_obj
            } else {
                0.0
            };

            let incumbent_gap = if child_obj < f64::MAX && self.best_obj < f64::MAX {
                child_obj - self.best_obj
            } else {
                f64::MAX
            };
            let incumbent_improvement = if child_obj < self.best_obj {
                self.best_obj - child_obj
            } else {
                0.0
            };

            let operator_outcome = if child_obj < parent_obj {
                "Improved"
            } else if child_obj == parent_obj {
                "Neutral"
            } else {
                "Degraded"
            };
            ...
            self.logs.push(OperatorObservation {
                ...
                child_pressure,
                pressure_delta,
                parent_objective: if parent_obj == f64::MAX {
                    -1.0
                } else {
                    parent_obj
                },
                child_objective: if child_obj == f64::MAX {
                    -1.0
                } else {
                    child_obj
                },
                ...
                incumbent_gap: if incumbent_gap == f64::MAX {
                    -1.0
                } else {
                    incumbent_gap
                },
    ...
    for instance_id in 1..=5 {
        ...
        for log in &solver.logs {
            writeln!(
                ...
                log.child_pressure,
                log.pressure_delta,
                log.parent_objective,
                log.child_objective,
                log.objective_delta,
                log.depth,
                log.survival_depth,
                log.incumbent_gap,
                log.elite_descendant,
                log.operator_outcome,
                log.incumbent_improvement
            )?;
        }
    }
