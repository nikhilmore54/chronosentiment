...
impl AuditTracker {
    ...
    fn emit_failure(&mut self, context: String) {
        ...
        let entry = self.contexts.entry(context).or_insert(ContextStats {
            success: 0, failure: 0, survival_depths: Vec::new()
        });
        entry.failure += 1;
...
fn build_context_tag(
    ...
    let vol_cat = if vol > max_cap * 0.5 { "Vol:High" } else if vol > max_cap * 0.1 { "Vol:Med" } else { "Vol:Low" };
    let len_cat = if path_nodes.len() > 6 { "Len:Long" } else if path_nodes.len() > 3 { "Len:Med" } else { "Len:Short" };
    
    let crosses_core = if path_nodes.iter().any(|&n| n < 10) { "Core:Yes" } else { "Core:No" };
    let cong_cat = if avg_congestion > 0.8 { "Cong:High" } else if avg_congestion > 0.5 { "Cong:Med" } else { "Cong:Low" };
...
fn main() -> anyhow::Result<()> {
    ...
    for (_level, d_mult, c_mult) in &stress_levels {
        ...
        for _ in 0..5000 {
            ...
            for d_idx in 0..total_demands {
                ...
                current_solution.srpaths.push(SrPath { d: d_idx, t: 0, w: w.clone() });
                ...
                let avg_congestion = if *c_mult < 1.0 { 0.9 } else if *c_mult < 1.5 { 0.6 } else { 0.3 };
                let tag = build_context_tag(&graph, &path_nodes, demand.v[0], max_cap, avg_congestion);
    ...
    for tag in &contexts {
        ...
        let mean = if sorted_depths.is_empty() { 0.0 } else {
            sorted_depths.iter().sum::<usize>() as f64 / sorted_depths.len() as f64
        };
        
        let median = if sorted_depths.is_empty() { 0.0 } else {
            sorted_depths[sorted_depths.len() / 2] as f64
        };
        
        let p95 = if sorted_depths.is_empty() { 0.0 } else {
            let idx = (sorted_depths.len() as f64 * 0.95) as usize;
            sorted_depths[idx.min(sorted_depths.len() - 1)] as f64
        };
        
        println!("{:<45} | {:<7} | {:<7} | {:>5.1}%       | {:>10.1} | {:>10.1} | {:>10.1}", 
            tag, stats.success, stats.failure, rate * 100.0, mean, median, p95);
    }
