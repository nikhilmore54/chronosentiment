...
impl<'a> Solver<'a> {
    fn solve_recursive(
        ...
        for (path, pressure) in paths_with_scores {
            if self.node_counter > 30_000 { break; }
            
            // M26.4B Active Pilot Pruning Gate: Prune D9/D10 pressure scores (safe >= 0.97)
            if self.mode == "Coralys" && pressure >= 0.97 {
                continue;
            }
...
fn main() -> anyhow::Result<()> {
    println!("=== M26.4B Active Advisory Run ===");
    ...
    let mut f = File::create("m26_4b_discovery_curves.csv")?;
    ...
    for instance_id in vec![3, 7] {
        ...
        for mode in modes {
            ...
            for log in solver.discovery_logs {
                writeln!(f, "{},{},{},{:.4}", log.instance_id, log.mode, log.nodes_expanded, log.best_obj)?;
            }
        }
    }
    
    println!("Logs written to m26_4b_discovery_curves.csv");
    Ok(())
}
