...
// ---------------------------------------------------------------------------
fn destroy_random(assignments: &HashMap<usize, Vec<u64>>, k: usize, rng: &mut Lcg) -> Vec<usize> {
...
fn destroy_congestion(
    ...
    let congested_nodes: HashSet<u64> = top_k_links
        .iter()
    ...
    if selected.len() < k {
        let remaining: Vec<usize> = assignments
            .keys()
            .copied()
...
fn main() -> anyhow::Result<()> {
    ...
    println!(
        "RP-404A — LNS Framework (destroy={}, k={}, iters={}, seed={})",
    ...
    for instance_id in 1..=20 {
        ...
        for _iter in 0..cfg.iters {
            ...
            let destroyed = match cfg.destroy {
                DestroyOperator::Random => destroy_random(&best_t0, cfg.k, &mut rng),
                DestroyOperator::Congestion => destroy_congestion(
                    &best_t0,
                    &current_srpaths,
                    &evaluator,
                    &link_capacity,
                ...
                DestroyOperator::HighCost => destroy_highcost(
                    &best_t0,
                    &current_srpaths,
                    &evaluator,
                    &link_capacity,
    ...
    println!("{}", "=".repeat(100));
    println!(
        "RP-404A vs RP-403: {} improved, {} regressed, {} unchanged",
    ...
    println!(
        "Solution files written to {}/setA-*-srpaths-{}.json",
        set_dir, out_suffix
    );
