...
fn load_solution(path: &str, nd: usize) -> Vec<SrPath> {
    ...
    let arr = match json["srpaths"].as_array() {
        Some(a) => a.clone(),
        None => return vec![],
    };
...
fn main() -> anyhow::Result<()> {
    ...
    for (i, iv) in sc.interventions.iter().enumerate() {
        ...
        for &lid in &iv.links {
            if let Some(l) = net.links.iter().find(|l| l.id == lid) {
                println!(
                    "  Link {}: {} -> {}, cap={:.2}, metric={:.2}",
                    lid, l.from, l.to, l.capacity, l.metric
                );
            }
        }
    }
    ...
    println!(
        "Min/Mean/Max cap: {:.2} / {:.2} / {:.2}",
        min_cap, mean_cap, max_cap
    );
    ...
    let vols_t1: Vec<f64> = tm
        .demands
        .iter()
        .map(|d| if d.v.len() > 1 { d.v[1] } else { d.v[0] })
        .collect();
    ...
    for (i, d) in tm.demands.iter().enumerate() {
        ...
        if r0.is_none() {
            unreachable_t0 += 1;
            println!(
                "  Demand {}: src={} dst={} UNREACHABLE at t=0 (base graph disconnected)",
                i, d.s, d.t
            );
        }
        if r1.is_none() {
            unreachable_t1 += 1;
            println!(
                "  Demand {}: src={} dst={} UNREACHABLE at t=1 (disconnected after intervention)",
                i, d.s, d.t
            );
        }
    ...
    println!(
        "Demands unreachable at t=1 (intervention applied): {}/{}",
        unreachable_t1, nd
    );
    ...
    println!(
        "Empty solution: obj={:.4}, valid={}",
        empty_r.obj, empty_r.valid
    );
    ...
    if let Some(loads) = ev.compute_loads(0, &empty_sol) {
        ...
        println!(
            "Overloaded links (sat >= 1.0) under default ECMP routing at t=0: {}",
            overloaded_links.len()
        );
        for (lid, flow, cap, util) in &overloaded_links {
            if let Some(l) = net.links.iter().find(|l| l.id == *lid) {
                println!(
                    "  Link {}: {} -> {}, cap={:.2}, flow={:.2}, util={:.4}",
                    lid, l.from, l.to, cap, flow, util
                );
            }
        }
    } else {
    ...
    if let Some(loads) = ev.compute_loads(1, &empty_sol) {
        ...
        overloaded_links.sort_by(|a, b| b.3.partial_cmp(&a.3).unwrap());
        println!(
            "Overloaded links (sat >= 1.0) under default ECMP routing at t=1: {}",
            overloaded_links.len()
        );
        for (lid, flow, cap, util) in &overloaded_links {
            if let Some(l) = net.links.iter().find(|l| l.id == *lid) {
    ...
    if let Some(bl) = bottleneck_link {
        println!(
            "Bottleneck link {}: {} -> {}, cap={:.2}",
            bottleneck_lid, bl.from, bl.to, bl.capacity
        );
        ...
        println!(
            "Demands routing via link 1173 at t=0 (default ECMP): {}/{}",
            demands_via_bottleneck_t0, nd
        );
        println!(
            "Total volume via link 1173 at t=0: {:.2} (cap={:.2}, util={:.4})",
            vol_via_bottleneck_t0,
            bl.capacity,
            vol_via_bottleneck_t0 / bl.capacity
        );
    }
    println!();
    ...
    println!(
        "RP-403 srpaths loaded: {} (of {} demands)",
        rp403_sp.len(),
        nd * ns
    );
    ...
    println!(
        "  Demands with explicit SR path at t=0: {}/{}",
        t0_assigned.len(),
        nd
    );
    println!(
        "  Demands with explicit SR path at t=1: {}/{}",
        t1_assigned.len(),
        nd
    );
    println!(
        "  Demands routed via default ECMP at t=0: {}/{}",
        nd - t0_assigned.len(),
        nd
    );
    println!(
        "  Demands routed via default ECMP at t=1: {}/{}",
        nd - t1_assigned.len(),
        nd
    );
    ...
    if let Some(loads) = ev.compute_loads(0, &rp403_sol) {
        ...
        for (lid, flow, cap, util) in &violations {
            if let Some(l) = net.links.iter().find(|l| l.id == *lid) {
                println!(
                    "  VIOLATION: Link {}: {} -> {}, cap={:.2}, flow={:.2}, overflow={:.4}",
                    lid,
                    l.from,
                    l.to,
                    cap,
    ...
    println!(
        "3. Root cause: Default ECMP routing of unassigned demands overloads link 1173 (12->36)."
    );
