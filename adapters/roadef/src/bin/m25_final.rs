...
fn run_m25_7a() {
    ...
    for i in 0..nodes - 1 {
        net.adj[i].push(i + 1);
        net.capacities.insert(Link(i, i + 1), 100);
    } // Backbone
    ...
    for d in &scenario.demands {
        precomputed_paths.push(baseline.find_all_paths(&scenario.network, d.src, d.dst));
    }

    baseline.solve(&scenario, 0, &HashMap::new(), 0, &precomputed_paths);
    println!(
        "Baseline | Best@Node: {} | Total Nodes: {}",
    ...
    println!(
        "{:<6} | {:<12} | {:<10} | {}",
        "Run", "Nodes Visited", "Best@Node", "Hard FP"
    );
    for run in 1..=10 {
        ...
        println!(
            "{:<6} | {:<12} | {:<10} | {}",
            run, coralys.nodes_visited, coralys.best_solution_visited, hard_fp
        );
    }
    println!();
}
...
fn run_m25_7b() {
    ...
    for _ in 0..98 {
        state.emit_causal_edge("Route15".to_string(), false);
    }
    for _ in 0..2 {
        state.emit_causal_edge("Route15".to_string(), true);
    }

    // Simulate 15% failure route
    for _ in 0..15 {
        state.emit_causal_edge("Route17".to_string(), false);
    }
    for _ in 0..85 {
        state.emit_causal_edge("Route17".to_string(), true);
    }
    ...
    println!(
        "Route15 Expected Risk: {:.2} | Observed: 0.98",
        r15.pressure
    );
    println!(
        "Route17 Expected Risk: {:.2} | Observed: 0.15",
        r17.pressure
    );
...
fn run_m25_7c() {
    ...
    println!(
        "Transition Latency <= 2 epochs? {}",
        l22.pressure > l15.pressure
    );
    println!();
}
...
fn run_m25_7e() {
    ...
    println!(
        "Context separated? {}",
        l15_m.pressure > 0.9 && l15_u.pressure < 0.1
    );
    println!();
}
