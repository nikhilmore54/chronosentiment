...
fn pad_random_seen(
    ...
    for i in 0..actual {
        let j = i + rng.next_usize(rem.len() - i);
        rem.swap(i, j);
    }
...
fn destroy_congestion(
    ...
    for l in &net.links {
        let s = sat.get(&l.id).copied().unwrap_or(0.0);
        let e = node_sat.entry(l.from).or_insert(0.0);
        if s > *e {
            *e = s;
        }
        let e2 = node_sat.entry(l.to).or_insert(0.0);
        if s > *e2 {
            *e2 = s;
        }
    }
    ...
    for sp in srpaths {
        if sp.t != 0 || !asgn.contains_key(&sp.d) {
            continue;
        }
        let score =
            sp.w.iter()
                .map(|n| node_sat.get(n).copied().unwrap_or(0.0))
...
fn destroy_bottleneck_link(
    ...
    let sat = compute_sat(ev, srpaths, 0, cap);
    let mut links_sorted: Vec<(u64, u64, f64)> = net
        .links
        .iter()
    ...
    for (d, s, t, _) in demands {
        src_dst.insert(*d, (*s, *t));
    }
    let mut sel: Vec<usize> = Vec::new();
    let mut seen: HashSet<usize> = HashSet::new();
    'outer: for (from, to, _) in &links_sorted {
        for sp in srpaths {
            if sp.t != 0 || !asgn.contains_key(&sp.d) || seen.contains(&sp.d) {
                continue;
            }
...
fn destroy_ecmp_conflict(
    ...
    for l in &net.links {
        edge_to_lid.insert((l.from, l.to), l.id);
    }
    let mut src_dst: HashMap<usize, (u64, u64)> = HashMap::new();
    for (d, s, t, _) in demands {
        src_dst.insert(*d, (*s, *t));
    ...
    if dls.is_empty() {
        return destroy_random(asgn, k, rng);
    }
    let mut load_scores: Vec<(usize, f64)> = dls
        .iter()
        .map(|(&d, edges)| {
            let s: f64 = edges
                .iter()
                .map(|e| {
                    let lid = edge_to_lid.get(e).copied().unwrap_or(0);
                    sat.get(&lid).copied().unwrap_or(0.0)
                })
                .sum();
            (d, s)
        })
    ...
    let pivot_edges = match dls.get(&pivot) {
        Some(e) => e.clone(),
        None => return destroy_random(asgn, k, rng),
    };
    let mut conflict: Vec<(usize, f64)> = dls
        .iter()
        .filter(|(&d, _)| d != pivot)
        .map(|(&d, edges)| {
            let s: f64 = edges
...
fn repair_rp401c(
    ...
    let mut partial: Vec<SrPath> = asgn
        .iter()
    ...
    for l in &net.links {
        sat.entry(l.id).or_insert(0.0);
    }
    let mut to_repair: Vec<(usize, u64, u64, f64)> = demands
        .iter()
        .filter(|(d, _, _, _)| dset.contains(d))
        .copied()
        .collect();
...
fn main() -> anyhow::Result<()> {
    ...
    println!(
        "RP-405 -- Adaptive Operator Selection (k={}, iters={}, seed={})",
    ...
    println!(
        "{:<10} {:>12} {:>12} {:>10} {:>8} {:>8} {:>30}",
        "Instance", "LNS obj", "RP-403 obj", "delta", "improved", "ms", "final_weights"
    );
    println!("{}", "-".repeat(110));
    ...
    for instance_id in 1..=20 {
        ...
        for iter_idx in 0..cfg.iters {
            ...
            if iter_idx > 0 && iter_idx % DECAY_WINDOW == 0 {
                for w in weights.iter_mut() {
                    *w = (*w * DECAY_FACTOR).max(MIN_WEIGHT);
                }
            }
            ...
            let destroyed = match op_idx {
                ...
                1 => {
                    destroy_congestion(&best_t0, &current_srpaths, &ev, &cap, &net, cfg.k, &mut rng)
                }
                2 => destroy_highcost(&best_t0, &current_srpaths, &demands, cfg.k, &mut rng),
                3 => destroy_bottleneck_link(
                    &best_t0,
                    &current_srpaths,
                    &ev,
                    &cap,
            ...
            let repaired_t0 = repair_rp401c(
                &net, &ev, &best_t0, &destroyed, &demands, &dis_both, max_seg, &cap, deadline,
            );
        ...
        let result = ev.evaluate_solution(&Solution {
    ...
    println!("{}", "=".repeat(110));
    println!(
        "RP-405 vs RP-403: {} improved, {} regressed, {} unchanged",
    ...
    println!(
        "Solution files written to {}/setA-*-srpaths-rp405-adaptive.json",
        set_dir
    );
    Ok(())
}
