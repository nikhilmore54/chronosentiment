...
fn main() -> anyhow::Result<()> {
    ...
    for (level, d_mult, c_mult) in &stress_levels {
        ...
        for _ in 0..10000 {
            ...
            current_solution.srpaths.push(SrPath { d: target_d_idx, t: 0, w: w.clone() });
            ...
            let tag = build_context_tag(&graph, &path_nodes, demand.v[0], max_cap, 0, false, avg_congestion);
    ...
    for tag in &contexts {
        ...
        let total = s + f;
        if total < 50 { continue; } // Filter out noise
