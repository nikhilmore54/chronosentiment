...
impl<'a> Solver<'a> {
    fn solve_recursive(
    ...
    ) -> (bool, f64) { // Returns (subtree_survived, best_subtree_obj)
        ...
        for path in paths {
            if self.node_counter > 50_000 { break; }
            ...
            let context = build_context_tag(self.graph, &path_nodes, demand.v[0], self.max_cap, current_avg_congestion);
            ...
            if res.valid {
                ...
                let (sub_success, sub_obj) = self.solve_recursive(demand_idx + 1, current_solution, next_cong);
...
fn find_k_shortest(graph: &Digraph, src: u64, dst: u64, k: usize) -> Vec<Vec<u64>> {
    ...
    while let Some(path) = queue.pop_front() {
        explored_count += 1;
        if explored_count > 10000 { break; } // prevent exponential blowup on dense graphs
        ...
        if path.len() > 6 { continue; } // max depth
...
fn main() -> anyhow::Result<()> {
    ...
    for instance_id in 1..=7 {
        ...
        for log in &solver.logs {
            let obj_str = if log.best_descendant_objective == f64::MAX { -1.0 } else { log.best_descendant_objective };
            writeln!(f, "{},{},{},{},{:.4},{},{:.4},{}", 
                instance_id, log.node_id, log.depth, log.context, log.pressure_score, 
                log.subtree_survived, obj_str, log.became_champion)?;
        }
    }
