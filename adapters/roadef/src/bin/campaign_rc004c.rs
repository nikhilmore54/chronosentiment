...
use roadef::moga_impl::{
    RoadefGenomeFactory, RoadefFitnessEvaluator,
    ConstructionMode, GreedyConstructorData,
};
use roadef::loader::{load_network, load_traffic_matrix, load_scenario};
use coralys_moga::traits::evaluator::{GenomeFactory, FitnessEvaluator, Evaluated};
...
fn pairwise_edge_overlap_rate(hashes: &[u64]) -> f64 {
    ...
    for &h in hashes { *counts.entry(h).or_insert(0) += 1; }
...
fn main() {
    ...
    for instance_name in TARGET_INSTANCES {
        ...
        for id in 0..N_INDIVIDUALS {
            ...
            records.push(IndividualRecord {
                id,
                valid: ev.is_valid(),
                surrogate_obj: if ev.is_valid() { ev.fitness() } else { f64::INFINITY },
                edge_hash,
                route_hash,
                construct_ms,
            });
            ...
            if id % 10 == 0 {
                eprintln!("  [{:3}/{}] valid={} construct_ms={}", id+1, N_INDIVIDUALS, ev.is_valid(), construct_ms);
            }
        }
        ...
        let valid_objs: Vec<f64> = records.iter()
        ...
        let (obj_mean, obj_std, obj_min, obj_max) = if valid_objs.is_empty() {
            ...
            let variance = valid_objs.iter().map(|x| (x - mean).powi(2)).sum::<f64>() / valid_objs.len() as f64;
        ...
        eprintln!("  Valid:          {}/{} ({:.1}%)", n_valid, N_INDIVIDUALS, validity_rate * 100.0);
        eprintln!("  Unique edges:   {}/{} ({:.1}%)", n_unique_edge, N_INDIVIDUALS, unique_edge_pct * 100.0);
        eprintln!("  Unique routes:  {}/{} ({:.1}%)", n_unique_route, N_INDIVIDUALS, unique_route_pct * 100.0);
        eprintln!("  Overlap:        {:.1}%", mean_pairwise_overlap * 100.0);
        eprintln!("  Obj mean/std:   {:.4} / {:.4}", obj_mean, obj_std);
        eprintln!("  Construct ms:   mean={:.0} total={}ms", mean_construct_ms, total_construct_ms);
    ...
    let overall_verdict = if instance_results.iter().all(|r| r.verdict == "PASS") {
        ...
        "FAIL — constructor diversity or validity below threshold; investigate post-submission".to_string()
    } else {
        "INSUFFICIENT_DATA".to_string()
    };
