...
use roadef::loader::{load_network, load_scenario, load_solution, load_traffic_matrix};
...
fn test_audit_intervention_edge() {
    ...
    let evaluator = RoadefEvaluator::new(&net, tm, scenario);
    let loads = evaluator
        .compute_loads(1, &solution)
        .expect("Should still be connected due to fallback");
    ...
    assert!(
        (loads.arc_flows[&0] - 100.0).abs() < 1e-6,
        "0->1 should have 100 flow"
    );
    assert!(
        (loads.arc_flows[&1] - 100.0).abs() < 1e-6,
        "1->0 should have 100 flow"
    );
    assert!(
        (loads.arc_flows[&2] - 100.0).abs() < 1e-6,
        "0->2 should have 100 flow"
    );
    assert!(
        (loads.arc_flows[&6] - 100.0).abs() < 1e-6,
        "2->3 should have 100 flow"
    );

    // 1->3 should have 0 flow
    assert!(
        (loads.arc_flows.get(&4).unwrap_or(&0.0) - 0.0).abs() < 1e-6,
        "1->3 should have 0 flow"
    );
...
fn test_audit_ecmp_tie() {
    ...
    let evaluator = RoadefEvaluator::new(&net, tm, scenario);
    let loads = evaluator
        .compute_loads(0, &solution)
        .expect("ECMP Tie should be connected");
