use cvrp::{CvrpInstance, CvrpCandidate, CvrpGenomeFactory, DistanceMetric};
...
fn main() {
    ...
    let bks_float = instance_for_bks.evaluate_routes_distance(&bks_routes, DistanceMetric::EuclideanFloat);
    let bks_integer = instance_for_bks.evaluate_routes_distance(&bks_routes, DistanceMetric::TspLibEuc2D);
    ...
    let eval_float = CvrpEvaluator { instance: instance_float.clone() };
    let mutator_float = CvrpMutator::new(instance_float.clone(), cvrp::RadiusPolicy::Control);
    let crossover_float = CvrpCrossover;
    let factory_float = CvrpGenomeFactory { num_customers: instance_float.customers.len() };
    let ls_float = CvrpLocalSearch { instance: instance_float.clone() };
    ...
    let eval_int = CvrpEvaluator { instance: instance_int.clone() };
    let mutator_int = CvrpMutator::new(instance_int.clone(), cvrp::RadiusPolicy::Control);
    let crossover_int = CvrpCrossover;
    let factory_int = CvrpGenomeFactory { num_customers: instance_int.customers.len() };
    let ls_int = CvrpLocalSearch { instance: instance_int.clone() };
    ...
    println!("  Depot (82, 76) -> Cust 2 (96, 44): Float={:.4}, Int={:.4}", f_dist, i_dist);
    ...
    println!("  Cust 2 (96, 44) -> Cust 13 (98, 52): Float={:.4}, Int={:.4}", f_dist2, i_dist2);

    println!("\nRoute totals for Official BKS:");
    for (i, route) in bks_routes.iter().enumerate() {
        let mut sub_routes = vec![route.clone()];
        let r_float = instance_for_bks.evaluate_routes_distance(&sub_routes, DistanceMetric::EuclideanFloat);
        let r_int = instance_for_bks.evaluate_routes_distance(&sub_routes, DistanceMetric::TspLibEuc2D);
        println!("  Route #{}: Nodes={:?}, Float={:.4}, Int={:.4}", i+1, route, r_float, r_int);
    }

    println!("\nValidation Table:");
    println!("| Solution | Official Integer (Benchmark) | Floating Euclidean (Research) | Difference | % Diff |");
    println!("| :--- | :---: | :---: | :---: | :---: |");
    
    let diff_bks = bks_float - bks_integer;
    let pct_bks = (diff_bks / bks_integer) * 100.0;
    println!("| Official BKS | {:.2} | {:.4} | {:.4} | {:.2}% |", bks_integer, bks_float, diff_bks, pct_bks);

    let diff_coralys_f = float_best_as_float - float_best_as_integer;
    let pct_coralys_f = (diff_coralys_f / float_best_as_integer) * 100.0;
    println!("| Coralys Best (Float mode) | {:.2} | {:.4} | {:.4} | {:.2}% |", float_best_as_integer, float_best_as_float, diff_coralys_f, pct_coralys_f);

    let diff_coralys_i = int_best_as_float - int_best_as_integer;
    let pct_coralys_i = (diff_coralys_i / int_best_as_integer) * 100.0;
    println!("| Coralys Best (Integer mode) | {:.2} | {:.4} | {:.4} | {:.2}% |", int_best_as_integer, int_best_as_float, diff_coralys_i, pct_coralys_i);
