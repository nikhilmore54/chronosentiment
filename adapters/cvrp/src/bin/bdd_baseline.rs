...
fn main() {
    ...
    println!(
        "Found {} Augerat instances to process.",
        augerat_instances.len()
    );
                            ...
                            Ok(res) => {
                                ...
                                for route in &res.global_best.eval.routes {
                                    ...
                                    for &node_id in route {
                                        ...
                                        if let Some(cust) =
                                            instance.customers.iter().find(|c| c.id == node_id)
                                        {
                                            load += cust.demand;
                                ...
                                let customer_coverage_ok = visited.len()
                                    == instance.customers.len()
                                    && total_visited == instance.customers.len();
                                let vehicle_count_ok =
                                    res.global_best.eval.routes.len() <= task.metadata.vehicles;

                                let status = if !customer_coverage_ok
                                    || !capacity_ok
                                    || !vehicle_count_ok
                                    || !is_feasible
                                {
    ...
    for _ in 0..num_tasks {
        ...
        println!(
            ...
            results.len(),
            num_tasks,
            results.last().unwrap().metadata.name
        );
    }
    ...
    let avg_gap = if successful_runs > 0 {
        total_gap / (successful_runs as f64)
    } else {
        0.0
    };
    ...
    let worst_gap = if !gaps.is_empty() {
        gaps[gaps.len() - 1]
    } else {
        0.0
    };
