...
impl AdvisoryCandidate for EvaluatedOffspring {
    ...
    fn fallback_cmp(&self, other: &Self) -> Ordering {
        self.eval.total_distance.partial_cmp(&other.eval.total_distance).unwrap_or(Ordering::Equal)
    }
}
...
fn run_pilot(use_advisory: bool, seed: u64) -> RunMetrics {
    ...
    for generation in 0..generations {
        ...
        if use_advisory && generation >= 100 {
            let default_ctx = CvrpContextKey { route_count:0, longest_route:0, route_balance:0};
            AdvisoryRanker::sort(&mut evals, &memory, &default_ctx);
        } else {
            evals.sort_by(|a, b| a.eval.total_distance.partial_cmp(&b.eval.total_distance).unwrap_or(Ordering::Equal));
        }
...
fn main() {
    ...
    for (i, &seed) in seeds.iter().enumerate() {
        ...
        println!("  Control: {:.2}, Treatment: {:.2} (Delta: {:.2})", control.best_objective, treatment.best_objective, delta);
    }
    
    let c_mean = control_objs.iter().sum::<f64>() / seeds.len() as f64;
    let t_mean = treatment_objs.iter().sum::<f64>() / seeds.len() as f64;
    
    let c_var = control_objs.iter().map(|v| (v - c_mean).powi(2)).sum::<f64>() / seeds.len() as f64;
    let t_var = treatment_objs.iter().map(|v| (v - t_mean).powi(2)).sum::<f64>() / seeds.len() as f64;
    ...
    println!("Mean Delta:     {:.2} (Positive is Improvement)", c_mean - t_mean);
    println!("Win Rate:       {}/{} ({:.1}%)", wins, seeds.len(), (wins as f64 / seeds.len() as f64) * 100.0);
