...
impl OpportunityMemory {
    ...
    fn record(&mut self, key: CvrpContextKey, is_champ: bool) {
        let stats = self.map.entry(key).or_insert(MemoryStats { obs: 0, champs: 0 });
        stats.obs += 1;
        if is_champ {
            stats.champs += 1;
        }
    ...
    for generation in 0..generations {
        ...
        for (rank, e) in evals.iter().enumerate() {
            ...
            if let Some(ctx) = &e.parent_ctx {
                ...
                let ctx_str = format!("{}-{}-{}", ctx.route_count, ctx.longest_route, ctx.route_balance);
                if generation >= 100 {
                    writeln!(out_csv, "{},{},{},{},{},{}", seed, generation, ctx_str, score, is_champ, survived).unwrap();
                }
...
fn main() {
    ...
    writeln!(out_csv, "seed,generation,parent_context,parent_score,became_champion,survived").unwrap();
