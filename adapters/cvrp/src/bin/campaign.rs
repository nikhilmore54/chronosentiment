...
use cvrplib_registry::{benchmark_metadata, BenchmarkFamily};
...
use cvrp::{CvrpInstance, CvrpGenomeFactory, Node, DistanceMetric};
use cvrp::moga_impl::{CvrpMutator, CvrpCrossoverRoutePreserving, CvrpLocalSearch, CvrpEvaluator};
use cvrp::qualification::feasibility::{run_pre_optimization_fcf_with_fc3, BenchmarkMeta, FeasibilityStatus};
...
fn parse_vrp(content: &str) -> Result<(CvrpInstance, String, Option<f64>, VehicleSource), String> {
    ...
    for line in content.lines() {
        ...
        if line.starts_with("NAME") {
            ...
            for part in name.split('-') {
                if part.starts_with('k') && part.len() > 1 && part[1..].chars().all(|c| c.is_ascii_digit()) {
                    if let Ok(v) = part[1..].parse::<usize>() {
                        if v > 0 { n_vehicles_name = Some(v); }
                    }
                }
            }
        } else if line.starts_with("VEHICLES") {
            // Strategy 1: explicit VEHICLES field
            if let Ok(v) = line.split(':').nth(1).unwrap_or("0").trim().parse::<usize>() {
                if v > 0 { n_vehicles_explicit = Some(v); }
            }
            ...
            if let Some(pos) = comment.to_lowercase().find("optimal value") {
                ...
                let val_str: String = after.chars()
                    .skip_while(|c| !c.is_ascii_digit())
                    .take_while(|c| c.is_ascii_digit() || *c == '.')
                    .collect();
                if let Ok(v) = val_str.parse::<f64>() { bks_from_file = Some(v); }
                ...
                if trimmed.chars().all(|c| c.is_ascii_digit() || c == '.' || c == '-') {
                    if let Ok(v) = trimmed.parse::<f64>() { bks_from_file = Some(v); }
                }
            }
            ...
            if let Some(pos) = comment.to_lowercase().find("no of trucks") {
                ...
                if let Ok(v) = val_str.parse::<usize>() {
                    if v > 0 { n_vehicles_comment = Some(v); }
                }
            }
        } else if line.starts_with("DIMENSION") {
            dimension = line.split(':').nth(1).unwrap_or("0").trim().parse().unwrap_or(0);
    ...
    if !is_explicit && edge_weight_type != "EUC_2D" {
        return Err(format!("Unsupported EDGE_WEIGHT_TYPE: {}", edge_weight_type));
    }
    if is_explicit && edge_weight_format.is_empty() {
        return Err(format!("EXPLICIT instance '{}' missing EDGE_WEIGHT_FORMAT", name));
    }
    if dimension == 0 { return Err("Missing DIMENSION".to_string()); }
    if capacity == 0  { return Err("Missing CAPACITY".to_string()); }
    ...
    let depot = Node { id: depot_id, x: depot_coord.0, y: depot_coord.1, demand: 0 };
...
fn extract_family(name: &str) -> String {
    ...
    if let Some(pos) = name.find('-') { return name[..pos].to_string(); }
    if let Some(pos) = name.find('_') { return name[..pos].to_string(); }
    let alpha: String = name.chars().take_while(|c| c.is_alphabetic()).collect();
    if !alpha.is_empty() { alpha } else { name.to_string() }
}

fn classify_result(best: f64, bks: Option<f64>) -> QualityClass {
    if best >= 99000.0 { return QualityClass::Invalid; }
    match bks {
        None => QualityClass::NoRef,
        Some(b) if b <= 0.0 => QualityClass::NoRef,
        Some(b) => {
            let gap = (best - b) / b * 100.0;
            if gap.abs() < 0.01      { QualityClass::Solved }
            else if gap < 1.0        { QualityClass::NearOptimal }
            else if gap < 5.0        { QualityClass::Competitive }
            else if gap < 20.0       { QualityClass::Weak }
...
fn run_instance(instance: CvrpInstance) -> Result<RunResult, String> {
    ...
    let ga_res = engine.run_ga_evolution(config)
        .map_err(|e| format!("Evolution error: {}", e))?;
    let runtime = start.elapsed().as_millis();

    let m = engine.metrics_snapshot().ok_or_else(|| "Metrics snapshot unavailable".to_string())?;
    ...
    let final_distances: Vec<f64> = ga_res.final_fitnesses.iter().map(|&f| 100000.0 - f).collect();

    let (proc0_inv, proc0_avg, proc0_total, proc0_min, proc0_max) = if let Some(pm) = m.processors.get(&0) {
        let total_ms = pm.total_runtime.as_secs_f64() * 1000.0;
        let avg_ms = if pm.invocation_count > 0 { total_ms / pm.invocation_count as f64 } else { 0.0 };
...
fn main() {
    ...
    println!("Config: pop={}, elite={}, gen={}, seed={}, metric=TspLibEuc2D",
    ...
    for (instance_id, path) in &vrp_files {
        ...
        let quick_dimension: usize = content.lines()
    ...
    println!("Will skip:       {} (>{} customers)", will_skip, MAX_CUSTOMERS);
    ...
    for (idx, (instance_id, path)) in vrp_files.iter().enumerate() {
        let content = match fs::read_to_string(path) {
            Ok(c) => c,
            Err(e) => {
                eprintln!("[{}/{}] ERROR reading {:?}: {}", idx+1, vrp_files.len(), path, e);
                results.push(make_error_result(*instance_id, format!("instance_{}", instance_id), "unknown"));
                continue;
            }
        };
        ...
        let (instance, name, bks, vehicle_source) = match parse_vrp(&content) {
            ...
            Err(e) => {
                ...
                eprintln!("[{}/{}] UNSUPPORTED {}: {}", idx+1, vrp_files.len(), instance_id, e);
                results.push(InstanceResult {
                    instance_id: *instance_id,
                    name: format!("instance_{}", instance_id),
                    family,
                    customers: 0, vehicles: 0, vehicle_source: "N/A".to_string(),
                    capacity: 0, bks: None, best_distance: 0.0, average_distance: 0.0,
                    worst_distance: 0.0, median_distance: 0.0, std_dev: 0.0,
                    gap_pct: 0.0, quality_class: QualityClass::Invalid.to_string(),
                    feasible: false, runtime_ms: 0, generations: 0, stagnation_generation: 0,
                    termination_reason: "unsupported".to_string(),
                    status: "unsupported".to_string(), skip_reason: Some(e),
                    pct_at_bks: 0.0, pct_within_1pct: 0.0, pct_within_2pct: 0.0,
                    pct_within_5pct: 0.0, pct_within_10pct: 0.0,
                    proc0_invocations: 0, proc0_avg_ms: 0.0, proc0_total_ms: 0.0,
                    vehicles_used: 0, evaluation_count: 0,
                    proc0_min_ms: 0.0, proc0_max_ms: 0.0,
                    convergence_generation: 0,
                    best_distance_integer: 0.0, best_distance_float: 0.0,
                    distance_metric: "N/A".to_string(),
                });
                continue;
            }
        ...
        if n_customers > MAX_CUSTOMERS {
            eprintln!("[{}/{}] SKIPPED {} ({} customers > {})", idx+1, vrp_files.len(), name, n_customers, MAX_CUSTOMERS);
            results.push(InstanceResult {
                instance_id: *instance_id, name: name.clone(), family,
                customers: n_customers, vehicles: n_vehicles,
                vehicle_source: vehicle_source.to_string(),
                capacity: cap, bks, best_distance: 0.0, average_distance: 0.0,
                worst_distance: 0.0, median_distance: 0.0, std_dev: 0.0,
                gap_pct: 0.0, quality_class: "skipped".to_string(),
                feasible: false, runtime_ms: 0, generations: 0, stagnation_generation: 0,
                termination_reason: "skipped".to_string(),
                status: "skipped".to_string(),
                skip_reason: Some(format!("{} customers > {} limit", n_customers, MAX_CUSTOMERS)),
                pct_at_bks: 0.0, pct_within_1pct: 0.0, pct_within_2pct: 0.0,
                pct_within_5pct: 0.0, pct_within_10pct: 0.0,
                proc0_invocations: 0, proc0_avg_ms: 0.0, proc0_total_ms: 0.0,
                vehicles_used: 0, evaluation_count: 0,
        ...
        let bks_str = bks.map(|b| format!("bks={:.2}", b)).unwrap_or_else(|| "bks=?".to_string());
        eprintln!("[{}/{}] Running {} ({} customers, {} vehicles, {}, {})",
        ...
        match run_result {
            ...
            Ok(r) => {
                ...
                let (pct_at_bks, pct_w1, pct_w2, pct_w5, pct_w10) = population_quality(&r.final_distances, bks.unwrap_or(0.0));
                ...
                let (opt_fuc, fcs) = if feasible {
                    let gap_str = if bks_is_fractional {
                        bks.map(|_b| format!(" gap={:+.2}% gap_fp={:+.2}%", gap_pct, gap_fp_pct)).unwrap_or_default()
                    } else {
                        bks.map(|_b| format!(" gap={:+.2}%", gap_pct)).unwrap_or_default()
                    };
                    ...
                    eprintln!("  → best={:.0}{}{}{}  [{}]  {}ms", r.best, fp_str, routes_flag, gap_str, quality_class, r.runtime_ms);
                ...
                // ── Execution Certificate (M18.3) ────────────────────────────────
                {
                    ...
                    let gap_opt = if bks.is_some() && feasible { Some(gap_pct) } else { None };
                    let gap_fp_opt = if bks_is_fractional && feasible { Some(gap_fp_pct) } else { None };
                ...
                results.push(InstanceResult {
                    ...
                    termination_reason: if r.stagnation < r.generations { "stagnation".to_string() } else { "generation_limit".to_string() },
                    status: if feasible { "ok".to_string() } else { "infeasible".to_string() },
                    skip_reason: None,
                    pct_at_bks,
    ...
    // ── Compute summary ──────────────────────────────────────────────────────
    let ran: Vec<&InstanceResult> = results.iter().filter(|r| r.status == "ok" || r.status == "infeasible").collect();
    let feasible_ran: Vec<&InstanceResult> = ran.iter().filter(|r| r.feasible).copied().collect();
    let with_bks: Vec<&InstanceResult> = feasible_ran.iter().filter(|r| r.bks.is_some()).copied().collect();

    let feasibility_rate = if ran.is_empty() { 0.0 } else { feasible_ran.len() as f64 / ran.len() as f64 * 100.0 };
    let avg_gap = if with_bks.is_empty() { 0.0 } else { with_bks.iter().map(|r| r.gap_pct).sum::<f64>() / with_bks.len() as f64 };
    let mut gaps: Vec<f64> = with_bks.iter().map(|r| r.gap_pct).collect();
    gaps.sort_by(|a,b| a.partial_cmp(b).unwrap());
    let median_gap = if gaps.is_empty() { 0.0 } else { gaps[gaps.len()/2] };
    let bks_matches = with_bks.iter().filter(|r| r.gap_pct.abs() < 0.01).count();

    let runtimes: Vec<u128> = ran.iter().map(|r| r.runtime_ms).collect();
    let avg_runtime = if runtimes.is_empty() { 0.0 } else { runtimes.iter().sum::<u128>() as f64 / runtimes.len() as f64 };
    let mut rt_sorted = runtimes.clone(); rt_sorted.sort();
    let median_runtime = if rt_sorted.is_empty() { 0 } else { rt_sorted[rt_sorted.len()/2] };
    let max_runtime = rt_sorted.last().copied().unwrap_or(0);

    let n_vehicles_field = results.iter().filter(|r| r.vehicle_source == "VEHICLES_FIELD").count();
    let n_comment = results.iter().filter(|r| r.vehicle_source == "COMMENT").count();
    let n_name = results.iter().filter(|r| r.vehicle_source == "NAME").count();
    let n_registry = results.iter().filter(|r| r.vehicle_source == "REGISTRY").count();
    ...
    md.push_str(&format!("**Instances:** {} total, {} ran, {} skipped, {} unsupported  \n", summary.total_instances, summary.supported_instances, summary.skipped_instances, summary.unsupported_instances));
    ...
    md.push_str(&format!("**Median gap (primary):** {:.2}%  |  **Avg gap:** {:.2}%  \n", summary.median_gap_pct, summary.avg_gap_pct));
    md.push_str(&format!("**Avg runtime:** {:.0}ms  |  **Median:** {}ms  |  **Max:** {}ms  \n\n", summary.avg_runtime_ms, summary.median_runtime_ms, summary.max_runtime_ms));

    // ── Executive Summary ─────────────────────────────────────────────────────
    md.push_str("## Executive Summary\n\n");
    // Compute ≤50 customer stats for the headline
    let le50_ran: Vec<&InstanceResult> = feasible_ran.iter().filter(|r| r.customers <= 50 && r.bks.is_some()).copied().collect();
    let le50_solved_or_near = le50_ran.iter().filter(|r| {
        r.gap_pct.abs() < 0.01 || (r.gap_pct >= 0.0 && r.gap_pct < 1.0)
    }).count();
    let le50_pct = if le50_ran.is_empty() { 0.0 } else { le50_solved_or_near as f64 / le50_ran.len() as f64 * 100.0 };
    // Compute ≤100 customer median gap
    let le100_gaps: Vec<f64> = feasible_ran.iter()
        .filter(|r| r.customers <= 100 && r.bks.is_some())
        .map(|r| r.gap_pct).collect();
    let mut le100_gaps_sorted = le100_gaps.clone(); le100_gaps_sorted.sort_by(|a,b| a.partial_cmp(b).unwrap());
    let le100_med_gap = if le100_gaps_sorted.is_empty() { 0.0 } else { le100_gaps_sorted[le100_gaps_sorted.len()/2] };
    md.push_str(&format!(
    ...
    md.push_str(&format!("| Under Investigation | {} |\n", qual_investigation));
    md.push_str(&format!("| Excluded (>200 customers) | {} |\n", qual_excluded));
    md.push_str(&format!("| Unsupported | {} |\n\n", qual_unsupported));

    // Qualification confidence: % of ran instances that are Verified or PartiallyVerified
    let ran_qual_ok = ran.iter().filter(|r| {
        let qm = cvrplib_registry::qualification_metadata(&r.name);
    ...
    let qual_confidence = if ran.is_empty() { 0.0 } else { ran_qual_ok as f64 / ran.len() as f64 * 100.0 };
    ...
    md.push_str("| Size | N | Solved | NearOpt | Compet | Weak | Poor | **MedGap** | AvgGap | AvgMs |\n");
    md.push_str("|------|---|--------|---------|--------|------|------|-----------|--------|-------|\n");
    {
        ...
        for (label, lo, hi) in size_buckets {
            let bucket_inst: Vec<&InstanceResult> = feasible_ran.iter()
                .filter(|r| r.customers >= *lo && r.customers <= *hi && r.bks.is_some())
                .copied().collect();
            if bucket_inst.is_empty() { continue; }
            ...
            let n_solved  = bucket_inst.iter().filter(|r| r.gap_pct.abs() < 0.01).count();
            let n_near    = bucket_inst.iter().filter(|r| r.gap_pct >= 0.01 && r.gap_pct < 1.0).count();
            let n_compet  = bucket_inst.iter().filter(|r| r.gap_pct >= 1.0  && r.gap_pct < 5.0).count();
            let n_weak    = bucket_inst.iter().filter(|r| r.gap_pct >= 5.0  && r.gap_pct < 20.0).count();
            let n_poor    = bucket_inst.iter().filter(|r| r.gap_pct >= 20.0).count();
            let fmt_cell = |cnt: usize, sum_gap: f64| -> String {
                if cnt == 0 { "—".to_string() }
                else if sum_gap < 0.001 { format!("{}(0.0%)", cnt) }
                else { format!("{}({:.1}%)", cnt, sum_gap / cnt as f64) }
            };
            let solved_gap: f64 = bucket_inst.iter().filter(|r| r.gap_pct.abs() < 0.01).map(|r| r.gap_pct).sum();
            let near_gap:   f64 = bucket_inst.iter().filter(|r| r.gap_pct >= 0.01 && r.gap_pct < 1.0).map(|r| r.gap_pct).sum();
            let compet_gap: f64 = bucket_inst.iter().filter(|r| r.gap_pct >= 1.0  && r.gap_pct < 5.0).map(|r| r.gap_pct).sum();
            let weak_gap:   f64 = bucket_inst.iter().filter(|r| r.gap_pct >= 5.0  && r.gap_pct < 20.0).map(|r| r.gap_pct).sum();
            let poor_gap:   f64 = bucket_inst.iter().filter(|r| r.gap_pct >= 20.0).map(|r| r.gap_pct).sum();
            md.push_str(&format!("| {} | {} | {} | {} | {} | {} | {} | **{:.2}%** | {:.2}% | {}ms |\n",
                label, n,
    ...
    md.push_str("|--------|---|------|--------|---------|--------|------|------|-----------|--------|-------|\n");
    {
        ...
        for fam_key in &fam_order {
            let finsts: Vec<&InstanceResult> = feasible_ran.iter()
            ...
            let n_near    = finsts.iter().filter(|r| r.gap_pct >= 0.01 && r.gap_pct < 1.0).count();
            let n_compet  = finsts.iter().filter(|r| r.gap_pct >= 1.0  && r.gap_pct < 5.0).count();
            let n_weak    = finsts.iter().filter(|r| r.gap_pct >= 5.0  && r.gap_pct < 20.0).count();
            ...
            let solved_gap: f64 = finsts.iter().filter(|r| r.gap_pct.abs() < 0.01).map(|r| r.gap_pct).sum();
            let near_gap:   f64 = finsts.iter().filter(|r| r.gap_pct >= 0.01 && r.gap_pct < 1.0).map(|r| r.gap_pct).sum();
            let compet_gap: f64 = finsts.iter().filter(|r| r.gap_pct >= 1.0  && r.gap_pct < 5.0).map(|r| r.gap_pct).sum();
            let weak_gap:   f64 = finsts.iter().filter(|r| r.gap_pct >= 5.0  && r.gap_pct < 20.0).map(|r| r.gap_pct).sum();
            let poor_gap:   f64 = finsts.iter().filter(|r| r.gap_pct >= 20.0).map(|r| r.gap_pct).sum();
            md.push_str(&format!("| {} | {} | {} | {} | {} | {} | {} | {} | **{:.2}%** | {:.2}% | {}ms |\n",
                fam_key, n, avg_c,
                fmt_cell(n_solved, solved_gap),
                fmt_cell(n_near,   near_gap),
    ...
    md.push_str("Standard qualification table showing the complete solution quality distribution.\n\n");
    md.push_str("| Gap Threshold | Instances | Percentage |\n|---------------|----------:|-----------:|\n");
    ...
    if feasible_with_bks_n > 0 {
        ...
        let le_half   = with_bks.iter().filter(|r| r.gap_pct >= 0.0 && r.gap_pct <= 0.5).count();
        let le_1      = with_bks.iter().filter(|r| r.gap_pct >= 0.0 && r.gap_pct <= 1.0).count();
        let le_2      = with_bks.iter().filter(|r| r.gap_pct >= 0.0 && r.gap_pct <= 2.0).count();
        let le_5      = with_bks.iter().filter(|r| r.gap_pct >= 0.0 && r.gap_pct <= 5.0).count();
        ...
        md.push_str(&format!("| Exact BKS (0%) | {} | {:.1}% |\n", exact_bks, exact_bks as f64 / feasible_with_bks_n as f64 * 100.0));
        md.push_str(&format!("| ≤0.5% | {} | {:.1}% |\n", le_half, le_half as f64 / feasible_with_bks_n as f64 * 100.0));
        md.push_str(&format!("| ≤1% | {} | {:.1}% |\n", le_1, le_1 as f64 / feasible_with_bks_n as f64 * 100.0));
        md.push_str(&format!("| ≤2% | {} | {:.1}% |\n", le_2, le_2 as f64 / feasible_with_bks_n as f64 * 100.0));
        md.push_str(&format!("| ≤5% | {} | {:.1}% |\n", le_5, le_5 as f64 / feasible_with_bks_n as f64 * 100.0));
        md.push_str(&format!("| >5% | {} | {:.1}% |\n", gt_5, gt_5 as f64 / feasible_with_bks_n as f64 * 100.0));
        if neg_bks > 0 {
            md.push_str(&format!("| Better than BKS (<0%) | {} | {:.1}% |\n", neg_bks, neg_bks as f64 / feasible_with_bks_n as f64 * 100.0));
        }
        md.push_str(&format!("| Infeasible | {} | {:.1}% |\n", infeas_n, infeas_n as f64 / ran.len() as f64 * 100.0));
    }
    md.push('\n');

    md.push_str("## Vehicle Count Provenance\n\n");
    md.push_str("| Source | Count |\n|--------|-------|\n");
    md.push_str(&format!("| VEHICLES field | {} |\n", summary.vehicles_from_field));
    md.push_str(&format!("| COMMENT | {} |\n", summary.vehicles_from_comment));
    md.push_str(&format!("| Name pattern (-kN) | {} |\n", summary.vehicles_from_name));
    md.push_str(&format!("| Registry | {} |\n\n", summary.vehicles_from_registry));

    // ── Metadata Provenance ───────────────────────────────────────────────────
    md.push_str("## Metadata Provenance\n\n");
    md.push_str("This section describes the provenance of benchmark metadata used in this campaign.\n\n");
    md.push_str("### Distance Semantics\n\n");
    md.push_str("| Metric | Count |\n|--------|-------|\n");
    let n_euc2d = results.iter().filter(|r| r.distance_metric == "TspLibEuc2D").count();
    let n_explicit = results.iter().filter(|r| r.distance_metric == "ExplicitMatrix").count();
    let n_na = results.iter().filter(|r| r.distance_metric == "N/A").count();
    ...
    md.push_str("|--------|-----------|---------------------|---------------------|\n");
    let fam_qual_families = ["CMT", "Tai", "Golden", "Li", "X", "A", "B", "E", "P", "F", "M"];
    for fam_prefix in &fam_qual_families {
        // Find a representative instance from this family
        let rep = results.iter().find(|r| r.family.starts_with(fam_prefix) || r.name.starts_with(fam_prefix));
        if let Some(r) = rep {
            let qm = cvrplib_registry::qualification_metadata(&r.name);
            md.push_str(&format!("| {} | {} | {} | {} |\n",
                fam_prefix,
    ...
    let avg_evals = if ran.is_empty() { 0.0 } else { total_evals as f64 / ran.len() as f64 };
    let avg_conv_gen = if ran.is_empty() { 0.0 } else { ran.iter().map(|r| r.convergence_generation as f64).sum::<f64>() / ran.len() as f64 };
    let avg_stagnation = if ran.is_empty() { 0.0 } else { ran.iter().map(|r| r.stagnation_generation as f64).sum::<f64>() / ran.len() as f64 };
    let avg_vehicles_used = if feasible_ran.is_empty() { 0.0 } else { feasible_ran.iter().map(|r| r.vehicles_used as f64).sum::<f64>() / feasible_ran.len() as f64 };
    md.push_str("| Metric | Value |\n|--------|-------|\n");
    md.push_str(&format!("| Total evaluations (all instances) | {} |\n", total_evals));
    md.push_str(&format!("| Avg evaluations per instance | {:.0} |\n", avg_evals));
    md.push_str(&format!("| Avg convergence generation | {:.1} |\n", avg_conv_gen));
    md.push_str(&format!("| Avg stagnation generation | {:.1} |\n", avg_stagnation));
    md.push_str(&format!("| Avg vehicles used (feasible) | {:.2} |\n", avg_vehicles_used));
    ...
    let classes = ["solved","near_optimal","competitive","weak","poor","invalid","skipped","unsupported","no_ref"];
    for cls in &classes {
        let cnt = results.iter().filter(|r| r.quality_class == *cls).count();
        let pct = cnt as f64 / results.len() as f64 * 100.0;
        if cnt > 0 { md.push_str(&format!("| {} | {} | {:.1}% |\n", cls, cnt, pct)); }
    }
    md.push('\n');
    ...
    md.push_str("|--------|-----------|----------|----------|-------------|-------------|\n");
    let mut families: Vec<String> = results.iter().map(|r| r.family.clone()).collect::<std::collections::HashSet<_>>().into_iter().collect();
    families.sort();
    for fam in &families {
        let fam_results: Vec<&InstanceResult> = results.iter().filter(|r| &r.family == fam && (r.status == "ok" || r.status == "infeasible")).collect();
        if fam_results.is_empty() { continue; }
        let feas = fam_results.iter().filter(|r| r.feasible).count();
        let with_b: Vec<&InstanceResult> = fam_results.iter().filter(|r| r.bks.is_some() && r.feasible).copied().collect();
        let ag = if with_b.is_empty() { f64::NAN } else { with_b.iter().map(|r| r.gap_pct).sum::<f64>() / with_b.len() as f64 };
        let mut fg: Vec<f64> = with_b.iter().map(|r| r.gap_pct).collect(); fg.sort_by(|a,b| a.partial_cmp(b).unwrap());
        let mg = if fg.is_empty() { f64::NAN } else { fg[fg.len()/2] };
        let bm = with_b.iter().filter(|r| r.gap_pct.abs() < 0.01).count();
        md.push_str(&format!("| {} | {} | {} | {:.2} | {:.2} | {} |\n", fam, fam_results.len(), feas, ag, mg, bm));
    }
    md.push('\n');
    ...
    for r in sorted_by_gap.iter().take(20) {
        md.push_str(&format!("| {} | {} | {} | {:.2} | {:.2} | {:.2} | {} |\n",
            r.name, r.family, r.customers, r.bks.unwrap_or(0.0), r.best_distance, r.gap_pct, r.runtime_ms));
    }
    md.push('\n');
    ...
    for r in &results {
        md.push_str(&format!("| {} | {} | {} | {} | {} | {} | {:.2} | {:.2} | {} | {} | {} | {} | {} |\n",
            ...
            r.bks.map(|b| format!("{:.2}", b)).unwrap_or("-".to_string()),
            r.best_distance, r.gap_pct, r.quality_class, r.feasible, r.runtime_ms, r.generations, r.status));
    }
    md.push('\n');
    ...
    if !infeasible.is_empty() {
        md.push_str(&format!("### Pipeline Failures ({} infeasible)\n\n", infeasible.len()));
    ...
    let negative_gaps: Vec<&InstanceResult> = with_bks.iter().filter(|r| r.gap_pct < -0.01).copied().collect();
    if !negative_gaps.is_empty() {
        md.push_str(&format!("### Negative Gaps — Better Than BKS ({} instances, investigate)\n\n", negative_gaps.len()));
        md.push_str("| Instance | BKS | Best | Gap% |\n|----------|-----|------|------|\n");
        for r in &negative_gaps {
            md.push_str(&format!("| {} | {:.2} | {:.2} | {:.2} |\n", r.name, r.bks.unwrap_or(0.0), r.best_distance, r.gap_pct));
        }
        md.push('\n');
    }

    // Large regressions (>5% gap)
    let large_gaps: Vec<&InstanceResult> = with_bks.iter().filter(|r| r.gap_pct > 5.0).copied().collect();
    if !large_gaps.is_empty() {
        md.push_str(&format!("### Large Regressions >5% Gap ({} instances)\n\n", large_gaps.len()));
        md.push_str("| Instance | Family | BKS | Best | Gap% |\n|----------|--------|-----|------|------|\n");
        let mut lg = large_gaps.clone(); lg.sort_by(|a,b| b.gap_pct.partial_cmp(&a.gap_pct).unwrap());
        for r in lg.iter().take(30) {
            md.push_str(&format!("| {} | {} | {:.2} | {:.2} | {:.2} |\n", r.name, r.family, r.bks.unwrap_or(0.0), r.best_distance, r.gap_pct));
        }
        md.push('\n');
    }
    ...
    for fam in &families {
        let fam_ran: Vec<&InstanceResult> = ran.iter().filter(|r| &r.family == fam).copied().collect();
        if fam_ran.is_empty() { continue; }
        let infeas_rate = fam_ran.iter().filter(|r| !r.feasible).count() as f64 / fam_ran.len() as f64;
        let fam_with_bks: Vec<&InstanceResult> = fam_ran.iter().filter(|r| r.bks.is_some() && r.feasible).copied().collect();
        let fam_avg_gap = if fam_with_bks.is_empty() { 0.0 } else { fam_with_bks.iter().map(|r| r.gap_pct).sum::<f64>() / fam_with_bks.len() as f64 };
        if infeas_rate > 0.2 || fam_avg_gap > 10.0 {
            md.push_str(&format!("- **{}**: {:.0}% infeasible, avg gap {:.1}%\n", fam, infeas_rate*100.0, fam_avg_gap));
        }
    }
    md.push('\n');
    ...
    let infeasible_pct = if ran.is_empty() { 0.0 } else { infeasible_count as f64 / ran.len() as f64 * 100.0 };
    ...
    let buckets = [(0u128,100u128,"<100ms"),(100,500,"100-500ms"),(500,1000,"500ms-1s"),(1000,5000,"1-5s"),(5000,30000,"5-30s"),(30000,u128::MAX,"30s+")];
    for (lo, hi, label) in &buckets {
        let cnt = ran.iter().filter(|r| r.runtime_ms >= *lo && r.runtime_ms < *hi).count();
        md.push_str(&format!("| {} | {} |\n", label, cnt));
    }
    md.push('\n');
    ...
    let avg_p0_ms = if ran.is_empty() { 0.0 } else { ran.iter().map(|r| r.proc0_avg_ms).sum::<f64>() / ran.len() as f64 };
    md.push_str(&format!("| Total proc0 invocations | {} |\n", total_p0_inv));
    md.push_str(&format!("| Avg proc0 time per call (ms) | {:.3} |\n\n", avg_p0_ms));

    // Skipped/unsupported table
    md.push_str("### Skipped / Unsupported Instances\n\n");
    let skipped_unsupported: Vec<&InstanceResult> = results.iter().filter(|r| r.status == "skipped" || r.status == "unsupported").collect();
    if skipped_unsupported.is_empty() {
        md.push_str("None.\n\n");
    } else {
        md.push_str("| Instance | Status | Reason |\n|----------|--------|--------|\n");
        for r in &skipped_unsupported {
            md.push_str(&format!("| {} | {} | {} |\n", r.name, r.status, r.skip_reason.as_deref().unwrap_or("-")));
        }
        md.push('\n');
    }
    ...
    md.push_str(&format!("| Benchmark coverage | {}/{} instances executed ({:.1}% of total) |\n",
        ran.len(), results.len(), ran.len() as f64 / results.len() as f64 * 100.0));
    md.push_str(&format!("| Feasibility rate | {:.1}% |\n", summary.feasibility_rate));
    md.push_str(&format!("| BKS coverage | {}/{} ran instances have BKS reference |\n",
        with_bks.len(), ran.len()));
    md.push_str(&format!("| Qualification level | {:.1}% Verified or PartiallyVerified |\n", qual_confidence));
    ...
    md.push_str("- **X (Uchoa)**: Fleet semantics require confirmation against Uchoa et al. 2017.\n");
    md.push_str("- **Golden/Li**: Excluded from current scope (>200 customers). Registry metadata verified.\n\n");

    md.push_str("### Next Steps\n\n");
    md.push_str("1. Verify CMT BKS values against Christofides et al. 1979 original publication.\n");
    ...
    println!("╠══════════════════════════════════════════════════════════════╣");
    println!("║ Instances ran:    {:>5}                                      ║", summary.supported_instances);
    println!("║ Feasible:         {:>5} ({:.1}%)                           ║", summary.feasible_instances, summary.feasibility_rate);
    println!("║ BKS matches:      {:>5}                                      ║", summary.bks_matches);
    println!("║ Avg gap:          {:>7.2}%                                   ║", summary.avg_gap_pct);
