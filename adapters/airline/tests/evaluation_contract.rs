...
use coralys_airline::legality::{
    LegalityChecker, base_return, coverage, duty_connectivity, duty_time, fdp, minimum_rest,
    qualification,
};
...
fn test_legal_roster_evaluation() {
    ...
    assert_eq!(
        report.rules.len(),
        7,
        "Should produce exactly 7 RuleSummary entries"
    );
...
fn test_reduce_rest_produces_violation() {
    ...
    let roster = roster_fixtures::canonical_roster();
    let roster = transformations::reduce_rest(&roster, &CrewId::new("C1"))
    ...
    let min_rest_summary = report
        .rules
        .iter()
        .find(|r| r.rule_id == "minimum_rest")
        .unwrap();
    assert!(
        min_rest_summary.violations.len() > 0,
        "Should have a minimum_rest violation"
    );
    assert!(
        min_rest_summary.violations[0].is_error(),
        "Should be an Error severity violation"
    );
}
...
fn test_json_serialization_preserves_report() {
    ...
    let deserialized: coralys_airline::evaluation::LegalityReport =
        serde_json::from_str(&json).expect("Deserialization failed");

    assert_eq!(
        report, deserialized,
        "JSON serialization should preserve report"
    );
}
