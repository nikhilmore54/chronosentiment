use coralys_moga::ecology::{EcologyMemory, EcologyPolicy, EcologySignal};
...
impl WorkforceEcologyAdapter {
    ...
    pub fn compute_signal(&self, nurse: NurseId, num_nurses: usize) -> EcologySignal {
        let total_assignments: f64 = (0..num_nurses).map(|n| self.get_assignments(n)).sum();

        let mean = if num_nurses > 0 {
