...
#[command(
    ...
    author,
    version,
    about = "Run the UltraCrew optimizer on a dataset"
)]
struct Cli {
...
fn main() -> anyhow::Result<()> {
    ...
    let input_path = args.input.ok_or_else(|| {
        anyhow::anyhow!(
            "--input is required unless --health, --list-profiles, or --show-config is used"
        )
    })?;
    ...
    let mean_hours = if hours_values.is_empty() {
        0.0
    } else {
    ...
    eprintln!(
        "  Coverage:          {}/{} shifts ({:.1}%)",
        assigned_shifts, total_shifts, coverage_pct
    );
    ...
    eprintln!(
        "    Min hours/worker:  {:.1}h",
        if min_hours == f64::MAX {
            0.0
        } else {
            min_hours
        }
    ...
    eprintln!(
        "✅ Optimization complete – schedule written to {}",
        args.output.display()
    );
    Ok(())
}
