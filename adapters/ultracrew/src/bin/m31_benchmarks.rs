...
fn run_pass(
    args: &Args,
    initial_memory: Option<OpportunityMemory<InrcContextKey>>,
    record_oracle: bool,
) -> (String, HashSet<InrcContextKey>) {
    ...
    let max_duration = std::time::Duration::from_secs(if args.time_limit_secs > 0 {
    ...
    for w in 0..args.weeks {
        ...
        let factory = BenchmarkGenomeFactory {
            ...
            m26_enabled: args.m26,
            m27_enabled: args.m27,
            historical_m30: args.historical_m30,
        };
        ...
        for gen in 0..max_gen_per_week {
            ...
            if args.m30 && gen >= memory_engage_gen {
                // Engage memory
        ...
        if w == 3 {
            ...
            w3_hard = best_eval.hc_coverage
                + best_eval.hc_skills
                + best_eval.hc_one_shift_per_day
                + best_eval.hc_forbidden_successions;
        }
    ...
    let known_ratio = if total_evals > 0 {
        known_evals as f64 / total_evals as f64
    } else {
        0.0
    };
