...
fn run_ablation(seed: u64, arm: Arm, out_csv: &mut File) {
    let base_dir = std::path::PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("tests/data/n030w8");
    let scenario = parse_scenario(base_dir.join("Sc-n030w8.json")).unwrap();
    ...
    let h0 = parse_history(base_dir.join("H0-n030w8-0.json")).unwrap();
    let mut current_history = h0.clone();
    
    for w in 0..8 {
        let wd_path = base_dir.join(format!("WD-n030w8-{}.json", w));
        ...
        if w == 7 {
            ...
            // We use next_hist to get total assignments across all 8 weeks + H0.
...
fn main() {
    ...
    let output_file = "horizon_test_n030w8.csv";
    ...
    println!("F.2D.8C.1 Horizon Test (8 weeks)");
    ...
    for seed in seeds {
        for &arm in &arms {
            ...
            println!("  Seed {} Arm {:?} completed in {:.1}s", seed, arm, elapsed.as_secs_f64());
        }
    }
    
    println!("\nHorizon Test completed successfully.");
}
