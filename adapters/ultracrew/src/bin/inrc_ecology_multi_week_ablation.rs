...
use coralys_moga::traits::{GenomeFactory, MutationOperator, CrossoverOperator};
use rand::{Rng, SeedableRng};
use rand::rngs::StdRng;

use ultracrew::inrc::optimization::{
    InrcContext, InrcOptimizer, InrcGenome
};
...
fn run_ablation(seed: u64, alpha: f64) -> RunMetrics {
    ...
    for w in 0..4 {
        ...
        cum_hard += hc as i32;
        if best.soft_report.total_penalty < best_weekly { best_weekly = best.soft_report.total_penalty; }
                        ...
                        if req_level.minimum > 0 {
                            ...
                            for &n in &available {
                                ...
                                if nurse.skills.contains(&req.skill) {
                                    fulfilled += 1;
                                    if fulfilled == req_level.minimum { break; }
                                }
                            }
        ...
        for n in 0..num_nurses {
            let week_assignments = next_hist.nurse_history[n].number_of_assignments - current_history.nurse_history[n].number_of_assignments;
            let week_weekends = next_hist.nurse_history[n].number_of_working_weekends - current_history.nurse_history[n].number_of_working_weekends;
    ...
    let assignment_variance = ecology_state.cumulative_assignments.iter()
        .map(|&x| (x as f64 - mean_assignments).powi(2))
        .sum::<f64>() / num_nurses as f64;

    let mean_weekends = ecology_state.cumulative_weekends.iter().sum::<usize>() as f64 / num_nurses as f64;
    let weekend_concentration = ecology_state.cumulative_weekends.iter()
    ...
    // Assignment range
    let max_a = *ecology_state.cumulative_assignments.iter().max().unwrap_or(&0);
    let min_a = *ecology_state.cumulative_assignments.iter().min().unwrap_or(&0);
...
fn main() {
    ...
    while i < args.len() {
        match args[i].as_str() {
            ...
            "--alphas" => {
                i += 1;
                alphas = args[i].split(',')
                    .map(|s| s.trim().parse::<f64>().expect("--alphas requires comma-separated floats"))
                    .collect();
            },
            "--output" => {
                i += 1;
    ...
    writeln!(file, "seed,alpha,cum_score,cum_hard,assignment_variance,weekend_concentration,\
        best_weekly,mean_weekly,gini,persistence_score,assignment_range,\
        normalized_assignment_range,coefficient_of_variation,coverage_ratio").unwrap();
    ...
    for i in 0..num_seeds {
        ...
        for &alpha in &alphas {
            ...
            writeln!(file, "{},{:.2},{},{},{:.4},{:.4},{},{:.4},{:.4},{:.4},{},{:.4},{:.4},{:.4}",
                seed, alpha, metrics.cum_score, metrics.cum_hard,
                metrics.assignment_variance, metrics.weekend_concentration,
                metrics.best_weekly, metrics.mean_weekly, metrics.gini, metrics.persistence_score,
                metrics.assignment_range, metrics.normalized_assignment_range,
                metrics.coefficient_of_variation, metrics.coverage_ratio).unwrap();
        }
    }
