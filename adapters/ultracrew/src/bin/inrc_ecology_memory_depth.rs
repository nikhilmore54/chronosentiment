...
fn run_ablation(seed: u64, arm: DepthArm, out_csv: &mut File) {
    ...
    for w in 0..4 {
        ...
        if depth > 0 {
            let start_idx = if week_history.len() > depth { week_history.len() - depth } else { 0 };
            for hist_w in start_idx..week_history.len() {
                for n in 0..num_nurses {
                    adapter.memory.accumulate(n, "assignments", week_history[hist_w][n]);
                }
            }
        }
        ...
        for n in 0..num_nurses {
            this_week[n] = (next_hist.nurse_history[n].number_of_assignments - current_history.nurse_history[n].number_of_assignments) as f64;
        }
...
fn main() {
    ...
    let arms = vec![DepthArm::Off, DepthArm::Depth1w, DepthArm::Depth2w, DepthArm::Depth4w, DepthArm::DepthFull];
