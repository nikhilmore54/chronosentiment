...
fn run_pilot(seed: u64, arm: Arm, out_csv: &mut File) {
    ...
    for w in 0..num_weeks {
        ...
        if w == num_weeks - 1 {
            ...
            writeln!(
                out_csv,
                "{},{:?},{},{},{:.4}",
                seed, arm, total_score, total_hard, gini
            )
            .unwrap();
        }
...
fn main() {
    ...
    let output_file = "inrc_natural_history_pilot_10seed.csv";

    let mut file = File::create(output_file).unwrap();
    writeln!(file, "seed,arm,soft_penalty,hard_penalty,gini").unwrap();

    println!("F.4A Natural History Pilot");
    ...
    println!("\nNatural History Pilot completed successfully.");
}
