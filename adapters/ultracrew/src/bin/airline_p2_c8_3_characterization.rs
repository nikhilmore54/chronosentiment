...
fn main() {
    let utilizations = vec![85, 90, 95, 98, 99, 100, 110];
    let num_seeds = 30;
    
    let mut f = File::create("p2_c8_phase3_results.csv").unwrap();
    ...
    // Smoke test first
    println!("Running Smoke Test (Util=98%, Seeds 1-2)...");
    run_matrix(&mut f, vec![98], 2);
    
    println!("Running Full Matrix...");
