...
fn tournament_selection<'a>(
    evals: &'a [(usize, InrcEvaluation)],
    k: usize,
    rng: &mut StdRng,
) -> &'a (usize, InrcEvaluation) {
...
fn run_pilot(seed: u64, out_csv: &mut File) {
    ...
    for w in 0..num_weeks {
        ...
        for gen in 0..100 {
            ...
            let mut evals: Vec<(usize, InrcEvaluation)> = population
                .into_iter()
                .map(|(id, g)| {
                    (
                        id,
            ...
            if best_overall_eval.is_none()
                || gen_best.fitness() > best_overall_eval.as_ref().unwrap().fitness()
            {
            ...
            if global_gen % 10 == 0 {
                ...
                for (sample_id, sample) in samples {
                    ...
                    if best_fit > sample.fitness() {
                        let new_id = next_id;
                        next_id += 1;
                        let new_eval = evaluator.evaluate(
                            &best_cand,
                            &coralys_moga::runtime::optimization::metric::MetricReport::default(),
                        );
    ...
    for (i, node) in lineage.iter().enumerate() {
        ...
        if node.method == "mutation" || node.method == "crossover" {
            ga_interventions += 1;
        }
