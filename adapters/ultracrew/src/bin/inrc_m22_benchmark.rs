...
fn run_pilot(seed: u64, mode: Mode, out_csv: &mut File) {
    ...
    for w in 0..num_weeks {
        ...
        if rng_env.gen_bool(0.2) {
            ...
            for day in days {
                for shift_type in &scenario.shift_types {
                    week_data.shift_off_requests.push(
                        ultracrew::inrc::models::InrcShiftOffRequest {
        ...
        if mode == Mode::PureSA {
            ...
            let mut current_eval = evaluator.evaluate(
                &current_cand,
            ...
            for gen in 0..sa_steps {
                ...
                let neighbor_eval = evaluator.evaluate(
                    &neighbor,
                    &coralys_moga::runtime::optimization::metric::MetricReport::default(),
                );
                ...
                if delta > 0.0 || rng.gen_range(0.0..1.0) < (delta / t).exp() {
                    ...
                    if current_eval.fitness() > best_eval.fitness() {
                        ...
                        stats
                            .sa_counterfactual_gains
                            .push(score_before - score_after);
            ...
            for gen in 0..100 {
                ...
                if gen % 10 == 0 {
                    ...
                    if (mode == Mode::Archive || mode == Mode::ArchiveSA)
                        && !passive_archive.is_empty()
                    {
                        let num_samples = 3.min(passive_archive.len());
                        samples = passive_archive
                            ...
                            if best_fit > sample.fitness() {
                                *sample = evaluator.evaluate(&best_cand, &coralys_moga::runtime::optimization::metric::MetricReport::default());
                                ...
                                if best_fit > best_overall.as_ref().unwrap().fitness() {
                                    ...
                                    stats
                                        .sa_counterfactual_gains
                                        .push(score_before_sa - score_after_sa);
        ...
        if w == num_weeks - 1 {
            ...
            let mut lifetimes: Vec<_> = families
                .iter()
                .map(|f| f.last_seen_gen - f.birth_gen)
                .collect();
            lifetimes.sort_unstable();
            let median_lt = if lifetimes.is_empty() {
                0
            } else {
                lifetimes[lifetimes.len() / 2]
            };
            let p90_lt = if lifetimes.is_empty() {
                0
            } else {
                lifetimes[(lifetimes.len() as f64 * 0.9) as usize]
            };
            let max_lt = if lifetimes.is_empty() {
                0
            } else {
                *lifetimes.last().unwrap()
            };
            ...
            let delayed_payoff_ratio = if discovery_rate > 0 {
                delayed_payoff_count as f64 / discovery_rate as f64
            } else {
                0.0
            };
            let avg_time_to_peak = if discovery_rate > 0 {
                total_time_to_peak as f64 / discovery_rate as f64
            } else {
                0.0
            };
            let sa_conversion_rate = if discovery_rate > 0 {
                sa_conversions as f64 / discovery_rate as f64
            } else {
                0.0
            };
            ...
            let median_sa_gain = if sa_gains.is_empty() {
                0.0
            } else {
                sa_gains[sa_gains.len() / 2]
            };
            let p95_sa_gain = if sa_gains.is_empty() {
                0.0
            } else {
                sa_gains[(sa_gains.len() as f64 * 0.95) as usize]
            };
            ...
            writeln!(
                ...
                median_sa_gain,
                p95_sa_gain,
                stats.improvements_by_offspring,
                stats.improvements_by_sa,
                stats.champion_lineage_depth,
                stats.archive_pulls,
                stats.archive_pulls_to_new_best,
                stats.population_pulls_to_new_best
            )
            .unwrap();
        }
