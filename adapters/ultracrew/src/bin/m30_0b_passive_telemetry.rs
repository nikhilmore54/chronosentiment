...
fn genome_distance(a: &InrcGenome, b: &InrcGenome) -> usize {
    a.bits.iter().zip(b.bits.iter()).filter(|(x, y)| x != y).count()
}
...
fn main() {
    ...
    let context = Arc::new(InrcContext::new(scenario, week_data, current_history, WorkforceEcology::new()));
    ...
    let mut population = (0..population_size).map(|_| evaluator.create(&mut rng)).collect::<Vec<_>>();
    ...
    for gen in 0..generations {
        ...
        if evals.is_empty() {
            population = (0..population_size).map(|_| evaluator.create(&mut rng)).collect();
            continue;
        }
        ...
        for record in pending_telemetry.drain(..) {
            ...
            if found {
                ...
                writeln!(out_csv, "{},{},{},{},{},{},{},{},{},{}",
                    record.generation, record.parent_fitness,
                    record.parent_hc_cov, record.parent_hc_skills, record.parent_hc_1shift, record.parent_hc_forb,
                    record.parent_soft_pen, objective_delta, survived_generation, became_champion
                ).unwrap();
            }
        }
