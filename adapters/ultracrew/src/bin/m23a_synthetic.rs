...
fn dist_x(bits: &[bool]) -> usize {
    dist(bits, 10, 31)
}
fn dist_y(bits: &[bool]) -> usize {
    dist(bits, 40, 61)
}
fn dist_z(bits: &[bool]) -> usize {
    dist(bits, 70, 91)
}

fn mutate(genome: &mut BitGenome, rng: &mut StdRng) {
    let rate = 1.0 / genome.bits.len() as f64;
    for b in genome.bits.iter_mut() {
...
fn evaluate(genome: &BitGenome, gen: usize, penalty: f64, target: char) -> f64 {
    ...
    let hx = dist_x(&genome.bits) == 0;
    let hy = dist_y(&genome.bits) == 0;
    let hz = dist_z(&genome.bits) == 0;
    if gen <= 500 {
        if hx {
            base_score -= penalty;
        }
...
fn step_ga(pop: &Vec<(BitGenome, f64)>, rng: &mut StdRng, k: usize) -> Vec<BitGenome> {
    ...
    while next_pop.len() < pop.len() {
        if rng.gen_bool(0.9) {
            ...
            let p2 = tournament(pop, k, rng);
            let (mut c1, mut c2) = crossover(&pop[p1].0, &pop[p2].0, rng);
            mutate(&mut c1, rng);
            mutate(&mut c2, rng);
            next_pop.push(c1);
            if next_pop.len() < pop.len() {
                next_pop.push(c2);
            }
        } else {
            let p1 = tournament(pop, k, rng);
...
fn step_sa(
    ...
    let mut next_pop = Vec::with_capacity(pop.len());
    for (g, score) in pop {
        let mut best_cand = g.clone();
        let mut best_fit = *score;
        let mut curr_cand = best_cand.clone();
        let mut curr_fit = best_fit;
        let mut t = 100.0;
        let alpha = 0.95;
        for _ in 0..50 {
...
fn run_simulation(seed: u64, mode: &str, penalty: f64) -> RunMetrics {
    ...
    for gen in 0..=1000 {
        ...
        for g in &pop {
            evals.push((g.clone(), evaluate(g, gen, penalty, target)));
        }
        ...
        if gen == 501 && use_vault && !vault.is_empty() {
            ...
            if is_random_vault {
                ...
                vault.sort_by_key(|g| match target {
                    'X' => dist_x(&g.bits),
                    'Y' => dist_y(&g.bits),
                    _ => dist_z(&g.bits),
                });
                for i in 0..10.min(vault.len()) {
                    selected.push(vault[i].clone());
                }
            }
...
fn main() {
    ...
    writeln!(
        res_file,
        "mode,seed,target,recovery,vault_hit,vault_overlap,best_nv_fitness"
    )
    .unwrap();
