...
fn test_history_transition() {
    ...
    let mut set_assign = |n: usize, d: usize, s: usize| {
        let idx = n * (ctx_arc.num_days * ctx_arc.shift_types.len()) + d * ctx_arc.shift_types.len() + s;
        bits[idx] = true;
    };
    ...
    let n0 = next_hist.nurse_history.iter().find(|nh| nh.nurse == "HN_0").unwrap();
    let n1 = next_hist.nurse_history.iter().find(|nh| nh.nurse == "HN_1").unwrap();
    ...
    assert_eq!(n0.number_of_consecutive_assignments, 0, "n0 consec assignments");
    assert_eq!(n0.number_of_working_weekends, ctx_arc.history.nurse_history[0].number_of_working_weekends + 1, "n0 worked weekend because Sat was worked");
    assert_eq!(n0.number_of_assignments, ctx_arc.history.nurse_history[0].number_of_assignments + 4, "n0 assignments");
    ...
    assert_eq!(n1.number_of_consecutive_assignments, 2, "n1 consec assignments");
    assert_eq!(n1.number_of_working_weekends, ctx_arc.history.nurse_history[1].number_of_working_weekends + 1, "n1 worked weekend");
    assert_eq!(n1.number_of_assignments, ctx_arc.history.nurse_history[1].number_of_assignments + 2, "n1 assignments");
}
