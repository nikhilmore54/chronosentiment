...
use ultracrew::optimization::{ScheduleContext, ScheduleGenome, ScheduleOptimizer};
...
fn build_context(scenario: Option<InrcScenario>, shifts: Vec<Shift>) -> Arc<ScheduleContext> {
    let workers = vec![
        Worker {
            id: 1,
            skills: vec![Skill::new("Pilot")],
        },
        Worker {
            id: 2,
            skills: vec![Skill::new("Copilot")],
        },
    ];
...
fn case_001_double_booking() {
    let shifts = vec![
        Shift {
            ...
            start_hour: 0,
            duration_hours: 8,
            required_skill: Skill::new("Pilot"),
        },
        Shift {
            id: 103,
            start_hour: 4,
    ...
    assert!(
        hard_violations > 0,
        "Expected HC2 (double booking) violation"
    );
...
fn case_003_rest_8h00() {
    ...
    assert_eq!(
        hard_violations, 0,
        "Expected no hard violations for exactly 8h rest"
    );
...
fn case_006_unqualified_worker() {
    let shift1 = Shift {
        ...
        start_hour: 0,
        duration_hours: 8,
        required_skill: Skill::new("Copilot"),
    };
    ...
    assert!(
        hard_violations > 0,
        "Expected HC1 violation (unqualified worker)"
    );
    assert!(!is_valid, "Genome with hard violations must be invalid");
}
