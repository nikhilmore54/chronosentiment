...
fn test_level2_seed_robustness() {
    ...
    let variance = fitness_scores.iter().map(|f| (f - mean).powi(2)).sum::<f64>() / num_seeds as f64;
    ...
    assert!(satisfaction_rate > 90.0, "Constraint Satisfaction Rate too low: {}%", satisfaction_rate);
}
