...
fn test_inrc_multi_week_parity() {
    ...
    for w in 0..4 {
        ...
        if w == 0 {
            ...
            ultracrew::inrc::exporter::export_inrc_history(&current_history, &out_his_path)
                .unwrap();
        }
        ...
        println!(
            "Week {} Validator DaysOff: {}, Coralys: {}",
            w, val_days_off, evaluation.soft_report.day_off_penalty
        );
    ...
    println!(
        "Days Off             | {:<7} | {:<9} ",
        coralys_days_off_total, val_days_off_total
    );
    println!(
        "Preferences          | {:<7} | {:<9} ",
        coralys_preferences_total, val_preferences_total
    );
    println!(
        "Complete Weekends    | {:<7} | {:<9} ",
        coralys_weekends_total, val_weekends_total
    );
    println!(
        "Consecutive Work     | {:<7} | {:<9} ",
        coralys_consecutive_total, val_consecutive_total
    );
    println!(
        "Optimal Coverage     | {:<7} | {:<9} ",
        coralys_optimal_total, val_optimal_total
    );
