...
fn generate_sequential_scenario(
    week: usize,
...
fn run_4_weeks(enable_memory: bool, base_seed: u64) -> (f64, f64) {
    ...
    let max_hours = cumulative_hours
        .iter()
        .cloned()
...
fn test_level4_ecology_ablation() {
    ...
    let fatigue_improvement =
        ((mean_no_mem_max_fatigue - mean_mem_max_fatigue) / mean_no_mem_max_fatigue) * 100.0;
    let fairness_improvement =
        ((mean_no_mem_variance - mean_mem_variance) / mean_no_mem_variance) * 100.0;

    println!("--- LEVEL 4: ECOLOGY ABLATION (30 PAIRED RUNS) ---");
    println!(
        "Memory Disabled - Mean Max 4-Week Hours: {:.2}, Mean 4-Week Variance: {:.2}",
        mean_no_mem_max_fatigue, mean_no_mem_variance
    );
    println!(
        "Memory Enabled  - Mean Max 4-Week Hours: {:.2}, Mean 4-Week Variance: {:.2}",
        mean_mem_max_fatigue, mean_mem_variance
    );
    println!(
        "Fatigue (Max Hours) Improvement:  {:.2}%",
        fatigue_improvement
    );
    println!(
        "Fairness (Variance) Improvement: {:.2}%",
