...
pub fn compare_outcomes(out1: &OutcomeRecord, out2: &OutcomeRecord) -> Result<(), String> {
    if out1.decision_id != out2.decision_id {
        return Err("decision_id mismatch".to_string());
    }
    if out1.evaluation_timestamp != out2.evaluation_timestamp {
        return Err("evaluation_timestamp mismatch".to_string());
    }
    if out1.observation_end_timestamp != out2.observation_end_timestamp {
        return Err("observation_end_timestamp mismatch".to_string());
    }
    if out1.horizon != out2.horizon {
        return Err("horizon mismatch".to_string());
    }
    if out1.holding_period_days != out2.holding_period_days {
        return Err("holding_period_days mismatch".to_string());
    }
    if out1.exit_reason != out2.exit_reason {
        return Err("exit_reason mismatch".to_string());
    }
    if out1.outcome_return != out2.outcome_return {
        return Err("outcome_return mismatch".to_string());
    }
    if out1.mfe != out2.mfe {
        return Err("mfe mismatch".to_string());
    }
    if out1.mae != out2.mae {
        return Err("mae mismatch".to_string());
    }
    if out1.maximum_drawdown != out2.maximum_drawdown {
        return Err("maximum_drawdown mismatch".to_string());
    }
    if out1.realized_volatility != out2.realized_volatility {
        return Err("realized_volatility mismatch".to_string());
    }
    Ok(())
}
...
fn test_c2_outcome_determinism() {
    ...
    let out1 = engine.measure_outcome(
        decision_id,
        &strategy,
        &strat_meta,
        &future_observations,
    ...
    let out2 = engine.measure_outcome(
        decision_id,
        &strategy,
        &strat_meta,
        &future_observations,
...
fn test_c2_negative_determinism_fault_injection() {
    ...
    assert!(
        result.is_err(),
        "Test should have caught the mismatch in outcome due to mutated future reality!"
    );
}
