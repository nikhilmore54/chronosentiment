...
use chronosentiment_adapter::metrics::instrument::{
    InstrumentMetricEngine, SimpleMovingAverageMetric,
};
...
use chronosentiment_adapter::reasoning::decision::{Decision, DecisionEngine};
use chronosentiment_adapter::reasoning::historical_reasoning::{
    HistoricalCase, HistoricalReasoningEngine, HistoricalReasoningReport,
};
use chronosentiment_adapter::reasoning::strategy::{OpportunityStrategy, StrategyEngine};
...
pub async fn execute_replay(
    ...
    assert!(
        context
            .instrument_contexts
            .get(&target_instrument_id)
            .unwrap()
            .observations
            .iter()
            .all(|o| o.effective_from <= evaluation_timestamp),
    ...
    let historical_report = HistoricalReasoningEngine.evaluate_with_cases(
        &assessment_profile,
        evaluation_timestamp,
        historical_cases,
    )?;
...
pub fn compare_artifact_graphs(
    run1: &ReplayArtifactGraph,
    run2: &ReplayArtifactGraph,
) -> Result<(), Box<dyn Error>> {
    // Assert 1: UUID independence
    assert_ne!(
        run1.run_id, run2.run_id,
        "Artifact UUIDs must be independent!"
    );
    assert_ne!(
        run1.decision.decision_id, run2.decision.decision_id,
        "Decision UUIDs must differ!"
    );
    assert_ne!(
        run1.strategy.decision_id, run2.strategy.decision_id,
        "Strategy UUIDs (which wrap decision ID) must differ!"
    );

    // Assert 2: Content hash equivalence
    assert_eq!(
        run1.content_hash, run2.content_hash,
        "Artifact graph content hashes must match!"
    );

    // 3. Field-level Graph Comparison (to detect tampering)
    // Metric Layer
    if run1.metric_report.metrics.len() != run2.metric_report.metrics.len() {
        return Err("Metric layer difference!".into());
    }

    // Assessment Layer
    if run1.assessment_profile.assessments[0].confidence
        != run2.assessment_profile.assessments[0].confidence
    {
        return Err("Assessment layer difference!".into());
    }

    // Historical Reasoning Layer
    if run1.historical_report.cases.len() != run2.historical_report.cases.len() {
        return Err("Historical reasoning layer difference!".into());
    }
    if run1.historical_report.similarity_score != run2.historical_report.similarity_score {
        return Err("Historical similarity difference!".into());
    }

    // Decision Layer
    if run1.decision.opportunity != run2.decision.opportunity {
        return Err("Decision opportunity difference!".into());
    }
    if run1.decision.confidence.evidence_quality != run2.decision.confidence.evidence_quality {
        return Err("Decision confidence difference!".into());
    }

    // Strategy Layer
    if run1.strategy.expected_return != run2.strategy.expected_return {
        return Err("Strategy return difference!".into());
    }
    if run1.strategy.entry_zone.min != run2.strategy.entry_zone.min {
        return Err("Strategy entry zone difference!".into());
    }
...
async fn test_c1_replay_determinism_run_twice() {
    ...
    let mut repo = MockObservationRepo {
        observations: Vec::new(),
    };
    for i in 1..=60 {
        repo.observations.push(create_mock_obs(
    ...
    let valid_case = HistoricalCase {
        ...
        assessment_profile_hash: "".to_string(),
        knowledge_lake_version: "".to_string(),
        engine_version: "".to_string(),
    };

    let run1 = execute_replay(&repo, inst_id, t, vec![valid_case.clone()])
        .await
        .unwrap();
    let run2 = execute_replay(&repo, inst_id, t, vec![valid_case.clone()])
        .await
        .unwrap();
...
async fn test_c1_tamper_detection_on_all_layers() {
    ...
    let mut run2 = execute_replay(&repo, inst_id, t, vec![valid_case.clone()])
        .await
        .unwrap();
    run2.assessment_profile.assessments[0].confidence += 0.01;
    assert!(
        compare_artifact_graphs(&run1, &run2).is_err(),
        "Assessment tamper undetected"
    );
...
async fn test_c1_temporal_firewall_future_case() {
    ...
    assert!(
        result.unwrap_err().to_string().contains("from the future"),
        "Future cases must be REJECTED!"
    );
    ...
    let success = execute_replay(&repo, inst_id, t, vec![valid_case])
        .await
        .unwrap();

    // Prove Historical Reasoning actually uses the historical case
    assert_eq!(
        success.historical_report.cases.len(),
        1,
        "Historical cases retrieved: 1"
    );
    assert!(
        success.historical_report.similarity_score > 0.0,
        "Historical reasoning similarity > 0"
    );
}
...
async fn test_c1_temporal_firewall_future_observation() {
    ...
    struct LeakyRepo {
        obs: Vec<ValidatedObservation>,
    }
    #[async_trait]
    impl ValidatedObservationRepository for LeakyRepo {
        async fn store_observation(
            &self,
            _obs: &ValidatedObservation,
        ) -> Result<(), Box<dyn Error>> {
            Ok(())
        }
        async fn get_observations_as_of(
    ...
    let leaky_repo = LeakyRepo {
        obs: vec![create_mock_obs(
