...
use chronosentiment_adapter::repository::{InstrumentRepository, ObservationRepository, PostgresRepository};
...
#[sqlx::test]
async fn test_instrument_and_observation_repository(pool: sqlx::PgPool) -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Verify retrieval
    let fetched_inst = repo.get_by_symbol("NSE", "RELIANCE").await?.expect("Instrument should exist");
    assert_eq!(fetched_inst.display_symbol, "RELIANCE");
    assert_eq!(fetched_inst.provider_ids.get("kite_token").unwrap(), "738561");
    ...
    obs.instrument_id = Some(inst.id);
    obs.quality_score = 0.95;
    ...
    assert_eq!(as_of_future.len(), 1, "Should find the observation in the future query");
    ...
    assert_eq!(as_of_past.len(), 0, "Should NOT find the observation in the past query");
