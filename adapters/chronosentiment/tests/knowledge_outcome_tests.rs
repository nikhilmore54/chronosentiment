...
use chronosentiment_adapter::repository::knowledge::{ArtifactMetadata, ArtifactType, ArtifactRepository};
...
fn get_mock_strategy(decision_id: Uuid) -> OpportunityStrategy {
    OpportunityStrategy {
        ...
        entry_zone: PriceRange { min: 98.0, max: 102.0 },
...
fn get_mock_obs(time: DateTime<Utc>, inst_id: Uuid, close: f64, high: f64, low: f64) -> ValidatedObservation {
...
async fn test_phase_d_final_acceptance_gate() {
    let db_url = std::env::var("DATABASE_URL").unwrap_or_else(|_| "postgres://invalid:invalid@localhost:5432/invalid".to_string());
    ...
    // Day 2 (entry)
    future_obs.push(get_mock_obs(eval_time + chrono::Duration::days(2), inst_id, 100.0, 101.0, 99.0));
    
    // Day 8 (between 5D and 10D)
    future_obs.push(get_mock_obs(eval_time + chrono::Duration::days(8), inst_id, 105.0, 106.0, 104.0));
    
    // Day 15 (between 10D and 20D)
    future_obs.push(get_mock_obs(eval_time + chrono::Duration::days(15), inst_id, 112.0, 115.0, 110.0)); // Hits target!
    
    // Day 25 (between 20D and 60D)
    future_obs.push(get_mock_obs(eval_time + chrono::Duration::days(25), inst_id, 85.0, 90.0, 80.0)); // Hits stop!
    
    // Day 70 (past 60D)
    future_obs.push(get_mock_obs(eval_time + chrono::Duration::days(70), inst_id, 200.0, 200.0, 200.0));
    
    // ---------------------------------------------------------
    // HORIZON MATRIX
    // ---------------------------------------------------------
    // Generate outcomes for all 4 horizons from the EXACT same frozen strategy
    let out_5d = engine.measure_outcome(decision_id, &strategy, &strat_meta, &future_obs, eval_time, 5, Some(inst_id));
    let out_10d = engine.measure_outcome(decision_id, &strategy, &strat_meta, &future_obs, eval_time, 10, Some(inst_id));
    let out_20d = engine.measure_outcome(decision_id, &strategy, &strat_meta, &future_obs, eval_time, 20, Some(inst_id));
    let out_60d = engine.measure_outcome(decision_id, &strategy, &strat_meta, &future_obs, eval_time, 60, Some(inst_id));
    ...
    assert_eq!(out_5d.strategy_id, strat_meta.artifact_id, "Same Strategy -> 5D");
    assert_eq!(out_10d.strategy_id, strat_meta.artifact_id, "Same Strategy -> 10D");
    assert_eq!(out_20d.strategy_id, strat_meta.artifact_id, "Same Strategy -> 20D");
    assert_eq!(out_60d.strategy_id, strat_meta.artifact_id, "Same Strategy -> 60D");
    ...
    let out_20d_b = engine.measure_outcome(decision_id, &strategy, &strat_meta, &future_obs, eval_time, 20, Some(inst_id));
    assert_ne!(out_20d.metadata.artifact_id, out_20d_b.metadata.artifact_id, "New evaluation = New artifact UUID");
    assert_eq!(out_20d.metadata.content_hash, out_20d_b.metadata.content_hash, "Same inputs = Same content hash");
    ...
    for out in [&out_5d, &out_10d, &out_20d, &out_60d] {
        assert!(out.metadata.lineage.parent_artifacts.contains(&decision_id), "Decision lineage");
        assert!(out.metadata.lineage.parent_artifacts.contains(&strat_meta.artifact_id), "Strategy lineage");
        assert_eq!(out.metadata.replay_context_hash, "mock_replay_hash_123", "Replay context");
        assert_eq!(out.metadata.knowledge_lake_version, "v14", "Knowledge Lake version");
        assert_eq!(out.metadata.engine_versions["OutcomeEngine"], "v1", "Engine version");
    }
    ...
    if has_db {
        ...
        assert!(store_res.is_ok(), "Failed to store outcome: {:?}", store_res.err());
        
        let dup_res = repo.store(&out_10d).await;
        assert!(dup_res.is_err(), "Duplicate UUID must be rejected");
        
        let fetched: OutcomeRecord = repo.get(out_10d.metadata.artifact_id).await.unwrap().expect("Outcome not found");
        assert_eq!(fetched.metadata.content_hash, out_10d.metadata.content_hash, "Round-trip hash");
        assert_eq!(fetched.outcome_return, out_10d.outcome_return, "Round-trip payload");
        assert_eq!(fetched.metadata.knowledge_lake_version, out_10d.metadata.knowledge_lake_version, "Round-trip metadata");
    }
...
async fn test_production_integration_replay_to_knowledge_lake() {
    ...
    // Mock future data
    let mut future_obs = Vec::new();
    future_obs.push(get_mock_obs(eval_time + chrono::Duration::days(1), inst_id, 100.0, 101.0, 99.0));
    future_obs.push(get_mock_obs(eval_time + chrono::Duration::days(10), inst_id, 115.0, 116.0, 114.0));
    ...
    for i in 0..n_decisions {
        ...
        for horizon in [5, 10, 20, 60] {
            let outcome = engine.measure_outcome(decision_id, &strategy, &strat_meta, &future_obs, eval_time, horizon, Some(inst_id));
            ...
            // Read back to verify
            let fetched: OutcomeRecord = repo.get(outcome.metadata.artifact_id).await.unwrap().expect("Outcome not found");
            assert_eq!(fetched.metadata.artifact_id, outcome.metadata.artifact_id);
        }
    }
    
    assert_eq!(persisted_outcomes, 4 * m_strategies, "Persisted outcome artifacts must be 4 x M");
