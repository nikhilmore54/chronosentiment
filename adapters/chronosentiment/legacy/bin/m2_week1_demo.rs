...
fn main() {
    ...
    println!("Target Instrument: {} ({})", instrument.display_symbol, instrument.exchange);

    // 2. Assign Profile
    let profile = ProfileAssigner::assign(&instrument);
    println!("Assigned Evaluation Profile: {} v{}", profile.name(), profile.version());
    println!();
