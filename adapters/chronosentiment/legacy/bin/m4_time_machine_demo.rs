...
use chronosentiment_adapter::reasoning::decision::{
    ConfidenceDecomposition, Decision, ExpectedHorizon as DecExpectedHorizon, Opportunity,
};
...
use chronosentiment_adapter::validation::replay_decision::{DecisionReplay, TemporalFirewall};
...
fn main() {
    ...
    println!(
        "   Hypothesis: {}",
        replay.hypotheses.hypotheses.first().unwrap().name
    );
    println!(
        "   Historical Reasoning: Found {} cases prior to {}",
        replay.historical_reasoning_report.cases.len(),
        evaluation_time
    );
    println!("   Strategy Generated:");
    println!(
        "     - Entry: {}-{}",
        replay.strategy.entry_zone.min, replay.strategy.entry_zone.max
    );
    println!(
        "     - Target: {}-{}",
        replay.strategy.target_zone.min, replay.strategy.target_zone.max
    );
    println!(
        "     - Stop: {}-{}\n",
    ...
    let outcome = OutcomeEngine.measure_outcome(
        Uuid::new_v4(),
        &replay.strategy,
    ...
    println!(
        "   Maximum Favourable Excursion (MFE): {:.1}%",
        outcome.mfe * 100.0
    );
    println!(
        "   Maximum Adverse Excursion (MAE): {:.1}%",
        outcome.mae * 100.0
    );
    println!("   Exit Reason: {}\n", outcome.exit_reason);
    ...
    println!(
        "   Aggregated across {} historical instances of this Assessment Profile:",
        calibration.sample_count
    );
    println!("   Win Rate: {:.1}%", calibration.win_rate * 100.0);
    println!(
        "   Median Return: {:.1}%",
        calibration.median_return * 100.0
    );
    println!("   Median MFE: {:.1}%", calibration.median_mfe * 100.0);
    println!("   Median MAE: {:.1}%", calibration.median_mae * 100.0);
    println!(
        "   Target Hit Rate: {:.1}%",
        calibration.target_hit_rate * 100.0
    );
    println!(
        "   Stop Hit Rate: {:.1}%",
        calibration.stop_hit_rate * 100.0
    );
