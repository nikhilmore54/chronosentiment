...
async fn main() -> Result<(), Box<dyn Error>> {
    ...
    let db_url = std::env::var("DATABASE_URL")
        .unwrap_or_else(|_| "postgres://nikhil@localhost:5432/postgres".to_string());
    ...
    for dt in &timestamps {
        for (inst_id, inst) in &instrument_map {
            ...
            for obs in &inst_context.observations {
                ...
                max_effective_from = Some(
                    max_effective_from.map_or(obs.effective_from, |m| m.max(obs.effective_from)),
                );
            }
