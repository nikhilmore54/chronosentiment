//! CS-P-006-C.3-D — live-rule ecology of sealed Search #2.
//!
...
use chronosentiment_adapter::decision_support::c3_rule_ecology::{
    analyze_live_rules, render_rule_ecology, SEARCH_THREE_AUTHORIZED,
};
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for rule in &report.live_rules {
        println!(
            "rule_{} n={} mean_v={:.6}",
            rule.rule_index, rule.n, rule.value.mean
        );
    }
...
fn parse_args() -> Result<(PathBuf, PathBuf), Box<dyn std::error::Error>> {
    ...
    Ok((
        ...
        output.unwrap_or_else(|| PathBuf::from(RESEARCH_DISCOVERY_TWO_DIR).join("rule_ecology")),
    ))
}
