...
use chronosentiment_adapter::ingestion::provider::{
    MarketDataProvider, TimeRange, ValidatedObservationTranslator,
};
...
struct ProfileStats {
    ...
    maes_20d: Vec<f64>,
    maes_60d: Vec<f64>,
    drawdowns_5d: Vec<f64>,
    drawdowns_10d: Vec<f64>,
    drawdowns_20d: Vec<f64>,
    drawdowns_60d: Vec<f64>,
}
...
impl ProfileStats {
    fn new(signature: String, hash: String) -> Self {
        Self {
            ...
            maes_20d: Vec::new(),
            maes_60d: Vec::new(),
            drawdowns_5d: Vec::new(),
            drawdowns_10d: Vec::new(),
            drawdowns_20d: Vec::new(),
            drawdowns_60d: Vec::new(),
        }
    }
}
...
fn compute_forward_stats(
    start_idx: usize,
    offset: usize,
    obs_list: &[ValidatedObservation],
    ...
    let entry_close = entry_obs
        .normalized_payload
        .get("close")
        .and_then(|v| v.as_f64())
        .unwrap_or(0.0);
    let exit_close = exit_obs
        .normalized_payload
        .get("close")
        .and_then(|v| v.as_f64())
        .unwrap_or(0.0);
    let entry_adj = entry_obs
        .normalized_payload
        .get("adj_close")
        .and_then(|v| v.as_f64())
        .unwrap_or(entry_close);
    let exit_adj = exit_obs
        .normalized_payload
        .get("adj_close")
        .and_then(|v| v.as_f64())
        .unwrap_or(exit_close);
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let repo = InMemoryObservationRepo {
        observations: all_obs.clone(),
    };
    ...
    for dt in &all_timestamps {
        ...
        for (inst_id, _inst) in &instrument_map {
            ...
            let profile =
                assessment_engine.assess(&metric_report, &[Concept::Trend, Concept::Momentum]);
            ...
            let stats = profile_stats
                .entry(hash.clone())
                .or_insert_with(|| ProfileStats::new(sig, hash));
            stats.n_samples += 1;
            ...
            let current_idx = instrument_history
                .iter()
                .position(|o| o.effective_from == *dt);
            if let Some(i) = current_idx {
                if let Some((ret, mfe, mae, dd)) = compute_forward_stats(i, 5, &instrument_history)
                {
                    stats.returns_5d.push(ret);
                    stats.mfes_5d.push(mfe);
                    stats.maes_5d.push(mae);
                    stats.drawdowns_5d.push(dd);
                }
                if let Some((ret, mfe, mae, dd)) = compute_forward_stats(i, 10, &instrument_history)
                {
                    stats.returns_10d.push(ret);
                    stats.mfes_10d.push(mfe);
                    stats.maes_10d.push(mae);
                    stats.drawdowns_10d.push(dd);
                }
                if let Some((ret, mfe, mae, dd)) = compute_forward_stats(i, 20, &instrument_history)
                {
                    stats.returns_20d.push(ret);
                    stats.mfes_20d.push(mfe);
                    stats.maes_20d.push(mae);
                    stats.drawdowns_20d.push(dd);
                }
                if let Some((ret, mfe, mae, dd)) = compute_forward_stats(i, 60, &instrument_history)
                {
                    stats.returns_60d.push(ret);
                    stats.mfes_60d.push(mfe);
                    stats.maes_60d.push(mae);
                    stats.drawdowns_60d.push(dd);
                }
            }
        }
    }
    ...
    for key in keys {
        ...
        let r5 = &stats.returns_5d;
        let r10 = &stats.returns_10d;
        let r20 = &stats.returns_20d;
        let r60 = &stats.returns_60d;

        let (p10_5, p90_5) = p10_p90(r5.clone());
        let (p10_10, p90_10) = p10_p90(r10.clone());
        let (p10_20, p90_20) = p10_p90(r20.clone());
        let (p10_60, p90_60) = p10_p90(r60.clone());

        println!(
            "| Positive return % | {:.1}% | {:.1}% | {:.1}% | {:.1}% |",
            win_rate(r5),
            win_rate(r10),
            win_rate(r20),
            win_rate(r60)
        ...
        println!(
            "| Median return     | {:.2}% | {:.2}% | {:.2}% | {:.2}% |",
            median(r5.clone()) * 100.0,
            median(r10.clone()) * 100.0,
            median(r20.clone()) * 100.0,
            median(r60.clone()) * 100.0
        );
        println!(
            "| Mean return       | {:.2}% | {:.2}% | {:.2}% | {:.2}% |",
            mean(r5) * 100.0,
        ...
        println!(
            "| Median MFE        | {:.2}% | {:.2}% | {:.2}% | {:.2}% |",
            median(stats.mfes_5d.clone()) * 100.0,
            median(stats.mfes_10d.clone()) * 100.0,
            median(stats.mfes_20d.clone()) * 100.0,
            median(stats.mfes_60d.clone()) * 100.0
        );
        println!(
            "| Median MAE        | {:.2}% | {:.2}% | {:.2}% | {:.2}% |",
            median(stats.maes_5d.clone()) * 100.0,
            median(stats.maes_10d.clone()) * 100.0,
            median(stats.maes_20d.clone()) * 100.0,
            median(stats.maes_60d.clone()) * 100.0
        );
        println!(
            "| Median drawdown   | {:.2}% | {:.2}% | {:.2}% | {:.2}% |",
            median(stats.drawdowns_5d.clone()) * 100.0,
            median(stats.drawdowns_10d.clone()) * 100.0,
            median(stats.drawdowns_20d.clone()) * 100.0,
            median(stats.drawdowns_60d.clone()) * 100.0
        );
        println!(
            "| P10 return        | {:.2}% | {:.2}% | {:.2}% | {:.2}% |",
            p10_5 * 100.0,
        ...
        if win_rate(r10) > best_win_rate {
            best_win_rate = win_rate(r10);
            best_horizon = "10D";
        }
        if win_rate(r20) > best_win_rate {
            best_win_rate = win_rate(r20);
            best_horizon = "20D";
        }
        if win_rate(r60) > best_win_rate {
            best_win_rate = win_rate(r60);
            best_horizon = "60D";
        }
