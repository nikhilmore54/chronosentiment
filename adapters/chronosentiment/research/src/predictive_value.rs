...
impl ResearchExperiment for PredictiveValueExperiment {
    ...
    async fn execute(
        ...
        let mut aggregate_map: HashMap<(String, String, String), Vec<serde_json::Value>> =
            HashMap::new();
        ...
        for ((sig_hash, sig, horizon), items) in aggregate_map {
            ...
            let entry_reached_count = items
                .iter()
                .filter(|i| i["entry_reached"].as_bool().unwrap_or(false))
                .count() as f64;
            let target_hit_count = items
                .iter()
                .filter(|i| i["target_hit"].as_bool().unwrap_or(false))
                .count() as f64;
            let stop_hit_count = items
                .iter()
                .filter(|i| i["stop_hit"].as_bool().unwrap_or(false))
                .count() as f64;
            ...
            let mut returns: Vec<f64> = items
                .iter()
                .map(|i| i["outcome_return"].as_f64().unwrap_or(0.0))
                .collect();
            let mut mfes: Vec<f64> = items
                .iter()
                .map(|i| i["mfe"].as_f64().unwrap_or(0.0))
                .collect();
            let mut maes: Vec<f64> = items
                .iter()
                .map(|i| i["mae"].as_f64().unwrap_or(0.0))
                .collect();
            let mut dds: Vec<f64> = items
                .iter()
                .map(|i| i["drawdown"].as_f64().unwrap_or(0.0))
                .collect();
