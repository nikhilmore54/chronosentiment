...
### RP-403 — Adaptive Candidate Generation and Diversity Recovery *(conditional on RP-402 evidence)*

**Gate:** RP-402 evidence satisfied. Proceed after root-cause analysis of setA-12 and setA-17.

**Question:** Can richer path diversity recover the remaining infeasible instances (setA-12 and setA-17) without sacrificing the gains already achieved by budget-aware transition planning?

**Hypothesis:** The remaining failures arise from insufficient path diversity rather than budget allocation. setA-12 requires alternative paths that the ECMP-aware greedy cannot generate. setA-17 may require a different demand prioritisation strategy given its budget=1 constraint.

**Evidence-driven framing (from RP-402):**
- setA-12 (budget=13): was feasible under RP-401C (26.12) but infeasible under RP-401D and RP-402. Consistent regression across two independent variants suggests a structural path-diversity limitation, not a budget problem.
- setA-17 (budget=1): the most constrained instance. Budget=1 allows at most one re-route. If infeasibility requires ≥2 re-routes, this is budget-limited, not path-diversity-limited. Root-cause analysis required before coding.

**Pre-coding root-cause analysis (required before RP-403 binary):**
1. setA-12 path audit: enumerate paths generated by RP-401C vs RP-402; identify structural differences.
...
- Select the combination that minimises the objective greedily, respecting budget constraint
- Measure: objective improvement vs RP-402, recovery of setA-12/17, regression check on all 20

**Expected binary:** `src/bin/rp403_adaptive_candidates.rs`

**Note:** The research question is not "more paths per se" but "the right alternative paths for the remaining hard instances." This framing aligns with what RP-402 has revealed about the remaining failures.
