...
| OBS-000 — Temporal Integrity Test Suite | **NEXT MILESTONE** — formal temporal firewall; 7 invariants: (1) T0 immutability — no observation process can modify any certified T0 field; (2) future-data exclusion — observation data with timestamp > T0 cannot participate in T0 creation/reconstruction; (3) boundary correctness — TARGET/REFERENCE_RISK/HORIZON cannot be classified using data that precedes T0 or wasn't available at observation time; (4) direction symmetry — LONG: target above T0 / risk below; SHORT: target below T0 / risk above; (5) append-only outcomes — outcome appended, never rewrites original decision; (6) replay determinism — same T0 snapshot + subsequent market observations → deterministic result; (7) no accidental research feedback — outcome cannot flow back into T0 decision; FUTURE-POISON TEST: T0 decision produced with future-poison data must either reject it or produce identical certified T0 result; must pass before LIVE-001 begins | 2026-08-19 |
| LIVE-001 — Live Market Data Ingestion | **PLANNED** — replace historical snapshot with fresh NSE market data; all C3-002 inputs (price, trend, momentum, volatility, volume, ATR) must come from same live snapshot; no mixing live price with stale derived features | 2026-08-19 |
