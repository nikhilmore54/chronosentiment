...
| LIVE-004 — Freshness + Provenance Certification | **PLANNED** — CERTIFIED = fresh enough data + complete C3-002 input snapshot (all inputs from same timestamp, no mixing) + reproducible pipeline + frozen policy artifact; data_age <= threshold → CERTIFIED; data_age > threshold → STALE/NOT_CERTIFIED; never label a stale decision as live; /live page shows market data age + last evaluation timestamp | 2026-08-19 |
