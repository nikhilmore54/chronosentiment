...
| LIVE-004 — Freshness + Provenance Certification | **NEXT MILESTONE** — pure certification boundary; consumes LIVE-001 snapshot + LIVE-002 state + LIVE-003 recommendation + frozen artifact identities; NO recalculation, NO new data fetch, NO algorithm changes; certification gates: (1) Freshness — source snapshot age <= threshold; (2) Snapshot coherence — all C3-002 inputs from same LIVE-001 boundary; (3) Completeness — required inputs exist for evaluated instrument; (4) Recommendation inputs — no required RecommendationEngine input silently substituted (relative_volume_20 SUBSTITUTED → DEGRADED); (5) Reproducibility — LIVE-003 reproducible from recorded artifacts; (6) Frozen artifacts — C3-002 hash + engine version match frozen identities; statuses: CERTIFIED (all gates pass) / DEGRADED (≥1 input substituted) / STALE (snapshot age > threshold) / INCOMPLETE (C3-002 inputs not from same coherent boundary); input_integrity field is SEPARATE from degradation_level — AVAILABLE/SUBSTITUTED/MISSING per input; determinism invariant: same artifacts → same certification always; provenance chain must answer: snapshot → C3-002 state → C3-002 artifact → RecommendationEngine → evidence store → recommendation → why CERTIFIED/DEGRADED/STALE/INCOMPLETE | 2026-08-19 |
| LIVE-005 — Live Decision Ledger | **PLANNED** — new live decisions append to ledger; /latest = newest per ticker; /history = all; historical 202-decision dataset = MVP baseline (separate from live prospective evidence); compare live distribution vs REC-BASELINE-001 (14 BUY/46 WATCH/41 NO_TRADE) as observable consequence of current market state | 2026-08-19 |
