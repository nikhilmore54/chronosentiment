...
| LIVE-001 — Live Market Data Ingestion | **NEXT MILESTONE** — prove that a fresh, complete, timestamp-consistent C3-002 input snapshot can be produced; AC: Snapshot ID + snapshot time + all C3-002 inputs (price, OHLCV, volume, derived) traceable to same coherent snapshot boundary; source timestamp recorded; LIVE-002 consumes snapshot without fetching anything else; LIVE-001 acquires reality, LIVE-002 interprets reality; historical csp006_p_enrich path kept separate — must not become LIVE-001 by accident | 2026-08-19 |
