...
| LIVE-005 — Live Decision Ledger | **NEXT MILESTONE** — pure append-only admission/ledger boundary; LIVE-005 records what LIVE-004 certified; it does not reinterpret what LIVE-004 certified; 7 ACs: (AC-L5-01) admission fidelity — CERTIFIED+DEGRADED admitted exactly as produced; (AC-L5-02) status preservation — certification_status never upgraded/downgraded by ledger; (AC-L5-03) T0 immutability — every T0 field immutable after insertion; (AC-L5-04) provenance completeness — every record links decision→LIVE-003→LIVE-002→LIVE-001→C3-002→engine→evidence→LIVE-004; (AC-L5-05) idempotency — replaying same LIVE-004 artifact must not create second T0 record; (AC-L5-06) no recomputation — no market-data fetch, no C3-002, no recommendation, no certification; (AC-L5-07) observation boundary — OBS-001 receives immutable T0 snapshot, subsequent observations appended separately; identity boundary: recommendation_id ≠ decision_id ≠ observation_id; admission policy: CERTIFIED+DEGRADED enter ledger; STALE+INCOMPLETE retained for audit only; OBS-001 primary cohort = CERTIFIED; DEGRADED = stratified secondary | 2026-08-20 |
| OBS-001 — Automatic Observation Engine | **PLANNED** — direction-aware MFE/MAE (LONG: max(price-ref)/ref; SHORT: max(ref-price)/ref); target_reached, risk_reached, first_exit, sessions_to_outcome; append-only, never modifies T0; sits on top of live T0 decisions; only after LIVE-005 complete | 2026-08-19 |
