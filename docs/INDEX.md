...
| LIVE-004 — Freshness + Provenance Certification | **PLANNED** — data_age <= threshold → CERTIFIED; data_age > threshold → STALE/NOT_CERTIFIED; never label a stale decision as live; /live page shows market data age + last evaluation timestamp | 2026-08-19 |
...
| OBS-001 — Automatic Observation Engine | **PLANNED** — direction-aware MFE/MAE (LONG: max(price-ref)/ref; SHORT: max(ref-price)/ref); target_reached, risk_reached, first_exit, sessions_to_outcome; append-only, never modifies T0; sits on top of live T0 decisions | 2026-08-19 |
