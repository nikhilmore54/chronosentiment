...
| LIVE-001 — Live Market Data Ingestion | **NEXT MILESTONE** — produce a persisted snapshot artifact (not in-memory); artifact fields: snapshot_id, snapshot_timestamp, market/session status, universe, instrument data, C3-002 source inputs, source/provider, source timestamps, acquisition timestamps, completeness status, provenance; all C3-002 inputs from same coherent snapshot boundary; LIVE-002 consumes that exact artifact without fetching anything else; first live decision must carry source_type=LIVE + snapshot_id + snapshot_timestamp (unmistakable provenance, separate from historical 202); historical csp006_p_enrich path frozen — must not become LIVE-001; start_backend.sh frozen as infrastructure | 2026-08-19 |
| LIVE-002 — Fresh C3-002 Evaluation | **PLANNED** — same frozen C3-002 policy + fresh inputs; algorithm unchanged; data_snapshot_timestamp + data_age_seconds mandatory in every certified decision | 2026-08-19 |
| LIVE-003 — Live RecommendationEngine Execution | **PLANNED** — same frozen RecommendationEngine v1; periodic evaluation cadence (1–5 min during market hours); measure evaluation/acquisition/generation latency | 2026-08-19 |
