    ...
    fn default() -> Self {
        StrategyEvaluation {
            ...
            strategy: Strategy {
                ...
                direction_bias: 50, vol_floor: 20, mom_floor: 20,
                edge_ratio: 150, participation_threshold: 30,
            },
            direction_ratio: 0.5,
            baseline_pnl: 0.0,
...
// Phase D.1.21: Reality DNA distance
let d5 = (a.direction_bias as f64 - b.direction_bias as f64).abs() / 50.0;
let d6 = (a.vol_floor as f64 - b.vol_floor as f64).abs() / 50.0;

((d1 + d2 + d3 + d4 + d5 + d6) / 6.0).min(1.0)
}
...
if rng.gen_bool(mutation_rate) {
    ...
    strategy.vol_floor = (strategy.vol_floor as i32 + rng.gen_range(-5..6))
        .clamp(5, 80) as u8;

    strategy.mom_floor = (strategy.mom_floor as i32 + rng.gen_range(-5..6))
        .clamp(5, 80) as u8;

    strategy.edge_ratio = (strategy.edge_ratio as i32 + rng.gen_range(-10..11))
        .clamp(100, 300) as u8;

    strategy.participation_threshold = (strategy.participation_threshold as i32 + rng.gen_range(-5..6))
        .clamp(10, 90) as u8;
}
}
...
let raw_bull = if is_bullish { (0.5 * norm_momentum) + (0.3 * norm_volume) + (0.2 * norm_vol_score) } else { 0.0 };
let raw_bear = if is_bearish { (0.5 * norm_momentum) + (0.3 * norm_volume) + (0.2 * norm_vol_score) } else { 0.0 };
...
let final_score = match strategy.direction_bias {
    ...
    _ => if bullish_score > bearish_score { bullish_score } else { bearish_score },
};
...
let regime = if norm_vol > high_vol_threshold {
    ...
    if is_bullish { MarketRegime::BullTrend } else if is_bearish { MarketRegime::BearTrend } else { MarketRegime::MeanReversion }
} else {
    MarketRegime::MeanReversion
};
...
pub(crate) fn ga_simulate_round_trip_at_cursor(
    ...
    let tp_dist = adjusted_atr * rr;
    let sl_dist = adjusted_atr * 1.0; // SL pegged to ATR unit
    
    let (tp_target, sl_target) = if is_long {
        (
            (buy_price as f64 + tp_dist) as u64,
            (buy_price as i64 - sl_dist as i64).max(1) as u64,
        )
    } else {
        (
            (buy_price as i64 - tp_dist as i64).max(1) as u64,
            (buy_price as f64 + sl_dist) as u64,
        )
    };

    if !is_long && std::env::var("GA_DEBUG").is_ok() {
        println!("SHORT_ENTRY → entry={} tp={} sl={}", buy_price, tp_target, sl_target);
    }
    ...
    for (j, event) in execution_events.iter().enumerate().skip(entry_idx + min_hold) {
        // Directional Excursion Tracking
        if is_long {
            ...
            // 🎯 Exit logic for SHORT (Symmetric Flip)
            if event.price >= sl_target {
                exit_price = (sl_target as f64 + slippage) as u64; 
                _exit_ts = event.exchange_ts;
                exit_event_idx = j;
                found_exit = true;
                exit_reason = GaExitReason::StopLoss;
                ...
                exit_price = (tp_target as f64 + slippage) as u64;
                _exit_ts = event.exchange_ts;
                exit_event_idx = j;
                found_exit = true;
                exit_reason = GaExitReason::TakeProfit;
    ...
    let efficiency = if realized_pnl >= 0.0 {
        ...
        let sl_delta = if is_long {
            (sl_target as f64 - buy_price as f64) / buy_price.max(1) as f64
        } else {
            (buy_price as f64 - sl_target as f64) / buy_price.max(1) as f64
        };
        let max_loss = sl_delta.min(-1e-6);
        let loss_eff = (realized_pnl / max_loss).clamp(0.0, 1.0);
        loss_eff * 0.2
    };

    if std::env::var("GA_DEBUG").is_ok() {
        println!("TRADE_DEBUG → side={} entry={} exit={} tp={} sl={} mfe={:.6} mae={:.6} pnl={:.6} eff={:.4}", 
...
pub(crate) fn evaluate_strategy(
    ...
    let is_struct_valid_adaptive = final_score >= (adaptive_threshold - 0.55) 
                                 && stability_raw > 0.3 
                                 && purity > 0.2 
                                 && agreement > 0.4; // Phase D.1.22: Hard AQG Baseline
    ...
    if !allow_bypass {
        ...
        if final_admission_reason.is_none() {
            ...
            let selection_th = DOMINANCE_FLOOR * 0.8;
            if best_dom < selection_th {
                if std::env::var("GA_DEBUG").is_ok() {
                    println!("WINDOW_REJECT → {} | dom={:.3} < th={:.3} => SKIP: LOW_CLARITY", 
    ...
    let mut entry_attempted: usize = 0;
    for (current_idx, conviction) in window_data {
        ...
        // HARD PARTICIPATION FILTER (Phase D.1.22)
        let threshold = (strategy.participation_threshold as f64 / 100.0).max(0.15);

        // Phase D.1.21 (Patch 7): Baseline Simulation (SMA crossover benchmark)
        let mut baseline_in_trade = false;
        let mut baseline_entry_price = 0.0;
        let mut baseline_bars_held = 0;
        
        for (i, bar) in signal_events.iter().enumerate() {
            let price = bar.price as f64;
            // Simple SMA-20 if enough history
            if i >= 20 {
                let sum: f64 = signal_events[(i-20)..i].iter().map(|b| b.price as f64).sum();
                let sma = sum / 20.0;
                
                if !baseline_in_trade && price > sma {
                    baseline_in_trade = true;
                    baseline_entry_price = price;
                    baseline_bars_held = 0;
                } else if baseline_in_trade {
                    baseline_bars_held += 1;
                    if baseline_bars_held >= 20 { // Fixed 20-bar hold benchmark
                        baseline_pnl += (price - baseline_entry_price) / baseline_entry_price;
                        baseline_in_trade = false;
                    }
                }
            }
        }
        ...
        // HARD PARTICIPATION FILTER (Phase D.1.22 Mechanical Correction)
        let threshold = (strategy.participation_threshold as f64 / 100.0).max(0.25);
        ...
        let aqg_threshold_local = adaptive_threshold; // Phase D.1.23: Use historical threshold for trade metrics

        // === PHASE D.1.23: DIRECTIONAL SPECIALIZATION LOCK ===
        // Determine side BEFORE simulation (Intent-Execution Separation)
        let is_long = match strategy.direction_bias {
            0..=25 => true,  // Overridden to Long if 100? Wait, check current mapping.
            // Mapping: 0=Short, 50=Dual, 100=Long. 
            // Correct mapping for Specialist Gates:
            // 0..=25   => Specialist SHORT (must be Short)
            // 75..=100 => Specialist LONG (must be Long)
            _ => conviction.bullish_score >= conviction.bearish_score,
        };
        
        // Final is_long decision with Purity Enforcement
        let side_is_long = if strategy.direction_bias <= 25 {
            false // Forced SHORT_ONLY specialist
        } else if strategy.direction_bias >= 75 {
            true  // Forced LONG_ONLY specialist
        } else {
        ...
        if let Some(outcome) = ga_simulate_round_trip_at_cursor(
            ...
            let aqg_health = if final_conviction.conviction_score >= aqg_threshold_local { 1.0 } else { 0.0 };
...
pub fn aggregate_strategy_reports(evaluations: Vec<StrategyEvaluation>, config: &GaConfig, generation: usize) -> Option<StrategyEvaluation> {
    ...
    let result = aggregate_strategy_reports_inner(elite_evals, scarcity_penalty, config, generation).map(|(e, _)| e);
    
    if let Some(ref best) = result {
        println!("PEAK_VS_BASELINE → ga={:.6}, baseline={:.6}", best.fitness, best.baseline_pnl);
    }
