use chronosentiment_core::ga::{Strategy, GaConfig, random_strategy, strategy_to_id, run_ga_evolution, ScenarioPair};
...
fn main() {
    ...
    for i in 0..2000 {
        ...
        mock_events.push(MarketEvent {
            ...
            price,
            quantity: 100,
            side: Some(if i % 2 == 0 { chronosentiment_core::Side::Buy } else { chronosentiment_core::Side::Sell }),
    ...
    println!("  - Final Gen Best Fitness: {:.4}", result.final_generation_best.fitness);
    println!("  - Organic Signal Flag (Best): {}", result.global_best.had_organic_signals);

    if let Some(ref report) = result.consensus_recommendations {
        println!("\n🔥 PHASE D.1.14: CONSENSUS TRUTH ({} active strategies):", report.active_strategies);
